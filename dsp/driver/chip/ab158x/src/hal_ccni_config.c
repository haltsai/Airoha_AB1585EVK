...
void dsp_profiling_config(hal_ccni_event_t event, void *msg){
    hal_ccni_status_t status;
    if (NULL != msg)
    mcu_to_dsp_info = (uint32_t *)hal_memview_infrasys_to_dsp0(((hal_ccni_message_t*)msg)->ccni_message[0]);
...
void ice_debug_dsp_test(hal_ccni_event_t event, void *msg)
{
    ...
    if (pMsg[1] == 1) {
        ...
        hal_gpt_delay_ms(60000);

        hal_nvic_restore_interrupt_mask(irq_status);
        log_hal_msgid_info("dsp 60s test ok\r\n", 0);
    }
...
const hal_ccni_function_t ccni_cm4_to_dsp0_function_table[CCNI_CM4_TO_DSP0_EVENT_MAX] = {
    ...
    {dsp_profiling_config},        /*CM4_TO_DSP0_WITH_MSG_CONFIG_PROI*/
    ...
    {default_isr},        /*CM4_TO_DSP0_WITHOUT_MSG_EVENT10*/
    {default_isr},        /*CM4_TO_DSP0_WITHOUT_MSG_EVENT11*/
