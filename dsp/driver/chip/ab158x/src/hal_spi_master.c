...
static const IRQn_Type g_spi_master_irq_code[HAL_SPI_MASTER_MAX] = {SPI_MST0_IRQn};
#ifdef HAL_CLOCK_MODULE_ENABLED
static const hal_clock_cg_id g_spi_master_cg_code[HAL_SPI_MASTER_MAX] = {HAL_CLOCK_CG_SPIMST0};
static const hal_clock_cg_id g_spi_master_bus_cg_code[HAL_SPI_MASTER_MAX] = {HAL_CLOCK_CG_SPIMST0_BUS};
// static const clock_mux_sel_id g_spi_master_cg_sel[HAL_SPI_MASTER_MAX] = {CLK_SPIMST0_SEL};
#endif
...
sleep_management_lock_request_t g_spi_master_sleep_handle[HAL_SPI_MASTER_MAX] = {SLEEP_LOCK_SPI_MST0};
sleep_management_backup_restore_module_t g_spi_master_sleep_backup_restore_handle[HAL_SPI_MASTER_MAX] = {SLEEP_BACKUP_RESTORE_SPI_MST0};
#endif
                                                ...
                                                uint32_t size)
{
    ...
    spi_master_set_rwaddr(master_port, SPI_MASTER_TX, hal_memview_mcu_to_infrasys(data));
                                                            ...
                                                            hal_spi_master_send_and_receive_config_t *spi_send_and_receive_config)
{
    ...
    if (spi_send_and_receive_config->send_data != NULL) {
        spi_master_set_rwaddr(master_port, SPI_MASTER_TX, hal_memview_mcu_to_infrasys(spi_send_and_receive_config->send_data));
    } else {
        spi_master_set_rwaddr(master_port, SPI_MASTER_TX, 0);
        spi_master_clear_fifo(master_port);
    }
    spi_master_set_rwaddr(master_port, SPI_MASTER_RX, hal_memview_mcu_to_infrasys(spi_send_and_receive_config->receive_buffer));
