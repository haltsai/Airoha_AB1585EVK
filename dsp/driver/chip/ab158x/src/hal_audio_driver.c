...
#endif
static int16_t ana_init_cnt;

extern uint16_t vow_pre_ch0_noise_msb;
extern uint16_t vow_pre_ch1_noise_msb;
#ifdef AIR_A2DP_NLE_ENABLE
static bool open_nle;
#endif
#ifdef AIR_RESET_SDM_ENABLE
static bool reset_sdm = false;
#endif

uint32_t g_afe_counter = 0; //== adc01_counter + adc23_counter + DAC_counter
uint32_t g_adc01_counter = 0; //== L+R
uint32_t g_adc01_L_counter = 0;
uint32_t g_adc01_R_counter = 0;

uint32_t g_adc23_counter = 0; //== L+R
uint32_t g_adc23_L_counter = 0;
uint32_t g_adc23_R_counter = 0;
...
*                                       Device agent                                       *
********************************************************************************************/
hal_audio_agent_t hal_device_convert_agent(hal_audio_control_t device, hal_audio_device_interface_t device_interface, bool is_tx)
{
    ...
    if (device & (HAL_AUDIO_CONTROL_DEVICE_ANALOG_MIC_DUAL | HAL_AUDIO_CONTROL_DEVICE_DIGITAL_MIC_DUAL | HAL_AUDIO_CONTROL_DEVICE_LOOPBACK | HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER | HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_L | HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_R | HAL_AUDIO_CONTROL_DEVICE_I2S_SLAVE | HAL_AUDIO_CONTROL_DEVICE_SPDIF)) {
        if (device_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2) {
            agent += 1;
        } else if (device_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_3) {
            agent += 2;
        } else if (device_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_4) {
            agent += 3;
        }
    }
...
hal_audio_device_agent_t hal_device_convert_device_agent(hal_audio_control_t device, hal_audio_device_interface_t device_interface)
{
    hal_audio_device_agent_t device_agent = HAL_AUDIO_DEVICE_AGENT_ERROR;
        ...
        switch (device) {
            ...
            case HAL_AUDIO_CONTROL_DEVICE_LINE_IN_R:
            case HAL_AUDIO_CONTROL_DEVICE_LINE_IN_DUAL:
                device_agent = HAL_AUDIO_DEVICE_AGENT_DEVICE_ADDA_UL2; //FORCE LINE-IN USE UL2
                break;
            case HAL_AUDIO_CONTROL_DEVICE_DIGITAL_MIC_L:
            case HAL_AUDIO_CONTROL_DEVICE_DIGITAL_MIC_R:
            case HAL_AUDIO_CONTROL_DEVICE_DIGITAL_MIC_DUAL:
            ...
            case HAL_AUDIO_CONTROL_DEVICE_INTERNAL_DAC_R:
            case HAL_AUDIO_CONTROL_DEVICE_INTERNAL_DAC_DUAL:
                device_agent = HAL_AUDIO_AGENT_DEVICE_ADDA_DL1;
                break;
            case HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER:
            case HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_L:
            case HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_R:
            ...
            case HAL_AUDIO_CONTROL_DEVICE_SPDIF:
                ...
                break;
            case HAL_AUDIO_CONTROL_DEVICE_ANC:
                device_agent = HAL_AUDIO_AGENT_DEVICE_ANC;
                break;
            case HAL_AUDIO_CONTROL_DEVICE_LOOPBACK:
                device_agent = HAL_AUDIO_DEVICE_AGENT_DEVICE_ADDA_UL1;
                break;
...
*                                       I2S master                                         *
********************************************************************************************/
afe_i2s_id_t hal_i2s_convert_id(hal_audio_control_t device, hal_audio_device_interface_t device_interface)
{
    ...
    if (device & (HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER | HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_L | HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_R | HAL_AUDIO_CONTROL_DEVICE_I2S_SLAVE)) {
        if (device_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_1) {
            i2s_id = AFE_I2S0;
        } else if (device_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2) {
            i2s_id = AFE_I2S1;
        } else if (device_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_3) {
            i2s_id = AFE_I2S2;
        } else if (device_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_4) {
...
afe_i2s_id_t hal_i2s_convert_id_by_agent(hal_audio_agent_t agent)
{
    ...
    switch (agent) {
        ...
        case HAL_AUDIO_AGENT_DEVICE_I2S2_SLAVE_RX:
            i2s_id = AFE_I2S2;
            break;
        case HAL_AUDIO_AGENT_DEVICE_I2S3_MASTER_DUAL_TX:
        case HAL_AUDIO_AGENT_DEVICE_I2S3_MASTER_L_TX:
        case HAL_AUDIO_AGENT_DEVICE_I2S3_MASTER_R_TX:
        case HAL_AUDIO_AGENT_DEVICE_I2S3_MASTER_DUAL_RX:
        case HAL_AUDIO_AGENT_DEVICE_I2S3_MASTER_L_RX:
        case HAL_AUDIO_AGENT_DEVICE_I2S3_MASTER_R_RX:
            i2s_id = AFE_I2S3;
            break;
    }
    return i2s_id;
...
bool hal_spdif_set_configuration(hal_audio_device_parameter_spdif_t *config)
{
    ...
    if (config->i2s_setting.i2s_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_1) {
        i2s_selection = AFE_SPDIF_SELECTION_I2S0;
    } else if (config->i2s_setting.i2s_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2) {
...
bool hal_updown_set_enable(afe_updown_sampler_id_t updown_id, bool enable)
{
    ...
    HAL_AUDIO_LOG_INFO("upwdown set enable, updown_id:%d, enable", 2, updown_id, enable);
    return false;
}
...
#ifdef AIR_RESET_SDM_ENABLE
bool hal_audio_dl_reset_sdm_enable(bool enable)
{
    if (enable) {
        if (!reset_sdm) {
            HAL_AUDIO_ENTER_CRITICAL();
            ...
            DSP_MW_LOG_I("[RESET SDM] disable\r\n", 1, enable);
            reset_sdm = true;
            HAL_AUDIO_EXIT_CRITICAL();
        }
    } else {
        if (reset_sdm) {
            //AFE_SET_REG(AFE_ADDA_DL_SDM_DCCOMP_CON, enable << AFE_ADDA_DL_SDM_RST_EN_POS, AFE_ADDA_DL_SDM_RST_EN_MASK);
            //DSP_MW_LOG_I("[RESET SDM] enable:%d\r\n", 1, enable);
            reset_sdm = false;
        }
    }
    return false;
...
bool hal_audio_dl_set_sdm_enable(bool enable)
{
    if (enable) {
        ...
        AFE_SET_REG(AFE_ANA_GAIN_MUX, 0x50, 0x005f);    //set HP gain control to ZCD module
    } else {
...
#ifdef AIR_A2DP_NLE_ENABLE
static U32 AFE_NLE_GAIN_LIMIT_DB = -20; // RANGE: 0~-44
bool hal_audio_dl_set_nle_gain(bool enable)
{
    if (enable) {
        if (open_nle == true) {
            return false;
        } else if (open_nle == false) {
            ...
            //if (hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_ANC)) {
                //return false;
            //}
            AFE_SET_REG(AFE_DL_NLE_L_CFG0, ((1 << 0) | (12 - A_GAIN_L_DB) << 16) | (D_GAIN_L_DB << 24), (1 << 0) | (0x3f << 16) | (0x3f << 24)); //set L-ch digital and analog target gain (d_gain+threshold_db,a_gain-threshold_db)
            AFE_SET_REG(AFE_DL_NLE_R_CFG0, ((1 << 0) | (12 - A_GAIN_R_DB) << 16) | (D_GAIN_R_DB << 24), (1 << 0) | (0x3f << 16) | (0x3f << 24)); //set R-ch digital and analog target gain (d_gain+threshold_db,a_gain-threshold_db)
        ...
        if (open_nle == false) {
            return false;
        } else if (open_nle == true) {
            int A_GAIN_L_DB = 12 - (AFE_READ(ZCD_CON2) & 0x3f);                 // read L-ch analog initial gain
            int A_GAIN_R_DB = 12 - (AFE_READ(ZCD_CON2) & 0xfc0 >> 6);           // read R-ch analog initial gain
            int D_GAIN_L_DB = 0;
            int D_GAIN_R_DB = 0;

            AFE_SET_REG(AFE_DL_NLE_L_CFG0, ((1 << 0) | (12 - A_GAIN_L_DB) << 16) | (0 << 24), (1 << 0) | (0x3f << 16) | (0x3f << 24)); // set L-ch analog initial gain
            AFE_SET_REG(AFE_DL_NLE_R_CFG0, ((1 << 0) | (12 - A_GAIN_R_DB) << 16) | (0 << 24), (1 << 0) | (0x3f << 16) | (0x3f << 24)); // set R-ch analog initial gain
...
void hal_audio_afe_enable_common_global(afe_analog_select_t analog_select, bool enable)
{
    ...
    if (enable) { //Global On
        if ((analog_select == AFE_ANALOG_ADC0) || (analog_select == AFE_ANALOG_ADC1) || (analog_select == AFE_ANALOG_DAC)) {
            ...
            ana_init_cnt ++;
            HAL_AUDIO_LOG_INFO("[Audio Driver]AFE COMMON GLOBAL ANALOG INITIAL CNT = %d", 1, ana_init_cnt);
            if (ana_init_cnt == 1) {
...
void hal_audio_ana_enable_ADC_common_global(afe_analog_select_t analog_select, afe_analog_control_t analog_control, bool enable)
{
    ...
    if (enable) {
        ...
        ANA_WRITE(AUDENC_ANA_CON27, 0x0002);
...
void hal_audio_ana_set_ADC_global(afe_analog_select_t analog_select, afe_analog_control_t analog_control, uint8_t performance, bool enable)
{
    ...
    if (enable) { //Global On,
...
#if 1 //bit6 up -->//20220125 update R-only test
                    ANA_SET_REG((AUDENC_ANA_CON3), (1 << AUDENC_ANA_CON3_PGAL_ACCFS_POS), (AUDENC_ANA_CON3_PGAL_ACCFS_MASK));
#endif
                } else { //0x00C0
                    ANA_SET_REG((AUDENC_ANA_CON3),
                                ((1 << AUDENC_ANA_CON3_PGAR_ACCFS_POS) | (1 << AUDENC_ANA_CON3_PGAL_ACCFS_POS)),
                                ((AUDENC_ANA_CON3_PGAR_ACCFS_MASK | AUDENC_ANA_CON3_PGAL_ACCFS_MASK)));
                }
            }
            if ((analog_control & AFE_ANALOG_L_CH)) {
        ...
        //PGA23
        if (analog_select == AFE_ANALOG_ADC1) { //wanna disable ADC23
            if ((analog_control & AFE_ANALOG_L_CH)) {
                ANA_SET_REG(AUDENC_ANA_CON12, 0 << AUDENC_ANA_CON12_PGAL_ACCFS_POS, AUDENC_ANA_CON12_PGAL_ACCFS_MASK);
            }
            if ((analog_control & AFE_ANALOG_R_CH)) {
                ANA_SET_REG(AUDENC_ANA_CON12, 0 << AUDENC_ANA_CON12_PGAR_ACCFS_POS, AUDENC_ANA_CON12_PGAR_ACCFS_MASK);
            }
            HAL_AUDIO_DELAY_US(1000);
            if ((analog_control & AFE_ANALOG_L_CH)) {
                ANA_WRITE(AUDENC_ANA_CON9, 0x0); //L
            }
            if ((analog_control & AFE_ANALOG_R_CH)) {
                ANA_WRITE(AUDENC_ANA_CON10, 0x0); //R
            }
        }
        //PGA01
        if (analog_select == AFE_ANALOG_ADC0) { //wanna disable ADC01
            if ((analog_control & AFE_ANALOG_L_CH)) {
                ANA_SET_REG(AUDENC_ANA_CON3, 0 << AUDENC_ANA_CON3_PGAL_ACCFS_POS, AUDENC_ANA_CON3_PGAL_ACCFS_MASK);
            }
            if ((analog_control & AFE_ANALOG_R_CH)) {
                ANA_SET_REG(AUDENC_ANA_CON3, 0 << AUDENC_ANA_CON3_PGAR_ACCFS_POS, AUDENC_ANA_CON3_PGAR_ACCFS_MASK);
            }
            HAL_AUDIO_DELAY_US(1000);
            if ((analog_control & AFE_ANALOG_L_CH)) {
                ANA_WRITE(AUDENC_ANA_CON0, 0x0); //L
            }
            if ((analog_control & AFE_ANALOG_R_CH)) {
                ANA_WRITE(AUDENC_ANA_CON1, 0x0); //R
            }
        }
        //CLK
        if ((analog_control & AFE_ANALOG_COMMON) && ((g_adc01_counter + g_adc23_counter) == 0)) { ////should be last ADC
            //ANA_SET_REG(AUDDEC_ANA_CON9, 0<<AUDDEC_ANA_CON9_DECODER_RST_POS, AUDDEC_ANA_CON9_DECODER_RST_MASK); //disable RESETB
        }
        ...
        if (analog_select == AFE_ANALOG_ADC0) { //wanna disable ADC01
            if ((analog_control & AFE_ANALOG_L_CH)) {
                ANA_SET_REG(AUDENC_ANA_CON37, 0 << AUDENC_ANA_CON37_PREAMP_L_REV_POS, AUDENC_ANA_CON37_PREAMP_L_REV_MASK);
                ANA_SET_REG(AUDENC_ANA_CON34, 0 << AUDENC_ANA_CON34_ADC01LDO_L_SW_EN_POS, AUDENC_ANA_CON34_ADC01LDO_L_SW_EN_MASK);
            }
            if ((analog_control & AFE_ANALOG_R_CH)) {
                ANA_SET_REG(AUDENC_ANA_CON38, 0 << AUDENC_ANA_CON38_PREAMP_R_REV_POS, AUDENC_ANA_CON38_PREAMP_R_REV_MASK);
                ANA_SET_REG(AUDENC_ANA_CON34, 0 << AUDENC_ANA_CON34_ADC01LDO_R_SW_EN_POS, AUDENC_ANA_CON34_ADC01LDO_R_SW_EN_MASK);
            }
        }
        ...
        if (analog_select == AFE_ANALOG_ADC1) { //wanna disable ADC23
            if ((analog_control & AFE_ANALOG_L_CH)) {
                ANA_SET_REG(AUDENC_ANA_CON40, 0 << AUDENC_ANA_CON40_PREAMP_L_REV_POS, AUDENC_ANA_CON40_PREAMP_L_REV_MASK);
                ANA_SET_REG(AUDENC_ANA_CON35, 0 << AUDENC_ANA_CON35_ADC23LDO_L_SW_EN_POS, AUDENC_ANA_CON35_ADC23LDO_L_SW_EN_MASK);
            }
            if ((analog_control & AFE_ANALOG_R_CH)) {
                ANA_SET_REG(AUDENC_ANA_CON41, 0 << AUDENC_ANA_CON41_PREAMP_R_REV_POS, AUDENC_ANA_CON41_PREAMP_R_REV_MASK);
                ANA_SET_REG(AUDENC_ANA_CON35, 0 << AUDENC_ANA_CON35_ADC23LDO_R_SW_EN_POS, AUDENC_ANA_CON35_ADC23LDO_R_SW_EN_MASK);
            }
        }

        //common in ADC
        if ((g_adc01_counter == 0) && (g_adc23_counter == 0)) { //should be last ADC
            hal_audio_ana_enable_ADC_common_global(analog_select, analog_control, false);
        }
        //common in ADC&DAC
...
bool hal_audio_ana_set_adc0_enable(hal_audio_device_parameter_adc_t *adc_parameter, afe_analog_control_t analog_control, bool enable)
{
    ...
    if (enable) {
        ...
        //afe_volume_analog_set_gain_by_index(AFE_HW_ANALOG_GAIN_INPUT1, 0, 0);
        ...
        if ((analog_control & AFE_ANALOG_L_CH)) {
            if (adc_parameter->performance == AFE_PEROFRMANCE_ULTRA_LOW_POWER_MODE) { //ULP = 1001 = 9
                ANA_SET_REG((AUDENC_ANA_CON34), (9 << AUDENC_ANA_CON34_L_CLK_POS), AUDENC_ANA_CON34_L_CLK_MASK);
            } else { //others(HP/NM/LP) = 1000 = 8
                ANA_SET_REG((AUDENC_ANA_CON34), (8 << AUDENC_ANA_CON34_L_CLK_POS), AUDENC_ANA_CON34_L_CLK_MASK);
            }
        }
        if ((analog_control & AFE_ANALOG_R_CH)) {
            if (adc_parameter->performance == AFE_PEROFRMANCE_ULTRA_LOW_POWER_MODE) { //ULP = 1001
                ANA_SET_REG((AUDENC_ANA_CON34), (9 << AUDENC_ANA_CON34_R_CLK_POS), AUDENC_ANA_CON34_R_CLK_MASK);
            } else { //others(HP/NM/LP)  = 1000
                ANA_SET_REG((AUDENC_ANA_CON34), (8 << AUDENC_ANA_CON34_R_CLK_POS), AUDENC_ANA_CON34_R_CLK_MASK);
            }
        }
        ...
        if (adc_parameter->performance == AFE_PEROFRMANCE_ULTRA_LOW_POWER_MODE) { //update ULP sequence on Nov.11
            ANA_WRITE(AUDENC_ANA_CON3, 0x0230);
        }
        ...
        if (!(analog_control & AFE_ANALOG_COMMON)) {
            ...
            if ((ana_clk_sel && (analog_control & AFE_ANALOG_R_CH)) ||
                (!ana_clk_sel && (analog_control & AFE_ANALOG_L_CH))) {
                HAL_AUDIO_LOG_WARNING("[ADC Driver]DSP - Warning Hal Audio Device ADC clock select %d :0x%x @@", 2, ana_clk_sel, AFE_GET_REG(AFUNC_AUD_CON4));
                //Disable UL path
                hal_audio_ul_set_enable(HAL_AUDIO_AGENT_DEVICE_ADDA_UL1_DUAL, false);
                hal_audio_ul_reset_fifo(HAL_AUDIO_AGENT_DEVICE_ADDA_UL1_DUAL, true);
                ...
                //Enable UL path
                hal_audio_ul_reset_fifo(HAL_AUDIO_AGENT_DEVICE_ADDA_UL1_DUAL, false);
                hal_audio_ul_set_enable(HAL_AUDIO_AGENT_DEVICE_ADDA_UL1_DUAL, true);
            }
        }
        ...
        if (g_adc01_L_counter == 0) { //last L user
            ANA_WRITE(AUDENC_ANA_CON0, 0x0); //0x200
        }
        if (g_adc01_R_counter == 0) {
            ANA_WRITE(AUDENC_ANA_CON1, 0x0); //0x204
        }
        if (g_adc01_counter == 0) { //last ADC01 user, close all ADC01

            ////OFF
            ANA_WRITE(AUDENC_ANA_CON37, 0x0); //0x294
            ANA_WRITE(AUDENC_ANA_CON38, 0x0); //0x298
            ANA_WRITE(AUDENC_ANA_CON34, 0x0);//0x288
            if(g_adc23_counter > 0){ //set when only ADC23 exist
                //ANA_WRITE(AUDENC_ANA_CON34, 0x3030); //bit4,5,12,13, 0330update
                ANA_SET_REG((AUDENC_ANA_CON34),
                            ((1 << AUDENC_ANA_CON34_ADC01_L_CLK_FROM_DL_POS)|(1 << AUDENC_ANA_CON34_ADC01LDO_L_SW_EN_POS)|(1 << AUDENC_ANA_CON34_ADC01_R_CLK_FROM_DL_POS)|(1 << AUDENC_ANA_CON34_ADC01LDO_R_SW_EN_POS)),
                            (AUDENC_ANA_CON34_ADC01_L_CLK_FROM_DL_MASK|AUDENC_ANA_CON34_ADC01LDO_L_SW_EN_MASK|AUDENC_ANA_CON34_ADC01_R_CLK_FROM_DL_MASK|AUDENC_ANA_CON34_ADC01LDO_R_SW_EN_MASK));
            ...
            ANA_WRITE(AUDENC_ANA_CON7, 0x0);//0x21C
            ANA_WRITE(AUDENC_ANA_CON2, 0x0);//0x208
...
bool hal_audio_ana_set_adc23_enable(hal_audio_device_parameter_adc_t *adc_parameter, afe_analog_control_t analog_control, bool enable)
{
    ...
    if (enable) {
        ...
        //afe_volume_analog_set_gain_by_index(AFE_HW_ANALOG_GAIN_INPUT2, 0, 0);
        ...
        if (!(analog_control & AFE_ANALOG_COMMON)) {
            ...
            if ((ana_clk_sel && (analog_control & AFE_ANALOG_R_CH)) ||
                ...
                //Disable UL path
                hal_audio_ul_set_enable(HAL_AUDIO_AGENT_DEVICE_ADDA_UL2_DUAL, false);
                hal_audio_ul_reset_fifo(HAL_AUDIO_AGENT_DEVICE_ADDA_UL2_DUAL, true);
                //Switch to ch2 while closing ch3
                ...
                //Enable UL path
                hal_audio_ul_reset_fifo(HAL_AUDIO_AGENT_DEVICE_ADDA_UL2_DUAL, false);
                hal_audio_ul_set_enable(HAL_AUDIO_AGENT_DEVICE_ADDA_UL2_DUAL, true);
            }
        }
        ...
        if (g_adc23_counter == 0) { //last ADC23 user, close all ADC23
            ...
            ////OFF
            ANA_WRITE(AUDENC_ANA_CON40, 0x0); //0x2A0
            ANA_WRITE(AUDENC_ANA_CON41, 0x0); //0x2A4
            ANA_WRITE(AUDENC_ANA_CON35, 0x0);//0x28C
            ANA_WRITE(AUDENC_ANA_CON16, 0x0);//0x240
            ANA_WRITE(AUDENC_ANA_CON11, 0x0);//0x22C
...
bool hal_audio_ana_set_dac_classg_enable(hal_audio_device_parameter_dac_t *dac_parameter, afe_analog_control_t analog_control, bool enable)
{
    ...
    HAL_AUDIO_LOG_INFO("DSP - Hal Audio ANA DAC:0x%x, Mode:%d, Performance:%d, dc_compensation:0x%x, first_boot=%d, CLASS-G Enable:%d", 6, analog_control, dac_parameter->dac_mode, dac_parameter->performance, dac_parameter->dc_compensation_value, first_boot, enable);
    HAL_AUDIO_ENTER_CRITICAL();
    if (enable) {
        ...
        if ((dac_parameter->dc_compensation_value) || (!first_boot)) {
            if (dac_parameter->dac_mode == HAL_AUDIO_ANALOG_OUTPUT_CLASSAB) {
                ...
                AFE_SET_REG(AFE_CLASSOP_CFG0,
                            ...
                            (AFE_CLASSOP_CFG0_USE_ANC_EN_MASK | AFE_CLASSOP_CFG0_USE_ANC_DATA_MASK));
#endif
                AFE_WRITE(AFE_CLASSG_CFG0, 0x11004201);              //level down hold time setting and classg enable
#else
        ...
        ANA_WRITE(AUDDEC_ANA_CON4, 0x00000000);
        ...
        if (dac_parameter->dac_mode == HAL_AUDIO_ANALOG_OUTPUT_CLASSAB && (dac_parameter->performance == AFE_PEROFRMANCE_HIGH_MODE)) {
        ...
        HAL_AUDIO_DELAY_US(600);
        AFE_WRITE(AFE_CONN8, 0x00004000);

        if (first_boot) {
            //Here Set Analog - HPTRIM (Simon)
            ANA_WRITE(AUDDEC_ANA_CON31, 0x00001461);
            ANA_WRITE(AUDDEC_ANA_CON6, 0x00000040);
            ANA_WRITE(AUDDEC_ANA_CON6, 0x00000070);
            //Here Set Analog - AuxADC(Simon)
            ANA_WRITE(0x43090078, 0x00000101);
            ANA_WRITE(0x42090400, 0x00000001);
            HAL_AUDIO_DELAY_US(5);
            ANA_WRITE(0x43090074, 0x00000001);
            HAL_AUDIO_DELAY_US(45);
            ANA_WRITE(0x4309008C, 0x00000001);
            ANA_WRITE(0x43090088, 0x00000001);
            ANA_WRITE(0x43090008, 0x00000100);
            ANA_WRITE(0x43090008, 0x00000000);
            //Execute SW HPtrim Offset Calibration
            ANA_WRITE(AUDDEC_ANA_CON6, 0x00000072);
            ANA_WRITE(0x43090004, 0x00000000);
            ANA_WRITE(0x43090004, 0x00000100);
            ANA_WRITE(AUDDEC_ANA_CON6, 0x00000074);
            ANA_WRITE(0x43090004, 0x00000000);
            ANA_WRITE(0x43090004, 0x00000100);
            ANA_WRITE(AUDDEC_ANA_CON6, 0x00000071);
            ANA_WRITE(0x43090004, 0x00000000);
            ANA_WRITE(0x43090004, 0x00000100);
            ANA_WRITE(0x43090030, 0x00000100);
            ANA_WRITE(AUDDEC_ANA_CON6, 0x00000073);
            ANA_WRITE(0x43090004, 0x00000000);
            ANA_WRITE(0x43090004, 0x00000100);
            HAL_AUDIO_DELAY_US(3);
            //HP-trim Calibarition Finish
            //Here Set Analog - AuxADC(Simon)
            ANA_WRITE(0x43090008, 0x00000100);
            ANA_WRITE(0x43090008, 0x00000000);
            ANA_WRITE(0x4309008C, 0x00000000);
            ANA_WRITE(0x43090088, 0x00000000);
            ANA_WRITE(0x43090074, 0x00000000);
            HAL_AUDIO_DELAY_US(45);
            ANA_WRITE(0x43090078, 0x00000100);
            ANA_WRITE(0x42090400, 0x00000000);
            HAL_AUDIO_DELAY_US(5);
            //Here Set Analog -HPTRIM & AuxADC(Simon)
            ANA_WRITE(0x42090118, 0x00000070);
            ANA_WRITE(0x4209017C, 0x00001460);
            ANA_WRITE(0xA2070118, 0x00000000);
            ANA_WRITE(0x42090118, 0x00000000);
        }
...
bool hal_audio_ana_set_dac_classd_enable(hal_audio_device_parameter_dac_t *dac_parameter, afe_analog_control_t analog_control, bool enable)
{
    ...
    HAL_AUDIO_LOG_INFO("DSP - Hal Audio ANA DAC:0x%x, Mode:%d, Performance:%d, dc_compensation:0x%x, CLASS-D Enable:%d", 5, analog_control, dac_parameter->dac_mode, dac_parameter->performance, dac_parameter->dc_compensation_value, enable);
    ...
    if (enable) {
        ...
        AFE_WRITE(AFE_CLASSG_CFG0, 0x11004004);
        AFE_WRITE(AFE_ADDA_UL_DL_CON0, 0x60606000);
        AFE_WRITE(AFE_DAC_CON0, 0x0);
        AFE_WRITE(AUDIO_TOP_CON0, 0xB0CCC000);
