...
static hal_audio_performance_mode_t afe_adc_performance_mode[AFE_ANALOG_NUMBER];
...
static int nle_sample_count;
#endif
#ifdef AIR_RESET_SDM_ENABLE
static int silence_sample_count;
#endif
...
void hal_audio_component_id_resource_management(audio_scenario_type_t type, hal_audio_agent_t agent, bool control);
...
void hal_audio_device_set_sub_component_id(hal_audio_device_parameter_t *handle, hal_audio_control_t device, hal_audio_agent_t agent, bool control);
void hal_audio_sub_component_id_resource_management(hal_audio_device_agent_t device_agent, hal_audio_sub_agent_t sub_agent, bool control);
...
bool hal_audio_device_set_delay_timer(hal_audio_device_parameter_t *handle, hal_audio_control_t device, hal_audio_agent_t agent, hal_audio_control_status_t control);
...
bool hal_audio_device_change_rate(hal_audio_device_parameter_t *handle, hal_audio_control_t device, hal_audio_agent_t agent, hal_audio_control_status_t control);
...
#endif
#ifdef AIR_RESET_SDM_ENABLE
bool hal_audio_dl_reset_sdm(VOID *DestBuf, U16 CopySize, U32 format, SOURCE_TYPE type);
#endif
...
uint32_t hal_audio_control_get_value(hal_audio_get_value_parameter_t *handle, hal_audio_get_value_command_t command)
{
    ...
    switch (command) {
        ...
        case HAL_AUDIO_GET_DEVICE_SAMPLE_RATE:
            get_value = hal_audio_device_get_rate(hal_device_convert_agent(handle->get_device_rate.device_control, handle->get_device_rate.device_interface, handle->get_device_rate.is_tx));
            break;
#if 0
...
{
    //Except for  ANC, VAD
    return (uint32_t)(hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_INPUT) + hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_OUTPUT));
}
...
bool g_dsp_agent_on[HAL_AUDIO_AGENT_NUMBERS];
bool g_dsp_agent_off[HAL_AUDIO_AGENT_NUMBERS];
bool g_dsp_sub_agent_on[HAL_AUDIO_SUB_AGENT_NUMBERS];
bool g_dsp_sub_agent_off[HAL_AUDIO_SUB_AGENT_NUMBERS];
...
bool hal_audio_status_get_sub_agent_status(hal_audio_sub_agent_t sub_agent)
{
    for (uint32_t i = 0; i <= (HAL_AUDIO_DEVICE_AGENT_DEVICE_NUMBERS); i++) {
        if (g_dsp_sub_agent_type[sub_agent] != 0) {
            //HAL_AUDIO_LOG_INFO("[Audio Agent] sub_agent %d is used", 1, sub_agent);
            return true; // means this sub_agent is enabled by some scenario
        }
    }
...
void hal_audio_component_id_resource_management(audio_scenario_type_t type, hal_audio_agent_t agent, bool control)
{
    ...
    if ((agent >= HAL_AUDIO_AGENT_DEVICE_I2S_MASTER_MIN) && (agent <= HAL_AUDIO_AGENT_DEVICE_I2S_MASTER_MAX)) {
        ...
        for (uint32_t i = 0; i < couple_agent_num; i++) {
            ...
            if (couple_agent[i] >= HAL_AUDIO_AGENT_DEVICE_I2S_SLAVE_MAX) {
                couple_agent[i] = HAL_AUDIO_AGENT_DEVICE_I2S_MASTER_MIN+couple_agent[i]-HAL_AUDIO_AGENT_DEVICE_I2S_SLAVE_MAX-1;
            }
            HAL_AUDIO_LOG_INFO("[Audio Agent] couple_agent[%d] = %d",2,i,couple_agent[i]);
        }
    ...
    if (control) {
        ...
        //if (g_dsp_agent_type[agent][0] || g_dsp_agent_type[agent][1] || g_dsp_agent_type[agent][2] || g_dsp_agent_type[agent][3]) {
            g_dsp_agent_on[agent] = false;
        } else {
            if (couple_agent[0] == HAL_AUDIO_AGENT_ERROR) {
                g_dsp_agent_on[agent] = true;
            } else {
                for (uint32_t i = 0; i < couple_agent_num; i++) {
                    if (hal_audio_status_get_agent_status(couple_agent[i])) {
                        HAL_AUDIO_LOG_INFO("[Audio Agent] couple_agent %d is used",1,couple_agent[i]);
                        g_dsp_agent_on[agent] = false;
                        break;
                    }
                    g_dsp_agent_on[agent] = true;
        ...
        if (hal_audio_status_get_agent_status(agent)) {
        //if (g_dsp_agent_type[agent][0] || g_dsp_agent_type[agent][1] || g_dsp_agent_type[agent][2] || g_dsp_agent_type[agent][3]) {
            g_dsp_agent_off[agent] = false;
        } else {
            if (couple_agent[0] == HAL_AUDIO_AGENT_ERROR) {
                g_dsp_agent_off[agent] = true;
            } else {
                for (uint32_t i = 0; i < couple_agent_num; i++) {
                    if (hal_audio_status_get_agent_status(couple_agent[i])) {
                        HAL_AUDIO_LOG_INFO("[Audio Agent] couple_agent %d is used",1,couple_agent[i]);
    ...
    HAL_AUDIO_LOG_INFO("[Audio Agent] agent %d, type %d, agent on %d, agent off %d", 4, agent, type, g_dsp_agent_on[agent], g_dsp_agent_off[agent]);
...
void hal_audio_sub_component_id_resource_management(hal_audio_device_agent_t device_agent, hal_audio_sub_agent_t sub_agent, bool control)
{
    ...
    //uint32_t index = device_agent / 32;
    //uint32_t bit_mask = device_agent % 32;

    if (control) {
        if (g_dsp_sub_agent_type[sub_agent] & (1 << device_agent)) {
            log_hal_msgid_error("[Audio Agent] ERROR: sub_agent %d agent %d is already enable", 2,
                                sub_agent,
                                device_agent);
            log_hal_msgid_error("[Audio Agent] sub_agent %d: [0x%x]", 2,
                                sub_agent,
        ...
        if (hal_audio_status_get_sub_agent_status(sub_agent)) {
            g_dsp_sub_agent_on[sub_agent] = false;
            g_dsp_sub_agent_off[sub_agent] = false;
        } else {
            g_dsp_sub_agent_on[sub_agent] = true;
            g_dsp_sub_agent_off[sub_agent] = false;
        }
        g_dsp_sub_agent_type[sub_agent] |= (1 << device_agent);
    } else {
        if ((g_dsp_sub_agent_type[sub_agent] & (1 << device_agent)) == 0) {
            log_hal_msgid_error("[Audio Agent] ERROR: sub_agent %d, agent %d is already disable", 2,
                                sub_agent,
                                device_agent);
            log_hal_msgid_error("[Audio Agent] sub_agent %d: [0x%x]", 2,
                                sub_agent,
        ...
        if (hal_audio_status_get_sub_agent_status(sub_agent)) {
            ...
            g_dsp_sub_agent_on[sub_agent] = false;
            g_dsp_sub_agent_off[sub_agent] = true;
        }
    }
    HAL_AUDIO_LOG_INFO("[Audio Agent] sub_agent %d, device_agent %d, sub_agent on %d, sub_agent off %d", 4, sub_agent, device_agent, g_dsp_sub_agent_on[sub_agent], g_dsp_sub_agent_on[sub_agent]);
...
void hal_audio_device_set_sub_component_id(hal_audio_device_parameter_t *handle, hal_audio_control_t device, hal_audio_agent_t device_agent, bool control)
{
    hal_audio_sub_agent_t sub_agent;
    //HAL_AUDIO_LOG_INFO("[Audio Agent] #hal_audio_device_set_sub_component_id# agent %d, control %d", 2, device_agent, control);
    ...
    switch (device) {
        ...
        case HAL_AUDIO_CONTROL_DEVICE_LOOPBACK:
            if (device_agent == HAL_AUDIO_DEVICE_AGENT_DEVICE_ADDA_UL2) {
                sub_agent = HAL_AUDIO_AFE_CLOCK_ADC23;
                hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
                sub_agent = HAL_AUDIO_AFE_CLOCK_ADC01;
                hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            } else if (device_agent == HAL_AUDIO_DEVICE_AGENT_DEVICE_ADDA_UL3) {
                sub_agent = HAL_AUDIO_AFE_CLOCK_ADC45;
                hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
                sub_agent = HAL_AUDIO_AFE_CLOCK_ADC01;
                hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            } else if (device_agent == HAL_AUDIO_DEVICE_AGENT_DEVICE_ADDA_UL4) {
                sub_agent = HAL_AUDIO_AFE_CLOCK_ANC;
            } else {
                sub_agent = HAL_AUDIO_AFE_CLOCK_ADC01;
                hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            }
            if (afe_samplerate_get_ul_samplerate(device_agent) > 48000) {
                sub_agent = HAL_AUDIO_AFE_CLOCK_ADC_HIRES;
                hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            }
            sub_agent = HAL_AUDIO_AFE_CONTROL_ADDA;
            hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            break;
        ...
        case HAL_AUDIO_CONTROL_DEVICE_INTERNAL_DAC_DUAL:
            sub_agent = HAL_AUDIO_AFE_CLOCK_DAC;
            hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            //if (afe_samplerate_get_dl_samplerate() > 48000) {
            if (handle->dac.rate > 48000) {
                sub_agent = HAL_AUDIO_AFE_CLOCK_DAC_HIRES;
                hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            }
            sub_agent = HAL_AUDIO_AFE_CONTROL_ADDA;
            hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
        ...
        case HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER_R:
            sub_agent = HAL_AUDIO_AFE_CLOCK_I2S0;
            hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            sub_agent = HAL_AUDIO_AFE_CLOCK_I2S1;
            hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            sub_agent = HAL_AUDIO_AFE_CLOCK_I2S2;
            hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            sub_agent = HAL_AUDIO_AFE_CLOCK_I2S3;
            hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);

            if (handle->i2s_master.is_low_jitter || handle->i2s_master.with_mclk) {
                if (hal_audio_i2s_get_apll_by_samplerate(handle->common.rate)==AFE_I2S_APLL1) {
                    sub_agent = HAL_AUDIO_AFE_CLOCK_APLL;
                    hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
                    if (handle->i2s_master.is_low_jitter) {
                        sub_agent = HAL_AUDIO_AFE_CLOCK_22M;
                        hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
                    }
                } else {
                    sub_agent = HAL_AUDIO_AFE_CLOCK_APLL2;
                    hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
                    if (handle->i2s_master.is_low_jitter) {
                        sub_agent = HAL_AUDIO_AFE_CLOCK_24M;
                        hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
                    }
                }
            }
        ...
        case HAL_AUDIO_CONTROL_DEVICE_I2S_SLAVE:
            sub_agent = HAL_AUDIO_AFE_CLOCK_I2S_SLV_HCLK;
            hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
            break;
        ...
        case HAL_AUDIO_CONTROL_DEVICE_SIDETONE:
            if (hal_audio_device_get_rate(hal_device_convert_agent(handle->sidetone.input_device, handle->sidetone.input_interface, handle->sidetone.is_tx)) !=  handle->sidetone.rate) {
                sub_agent = HAL_AUDIO_AFE_CLOCK_I2S0;
                hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
                sub_agent = HAL_AUDIO_AFE_CLOCK_I2S1;
                hal_audio_sub_component_id_resource_management(device_agent, sub_agent, control);
...
bool hal_audio_device_set_agent(hal_audio_device_parameter_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    ...
    #endif
    HAL_AUDIO_LOG_INFO("#hal_audio_device_set_agent# device 0x%x, agent %d, type %d, on/off %d", 4, device, agent, type, control);
    ...
    if (control == HAL_AUDIO_CONTROL_ON) {
        /* Add mutex to protect multi task access the global variable: g_dsp_agent_type */
        hal_audio_device_set_mutex_lock(g_audio_device_mutex);
        if (!hal_audio_device_set_delay_timer(handle, device, agent, control)) {
            ...
            *device_status = HAL_AUDIO_DEVICE_STATUS_OPEN;
            ...
            #ifdef MTK_ANC_ENABLE
            if ((device & HAL_AUDIO_CONTROL_DEVICE_ANC) && (hal_audio_anc_get_using_count(handle) < 1)) {
                g_dsp_agent_on[agent] = false;
            } else {
        ...
        if (!g_dsp_agent_off[agent]) {
            ...
            if ((*device_status == HAL_AUDIO_DEVICE_STATUS_PENDING) ||
                (!hal_audio_device_set_delay_timer(handle, device, agent, control))) {
                //No delay timer
    ...
    if (dsp_vow && (device != HAL_AUDIO_CONTROL_DEVICE_VOW)) { //force vow to switch phase 0 to phase 1
        ...
        if (handle->analog_mic.mic_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2) {

            analog_select = AFE_ANALOG_ADC1;
        } else if (handle->analog_mic.mic_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_3) {
        ...
        if (control == HAL_AUDIO_CONTROL_OFF) {
            ...
            if (amp_control.delay_handle.agent != HAL_AUDIO_AGENT_ERROR) {
                ...
                if (device != amp_control.delay_handle.device_parameter.common.audio_device) {
                    ...
                    hal_audio_device_change_rate(handle, device, agent, control);
#endif
                    with_timer = true;
                }
                hal_audio_device_set_output_gpio(control, false);
...
{
    UNUSED(handle);
    HAL_AUDIO_LOG_INFO("#hal_audio_device_distinguish_channel#", 0);
    if (device & HAL_AUDIO_CONTROL_DEVICE_ANALOG_MIC_DUAL) {
#if (HAL_AUDIO_KEEP_ADC_HIGHER_PERFORMANCE_MODE)
        ...
        if (control == HAL_AUDIO_CONTROL_ON) {
            if ((hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_ANC)) && (hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_ADDA_DL1)) &&
        ...
        if (is_force_change_rate) {
            ...
            hal_audio_dl_set_hires(HAL_AUDIO_AGENT_DEVICE_ADDA_DL1, false);
            //Change rate
            afe_samplerate_set_dl_samplerate(dl_require_rate);
            hal_audio_dl_set_hires(HAL_AUDIO_AGENT_DEVICE_ADDA_DL1, true);
...
{
    UNUSED(control);
    uint32_t device_rate, changing_rate;
    device_rate = hal_audio_device_get_rate(hal_device_convert_agent(device, handle->common.device_interface, handle->common.is_tx));
    if (device_rate != handle->common.rate) {
        changing_rate = handle->common.rate;
        if (device & HAL_AUDIO_CONTROL_DEVICE_INTERNAL_DAC_DUAL) {
            handle->dac.rate = device_rate;
            hal_audio_device_set_sub_component_id(handle, device, agent, HAL_AUDIO_CONTROL_OFF);
            ...
            handle->dac.rate = changing_rate;
            hal_audio_device_set_sub_component_id(handle, device, agent, HAL_AUDIO_CONTROL_ON);
            ...
            handle->common.rate = changing_rate;
            hal_audio_device_set_sub_component_id(handle, device, agent, HAL_AUDIO_CONTROL_ON);
...
bool hal_audio_device_set_linein(hal_audio_device_parameter_linein_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    hal_audio_agent_t device_agent = HAL_AUDIO_AGENT_DEVICE_ADDA_UL2_DUAL;
    if (control == HAL_AUDIO_CONTROL_ON) {
...
bool hal_audio_device_set_ul_loopback(hal_audio_device_parameter_loopback_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    ...
    hal_audio_agent_t device_agent = hal_device_convert_agent(device, handle->ul_interface, handle->is_tx);
    if (control == HAL_AUDIO_CONTROL_ON) {
...
bool hal_audio_device_set_analog_mic(hal_audio_device_parameter_analog_mic_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    hal_audio_agent_t device_agent = hal_device_convert_agent(device, handle->mic_interface, handle->is_tx);
    if (control == HAL_AUDIO_CONTROL_ON) {
...
bool hal_audio_device_set_vow(hal_audio_device_parameter_vow_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    ...
    if ((handle->mic_selection | handle->mic1_selection)&HAL_AUDIO_CONTROL_DEVICE_DIGITAL_MIC_DUAL) {
        ...
        if (handle->mic_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2) {
            analog_select = AFE_ANALOG_ADC1;
        } else if (handle->mic_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_3) {
            analog_select = AFE_ANALOG_ADC2;
        }
        if (handle->mic1_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2) {
            analog_select1 = AFE_ANALOG_ADC1;
        } else if (handle->mic1_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_3) {
...
bool hal_audio_device_set_sidetone(hal_audio_device_parameter_sidetone_t *handle, hal_audio_control_t device, hal_audio_control_status_t control)
{
    ...
    if (control == HAL_AUDIO_CONTROL_ON) {
        ...
        if (hal_audio_device_get_rate(hal_device_convert_agent(handle->input_device, handle->input_interface, handle->is_tx)) !=  handle->rate) {
            HAL_AUDIO_LOG_INFO("DSP - sidetone tick_align_enable %d, %d", 2, hal_audio_device_get_rate(hal_device_convert_agent(handle->input_device, handle->input_interface, handle->is_tx)), handle->rate);
            sidetone_control.tick_align_enable = true;
#ifdef AIR_SIDETONE_USE_I2S_TICK_SOURCE
            i2s_id = AIR_SIDETONE_USE_I2S_TICK_SOURCE;
            i2s_agent = HAL_AUDIO_AGENT_DEVICE_I2S0_MASTER_DUAL_TX + (i2s_id - AFE_I2S0);
#else
            if ((hal_device_convert_agent(handle->output_device, handle->output_interface, handle->is_tx)) == HAL_AUDIO_AGENT_DEVICE_I2S0_MASTER_DUAL_RX) {
                i2s_id = AFE_I2S0;
                i2s_agent = HAL_AUDIO_AGENT_DEVICE_I2S0_MASTER_DUAL_RX;
            } else if ((hal_device_convert_agent(handle->output_device, handle->output_interface, handle->is_tx)) == HAL_AUDIO_AGENT_DEVICE_I2S1_MASTER_DUAL_RX) {
                i2s_id = AFE_I2S1;
                i2s_agent = HAL_AUDIO_AGENT_DEVICE_I2S1_MASTER_DUAL_RX;
            } else if ((hal_device_convert_agent(handle->output_device, handle->output_interface, handle->is_tx)) == HAL_AUDIO_AGENT_DEVICE_I2S2_MASTER_DUAL_RX) {
                i2s_id = AFE_I2S2;
                i2s_agent = HAL_AUDIO_AGENT_DEVICE_I2S2_MASTER_DUAL_RX;
            } else if ((hal_device_convert_agent(handle->output_device, handle->output_interface, handle->is_tx)) == HAL_AUDIO_AGENT_DEVICE_I2S3_MASTER_DUAL_RX) {
                i2s_id = AFE_I2S3;
                i2s_agent = HAL_AUDIO_AGENT_DEVICE_I2S3_MASTER_DUAL_RX;
            } else {
                i2s_id = AFE_I2S0;
            ...
            hal_audio_i2s_set_clk(i2s_id, true);
            ...
            hal_sidetone_set_input(hal_device_convert_agent(handle->input_device, handle->input_interface, handle->is_tx), true);
        }

        //hal_sidetone_set_input(hal_device_convert_agent(handle->input_device, handle->input_interface), true);
        hal_sidetone_set_output(hal_device_convert_agent(handle->output_device, handle->output_interface, handle->is_tx), true);
        ...
        if (!sidetone_control.ramp_down_done && sidetone_control.with_ramp_control) {
            ...
            if (sidetone_control.tick_align_enable) {
                ...
                hal_audio_i2s_set_clk(i2s_id, false);
            }
...
*                                       Memory agent                                       *
********************************************************************************************/
#if AIR_AUDIO_SRAM_COMPONENT_TYPE_ENABLE

#if 0
// audio sram size
#define AUDIO_SRAM_SIZE_A2DP                  40960
#define AUDIO_SRAM_SIZE_HFP_UL                32768
#define AUDIO_SRAM_SIZE_HFP_DL                32768
#define AUDIO_SRAM_SIZE_LE_CALL_UL            16384
#define AUDIO_SRAM_SIZE_LE_CALL_DL            16384
#define AUDIO_SRAM_SIZE_LE_MUSIC_DL           20480
#define AUDIO_SRAM_SIZE_ESA                   49152
#define AUDIO_SRAM_SIZE_MULTI_MIC             8192
#define AUDIO_SRAM_SIZE_VP                    16384
#define AUDIO_SRAM_SIZE_TDM                   16384
#define AUDIO_SRAM_SIZE_WWE                   4096

// component_sram_block_num
#define AUDIO_SRAM_NO_USE                     0
#define AUDIO_SRAM_BLOCK_NUM_A2DP             (AUDIO_SRAM_SIZE_A2DP%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_A2DP/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_A2DP/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_HFP_UL           (AUDIO_SRAM_SIZE_HFP_UL%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_HFP_UL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_HFP_UL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_HFP_DL           (AUDIO_SRAM_SIZE_HFP_DL%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_HFP_DL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_HFP_DL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_LE_CALL_UL       (AUDIO_SRAM_SIZE_LE_CALL_UL%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_LE_CALL_UL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_LE_CALL_UL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_LE_CALL_DL       (AUDIO_SRAM_SIZE_LE_CALL_DL%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_LE_CALL_DL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_LE_CALL_DL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_LE_MUSIC_DL      (AUDIO_SRAM_SIZE_LE_MUSIC_DL%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_LE_MUSIC_DL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_LE_MUSIC_DL/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_ESA              (AUDIO_SRAM_SIZE_ESA%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_ESA/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_ESA/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_MULTI_MIC        (AUDIO_SRAM_SIZE_MULTI_MIC%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_MULTI_MIC/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_MULTI_MIC/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_VP               (AUDIO_SRAM_SIZE_VP%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_VP/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_VP/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_TDM              (AUDIO_SRAM_SIZE_TDM%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_TDM/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_TDM/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE
#define AUDIO_SRAM_BLOCK_NUM_WWE              (AUDIO_SRAM_SIZE_WWE%HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE!=0) ? AUDIO_SRAM_SIZE_WWE/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE+1 : AUDIO_SRAM_SIZE_WWE/HAL_AUDIO_MEMORY_SRAM_BLOCK_SIZE

// component_sram_search_block
#define AUDIO_SRAM_SEARCH_BLOCK_A2DP          1
#define AUDIO_SRAM_SEARCH_BLOCK_HFP_UL        1
#define AUDIO_SRAM_SEARCH_BLOCK_HFP_DL        1
#define AUDIO_SRAM_SEARCH_BLOCK_LE_CALL_UL    1
#define AUDIO_SRAM_SEARCH_BLOCK_LE_CALL_DL    1
#define AUDIO_SRAM_SEARCH_BLOCK_LE_MUSIC_DL   1
#define AUDIO_SRAM_SEARCH_BLOCK_ESA           1
#define AUDIO_SRAM_SEARCH_BLOCK_MULTI_MIC     MAX(MAX(MAX(AUDIO_SRAM_BLOCK_NUM_A2DP, AUDIO_SRAM_BLOCK_NUM_HFP_UL),AUDIO_SRAM_BLOCK_NUM_LE_CALL_UL),AUDIO_SRAM_BLOCK_NUM_LE_MUSIC_DL)+1
#define AUDIO_SRAM_SEARCH_BLOCK_VP            AUDIO_SRAM_SEARCH_BLOCK_MULTI_MIC + AUDIO_SRAM_BLOCK_NUM_MULTI_MIC
#define AUDIO_SRAM_SEARCH_BLOCK_WWE           AUDIO_SRAM_SEARCH_BLOCK_VP + HAL_AUDIO_COMPONENT_VP
#define AUDIO_SRAM_SEARCH_BLOCK_TDM           AUDIO_SRAM_SEARCH_BLOCK_WWE + HAL_AUDIO_COMPONENT_WWE

// component_sram_search_direction
#define AUDIO_SRAM_SEARCH_DIRECTION_POS       1
#define AUDIO_SRAM_SEARCH_DIRECTION_NEG       -1
#else
// component_sram_block_num
#define AUDIO_SRAM_NO_USE                     0
#define AUDIO_SRAM_BLOCK_NUM_A2DP             20
#define AUDIO_SRAM_BLOCK_NUM_HFP_UL           16
#define AUDIO_SRAM_BLOCK_NUM_HFP_DL           16
#define AUDIO_SRAM_BLOCK_NUM_LE_CALL_UL       8
#define AUDIO_SRAM_BLOCK_NUM_LE_CALL_DL       8
#define AUDIO_SRAM_BLOCK_NUM_LE_MUSIC_DL      10
#define AUDIO_SRAM_BLOCK_NUM_ESA              24
#define AUDIO_SRAM_BLOCK_NUM_MULTI_MIC        4
#define AUDIO_SRAM_BLOCK_NUM_VP               8
#define AUDIO_SRAM_BLOCK_NUM_TDM              8
#define AUDIO_SRAM_BLOCK_NUM_WWE              2
...
#define AUDIO_SRAM_SEARCH_BLOCK_LE_MUSIC_DL   1
#define AUDIO_SRAM_SEARCH_BLOCK_ESA           1
#define AUDIO_SRAM_SEARCH_BLOCK_MULTI_MIC     AUDIO_SRAM_SEARCH_BLOCK_A2DP + AUDIO_SRAM_BLOCK_NUM_A2DP
#define AUDIO_SRAM_SEARCH_BLOCK_VP            AUDIO_SRAM_SEARCH_BLOCK_MULTI_MIC + AUDIO_SRAM_BLOCK_NUM_MULTI_MIC
#define AUDIO_SRAM_SEARCH_BLOCK_WWE           AUDIO_SRAM_SEARCH_BLOCK_VP + AUDIO_SRAM_BLOCK_NUM_VP
#define AUDIO_SRAM_SEARCH_BLOCK_TDM           36
...
audio_sram_table_t audio_sram_table[AUDIO_SCENARIO_TYPE_END] = {
             //component_type,                       //component_sram_block_num,           //component_sram_search_block,        //component_sram_search_direction,
/*  0  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_COMMON */
/*  1  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_AMP*/
/*  2  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_VP_PRE */
/*  3  */   {HAL_AUDIO_COMPONENT_VP,                 AUDIO_SRAM_BLOCK_NUM_VP,              AUDIO_SRAM_SEARCH_BLOCK_VP,           AUDIO_SRAM_SEARCH_DIRECTION_POS},    /* AUDIO_SCENARIO_TYPE_VP */
/*  4  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_VP_DUMMY_PRE */
/*  5  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_VP_DUMMY */
/*  6  */   {HAL_AUDIO_COMPONENT_HFP_UL,             AUDIO_SRAM_BLOCK_NUM_HFP_UL,          AUDIO_SRAM_SEARCH_BLOCK_HFP_UL,       AUDIO_SRAM_SEARCH_DIRECTION_NEG},    /* AUDIO_SCENARIO_TYPE_HFP_UL */
/*  7  */   {HAL_AUDIO_COMPONENT_HFP_DL,             AUDIO_SRAM_BLOCK_NUM_HFP_DL,          AUDIO_SRAM_SEARCH_BLOCK_HFP_DL,       AUDIO_SRAM_SEARCH_DIRECTION_POS},    /* AUDIO_SCENARIO_TYPE_HFP_DL */
/*  8  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_RECORD */
/*  9  */   {HAL_AUDIO_COMPONENT_WWE,                AUDIO_SRAM_BLOCK_NUM_WWE,             AUDIO_SRAM_SEARCH_BLOCK_WWE,          AUDIO_SRAM_SEARCH_DIRECTION_POS},    /* AUDIO_SCENARIO_TYPE_WWE */
/* 10  */   {HAL_AUDIO_COMPONENT_A2DP,               AUDIO_SRAM_BLOCK_NUM_A2DP,            AUDIO_SRAM_SEARCH_BLOCK_A2DP,         AUDIO_SRAM_SEARCH_DIRECTION_POS},    /* AUDIO_SCENARIO_TYPE_A2DP */
/* 11  */   {HAL_AUDIO_COMPONENT_LE_CALL_UL,         AUDIO_SRAM_BLOCK_NUM_LE_CALL_UL,      AUDIO_SRAM_SEARCH_BLOCK_LE_CALL_UL,   AUDIO_SRAM_SEARCH_DIRECTION_NEG},    /* AUDIO_SCENARIO_TYPE_BLE_UL */
/* 12  */   {HAL_AUDIO_COMPONENT_LE_CALL_DL,         AUDIO_SRAM_BLOCK_NUM_LE_CALL_DL,      AUDIO_SRAM_SEARCH_BLOCK_LE_CALL_DL,   AUDIO_SRAM_SEARCH_DIRECTION_POS},    /* AUDIO_SCENARIO_TYPE_BLE_DL  */
/* 13  */   {HAL_AUDIO_COMPONENT_LE_MUSIC_DL,        AUDIO_SRAM_BLOCK_NUM_LE_MUSIC_DL,     AUDIO_SRAM_SEARCH_BLOCK_LE_MUSIC_DL,  AUDIO_SRAM_SEARCH_DIRECTION_POS},    /* AUDIO_SCENARIO_TYPE_BLE_MUSIC_DL  */
/* 14  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_ANC */
/* 15  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_SIDETONE */
/* 16  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_VOW */
/* 17  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_LINE_IN */
/* 18  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_LINE_OUT */
/* 19  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_SPDIF */
/* 20  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_MCLK */
/* 21  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_USB_AUDIO_PLAYBACK */
/* 22  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_PLAYBACK */
/* 23  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_DC_COMPENSATION */
/* 24  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_GSENSOR_FUNCTION_D */
/* 25  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_GSENSOR_FUNCTION_F */
/* 26  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_GSENSOR_FUNCTION_G */
/* 27  */   {HAL_AUDIO_COMPONENT_MULTI_MIC,          AUDIO_SRAM_BLOCK_NUM_MULTI_MIC,       AUDIO_SRAM_SEARCH_BLOCK_MULTI_MIC,    AUDIO_SRAM_SEARCH_DIRECTION_POS},    /* AUDIO_SCENARIO_TYPE_MULTI_MIC_STREAM_FUNCTION_A */
/* 28  */   {HAL_AUDIO_COMPONENT_ESA,                AUDIO_SRAM_BLOCK_NUM_ESA,             AUDIO_SRAM_SEARCH_BLOCK_ESA,          AUDIO_SRAM_SEARCH_DIRECTION_POS},    /* AUDIO_SCENARIO_TYPE_MULTI_MIC_STREAM_FUNCTION_B */
/* 29  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_MULTI_MIC_STREAM_FUNCTION_C */
/* 30  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_MULTI_MIC_STREAM_FUNCTION_F */
/* 31  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_GAMING_MODE_VOICE_HEADSET */
/* 32  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_GAMING_MODE_VOICE_DONGLE_USB_OUT */
/* 33  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_GAMING_MODE_MUSIC_DONGLE_USB_IN_0 */
/* 34  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_GAMING_MODE_MUSIC_DONGLE_USB_IN_1 */
/* 35  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_GAMING_MODE_VOICE_DONGLE_LINE_OUT */
/* 36  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_GAMING_MODE_MUSIC_DONGLE_LINE_IN */
/* 37  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_ANC_MONITOR_STREAM */
/* 38  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_TEST_AUDIO_LOOPBACK */
/* 39  */   {HAL_AUDIO_COMPONENT_TDM,                AUDIO_SRAM_BLOCK_NUM_TDM,             AUDIO_SRAM_SEARCH_BLOCK_TDM,          AUDIO_SRAM_SEARCH_DIRECTION_POS},    /* AUDIO_SCENARIO_TYPE_TDM */
/* 40  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_OUT */
/* 41  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_0 */
/* 42  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_WIRED_AUDIO_USB_IN_1 */
/* 43  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_WIRED_AUDIO_LINE_OUT */
/* 44  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_WIRED_AUDIO_LINE_IN */
/* 45  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_WIRED_AUDIO_DUAL_CHIP_LINE_IN_MASTER */
/* 46  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_WIRED_AUDIO_DUAL_CHIP_LINE_IN_SLAVE */
/* 47  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_WIRED_AUDIO_DUAL_CHIP_LINE_OUT_MASTER */
/* 48  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_ADVANCED_PASSTHROUGH_HEARING_AID */
/* 49  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_BLE_AUDIO_DONGLE_VOICE_USB_OUT */
/* 50  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_BLE_AUDIO_DONGLE_MUSIC_USB_IN_0 */
/* 51  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_BLE_AUDIO_DONGLE_MUSIC_USB_IN_1 */
/* 52  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_BLE_AUDIO_DONGLE_VOICE_LINE_OUT */
/* 53  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_BLE_AUDIO_DONGLE_MUSIC_LINE_IN */
/* 54  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_AUDIO_HW_LOOPBACK_I2S0_TO_DAC */
/* 55  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_AUDIO_HW_LOOPBACK_ADC_TO_I2S0 */
/* 56  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /* AUDIO_SCENARIO_TYPE_AUDIO_HW_LOOPBACK_LINEIN_TO_I2S2 */
/* 57  */   {HAL_AUDIO_COMPONENT_NO_USE,             AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE,                    AUDIO_SRAM_NO_USE},                  /*AUDIO_SCENARIO_TYPE_ADAPTIVE_EQ_MONITOR_STREAM*/
};
#endif
    ...
    if (control == HAL_AUDIO_CONTROL_ON) {
        ...
        if (agent == HAL_AUDIO_AGENT_MEMORY_AWB2) {
            ...
            //Connect from UL loopback
            {
                ...
                loopback.ul_interface = HAL_AUDIO_CONTROL_DEVICE_INTERFACE_4;
...
#if defined AIR_AUDIO_SRAM_COMPONENT_TYPE_ENABLE
uint32_t hal_memory_allocate_sram(audio_scenario_type_t type, hal_audio_agent_t user_type, uint32_t size)
{
    ...
    hal_audio_component_type_t component_type = HAL_AUDIO_COMPONENT_NO_USE;
    int32_t component_block_num = 0, component_search_block = 0, component_search_direction = 0, component_search_block_end = 0;
    //printf("[Audio SRAM][Allocate] scenario_type:%d,user_type:%d,size:%d",type,user_type,size);
    uint32_t i;
    for (i = 0 ; i < AUDIO_SCENARIO_TYPE_END ; i++) {
        if (i == type) {
    ...
    if (component_type == HAL_AUDIO_COMPONENT_NO_USE) {
        HAL_AUDIO_LOG_ERROR("[Audio SRAM][Allocate] DSP - Error scenario type:%d is not in audio_sram_table", 1, type);
        assert(FALSE);
    }
    component_search_block_end = component_search_block + component_block_num + 1;
    ...
    //printf("[Audio SRAM][Allocate] component_type:%d,component_block_num:%d,component_search_block:%d,component_search_block_end:%d,component_search_direction:%d",component_type,component_block_num,component_search_block,component_search_block_end,component_search_direction);

    hal_sram_manager.remain_block = component_block_num;

    HAL_AUDIO_SEMAPHORE_TAKE(hal_sram_manager.semaphore_ptr);
    top_index = bottom_index = 0;
    for (; (search_block >= component_search_block) && (search_block < component_search_block_end) ; search_block += component_search_direction) {
        //printf("[Audio SRAM][Allocate] search_block:%d, user_type:%d",search_block,hal_sram_manager.block[search_block].user);
        if (hal_sram_manager.block[search_block].user == HAL_AUDIO_AGENT_ERROR) {
            ...
            //printf("allow_size:%d",allow_size);
        } else {
            allow_size = 0;
        }
...
uint32_t hal_memory_free_sram(audio_scenario_type_t type, hal_audio_agent_t user_type)
{
    ...
    hal_audio_component_type_t component_type = HAL_AUDIO_COMPONENT_NO_USE;
    int32_t component_block_num = 0, component_search_block = 0, component_search_block_end = 0;
    uint32_t i;
    ...
    component_search_block_end = component_search_block + component_block_num;

    HAL_AUDIO_SEMAPHORE_TAKE(hal_sram_manager.semaphore_ptr);

    for (search_block = component_search_block ; search_block < component_search_block_end ; search_block++) {
...
bool hal_audio_hardware_gain_set_agent(afe_hardware_digital_gain_t gain_select, uint32_t samplerate, audio_scenario_type_t type, hal_audio_control_status_t control)
{
    ...
    if (control == HAL_AUDIO_CONTROL_ON) {
        hal_audio_component_id_resource_management(type, hw_gain_agent, control);
        if (g_dsp_agent_on[hw_gain_agent]) {
#ifdef AIR_HWGAIN_SET_FADE_TIME_ENABLE
            ...
            if (gain_select == AFE_HW_DIGITAL_GAIN3){
        ...
        hal_audio_component_id_resource_management(type, hw_gain_agent, control);
        if (g_dsp_agent_off[hw_gain_agent]) {
            afe_volume_digital_set_mute(gain_select, AFE_VOLUME_MUTE_BLOCK_DISABLE, true);
            if (gain_select == HAL_AUDIO_AGENT_BLOCK_HWGAIN3) {
                hal_hw_gain_set_enable(gain_select, samplerate, false);
...
bool hal_audio_i2s_set_clk(afe_i2s_id_t i2s_id, bool enable)
{
    ...
    hal_audio_clock_enable_i2s1(enable);
    hal_audio_clock_enable_i2s2(enable);
    hal_audio_clock_enable_i2s3(enable);
#endif
    return false;
}
...
bool hal_audio_updown_set_agent(afe_updown_configuration_t *configure, afe_updown_sampler_id_t updown_id, audio_scenario_type_t type, hal_audio_control_status_t control)
{
    ...
    uint32_t input_rate = configure->input_rate;
    uint32_t output_rate = configure->output_rate;
    uint32_t non_integer_multiple_rate = configure->non_integer_multiple_rate;
    if (updown_id == AFE_UPDOWN_SAMPLER_UP_CH01) {
        ...
        if (configure->is_echo_configure_input) {
            updown_agent = HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE23_CONFIGURE;
        } else {
            updown_agent = HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE23;
        }
    ...
    HAL_AUDIO_LOG_INFO("[UPDOWN_SAMPLER] updown_id:%d,updown_agent:%d,input_rate:%d, output_rate:%d,non_integer_multiple_rate:%d,is_non_integer_multiple:%d,is_echo_configure_input:%d,control:%d\r\n", 8, updown_id, updown_agent, input_rate, output_rate, non_integer_multiple_rate, configure->is_non_integer_multiple, configure->is_echo_configure_input,control);

    if (control == HAL_AUDIO_CONTROL_ON) {
        hal_audio_component_id_resource_management(type, updown_agent, control);
        if (g_dsp_agent_on[updown_agent]) {
            if (configure->is_non_integer_multiple) {
                if (configure->input_rate >= configure->output_rate) {
                    if ((updown_agent == HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE01) || (updown_agent == HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE23)) {
                        input_rate = configure->input_rate;
                        output_rate = configure->non_integer_multiple_rate;
                    } else {
                        input_rate = configure->non_integer_multiple_rate;
                        output_rate = configure->output_rate;
                    }
                } else {
                    if ((updown_agent == HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE01) || (updown_agent == HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE23)) {
                        input_rate = configure->non_integer_multiple_rate;
                ...
                HAL_AUDIO_LOG_INFO("updown_id:%d ,input_rate:%d, output_rate:%d\r\n", 3, updown_id, input_rate, output_rate);
                hal_audio_updown_set_configuration(updown_id, input_rate, output_rate);
                hal_tick_align_set_updown(updown_id, configure->tick_align, true);
                hal_audio_updown_set_start(updown_id, control);
            } else {
                if (updown_agent == HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE23_CONFIGURE) {
                    hal_updown_set_input_rate(updown_id, configure->input_rate);
                    hal_tick_align_set_updown(updown_id, configure->tick_align, true);
                } else if (updown_agent == HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE23) {
                    hal_updown_set_output_rate(updown_id, non_integer_multiple_rate);
                    hal_updown_set_ratio(updown_id, hal_updown_get_input_rate(updown_id), non_integer_multiple_rate);
                    hal_audio_updown_set_start(updown_id, control);
                    ...
                    hal_updown_set_input_rate(updown_id, non_integer_multiple_rate);
                    hal_tick_align_set_updown(updown_id, configure->tick_align, true);
                    hal_updown_set_output_rate(updown_id, output_rate);
                    hal_updown_set_ratio(updown_id, hal_updown_get_input_rate(updown_id), output_rate);
                    hal_audio_updown_set_start(updown_id, control);
                } else {
        ...
        } else if ((updown_agent != HAL_AUDIO_AGENT_BLOCK_DOWN_SAMPLE23_CONFIGURE) && (updown_agent != HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE23_CONFIGURE)) {
        ...
        hal_audio_component_id_resource_management(type, updown_agent, control);
        if (g_dsp_agent_off[updown_agent]) {
            if (updown_agent == HAL_AUDIO_AGENT_BLOCK_DOWN_SAMPLE23_CONFIGURE) {
                ...
                hal_audio_updown_set_start(updown_id, control);
            } else if (updown_agent == HAL_AUDIO_AGENT_BLOCK_UP_SAMPLE23_CONFIGURE) {
                hal_tick_align_set_updown(updown_id, configure->tick_align, false);
            } else {
                hal_tick_align_set_updown(updown_id, configure->tick_align, false);
                hal_audio_updown_set_start(updown_id, control);
...
int32_t hal_audio_updown_get_agent_count(afe_updown_sampler_id_t updown_id)
{
    ...
    int32_t i = hal_audio_status_get_agent_status(updown_agent);
    return i;
...
{
    uint32_t i, gcd, lcm, non_integer_multiple_rate;
    if ((input_rate == 0) || (output_rate == 0) || (input_rate % 16000 != 0) ||(output_rate % 16000 != 0)) {
        return input_rate;
    }
    ...
    lcm = (input_rate * output_rate) / gcd;
    if (input_rate >= output_rate) {
        if (input_rate % output_rate == 0) {
            non_integer_multiple_rate = input_rate;
        } else {
            non_integer_multiple_rate = lcm;
        }
    } else {
        if (output_rate % input_rate == 0) {
            non_integer_multiple_rate = output_rate;
        } else {
            non_integer_multiple_rate = gcd;
        }
    }
    HAL_AUDIO_LOG_INFO("upwdown get non integer multiple rate, input_rate:%d, output_rate:%d, non_integer_multiple_rate:%d\r\n", 3, input_rate, output_rate, non_integer_multiple_rate);
    return non_integer_multiple_rate;
}
...
void hal_audio_adda_set_enable(bool enable)
{
    hal_audio_sub_agent_t sub_agent = HAL_AUDIO_AFE_CONTROL_ADDA;
    HAL_AUDIO_ENTER_CRITICAL();
    if (enable) {
        if (g_dsp_sub_agent_on[sub_agent]) {
            hal_audio_adda_set_enable_register(enable);
            HAL_AUDIO_LOG_INFO("[DEBUG] #hal_audio_adda_set_enable# Enable HAL_AUDIO_AFE_CONTROL_ADDA", 0);
        }
    } else {
...
bool hal_audio_adda_set_ul_clock(hal_audio_agent_t device_agent, bool enable)
{
    ...
    if ((device_agent == HAL_AUDIO_AGENT_DEVICE_ADDA_UL2_DUAL) || (device_agent == HAL_AUDIO_AGENT_DEVICE_ADDA_UL2_L) || (device_agent == HAL_AUDIO_AGENT_DEVICE_ADDA_UL2_R)) {
        hal_audio_clock_enable_adc2(enable);
    } else if ((device_agent == HAL_AUDIO_AGENT_DEVICE_ADDA_UL3_DUAL) || (device_agent == HAL_AUDIO_AGENT_DEVICE_ADDA_UL3_L) || (device_agent == HAL_AUDIO_AGENT_DEVICE_ADDA_UL3_R)) {
        hal_audio_clock_enable_adc3(enable);
    } else if ((device_agent == HAL_AUDIO_AGENT_DEVICE_ADDA_UL4_DUAL) || (device_agent == HAL_AUDIO_AGENT_DEVICE_ADDA_UL4_L) || (device_agent == HAL_AUDIO_AGENT_DEVICE_ADDA_UL4_R)) {
...
#endif
        hal_audio_dl_set_src_enable(true);
        if ((hal_volume_get_analog_mode(AFE_HW_ANALOG_GAIN_OUTPUT) == HAL_AUDIO_ANALOG_OUTPUT_CLASSAB) || (hal_volume_get_analog_mode(AFE_HW_ANALOG_GAIN_OUTPUT) == HAL_AUDIO_ANALOG_OUTPUT_CLASSG2) || (hal_volume_get_analog_mode(AFE_HW_ANALOG_GAIN_OUTPUT) == HAL_AUDIO_ANALOG_OUTPUT_CLASSG3))  {
...
static S32 AFE_SILENCE_GAIN_THRESHOLD_DB = -85; // mute
...
bool hal_audio_dl_set_nle(VOID *DestBuf, U16 CopySize, U32 format, SOURCE_TYPE type)
{
    ...
    if (format == 2) {
        ...
        for (index = 0; index < CopySize; index += 4) {
            ...
            //HAL_AUDIO_LOG_INFO("gain_threshold_32:%d, |data_32|0:%d, |data_32|+gain:%d", 3, gain_threshold_32, abs(data_32), abs(data_32)-gain_32);
...
bool hal_audio_dl_reset_sdm(VOID *DestBuf, U16 CopySize, U32 format, SOURCE_TYPE type)
{
    ...
    //HAL_AUDIO_LOG_INFO("[RESET SDM] first_silence 0:%d", 1, first_silence);

    int index = 0;
    U32 data_address = 0;
    S16 data_16 = 0;
    S32 data_32 = 0;
    //HAL_AUDIO_LOG_INFO("[RESET SDM] HW_DIGITAL_GAIN_DB = %d, silence_gain_threshold_16:%d, silence_gain_threshold_32:%d", 3, HW_DIGITAL_GAIN_DB, silence_gain_threshold_16, silence_gain_threshold_32);
    afe_hardware_digital_gain_t gain_select;

    if (type == SOURCE_TYPE_CM4_VP_PLAYBACK) {
        gain_select = AFE_HW_DIGITAL_GAIN2;
    } else {
        gain_select = AFE_HW_DIGITAL_GAIN1;
    }
    if (hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_ANC) ||  hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_DEVICE_SIDETONE)) {
        hal_audio_dl_reset_sdm_enable(false);
        silence_sample_count = 0;
        return false;
    }
    if (hal_hw_gain_get_target(gain_select) == 0) {
    ...
    if (format == 2) {
        ...
        for (index = 0; index < CopySize; index += 4) {
            data_address = (U32)(DestBuf + index);
            data_32 = *(S32 *)(data_address);
            //HAL_AUDIO_LOG_INFO("silence_gain_threshold_32:%d, |data_32|:%d, |data_32|+gain:%d", 3, silence_gain_threshold_32, abs(data_32), abs(data_32)-hwgain_32);
            if (abs(data_32) > 0) {
                hal_audio_dl_reset_sdm_enable(false);
                silence_sample_count = 0;
                return false;
    ...
    silence_sample_count += (CopySize / format);
    //HAL_AUDIO_LOG_INFO("[RESET SDM] silence_sample_count:%d, data_16:%d, data_32:%d, data_address:0x%x", 4, silence_sample_count, data_16, data_32, data_address);

    if (silence_sample_count >= 4800) {
        hal_audio_dl_reset_sdm_enable(true);
        silence_sample_count = 0;
    }
    return false;
...
bool hal_audio_volume_set_analog_input_gain(hal_audio_volume_analog_input_gain_parameter_t *input_gain)
{
    ...
    if (input_gain->device_interface ==  HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2) {
        gain_select = AFE_HW_ANALOG_GAIN_INPUT2;
    } else if (input_gain->device_interface ==  HAL_AUDIO_CONTROL_DEVICE_INTERFACE_3) {
...
void hal_audio_irq_src_in_stream_handler(hal_nvic_irq_t irq)
{
    UNUSED(irq);
    ...
    // U16 outSRC_wo;
    // U16 outSRC_count;
    // U16 outSRC_mem_size = 12288;
    irq_status = AFE_GET_REG(ASM_IFR)&ASM_IFR_MASK;
    irq_en_mask = (AFE_GET_REG(ASM_IER))&ASM_IFR_MASK;

    if(irq_status & 0x100){
        // outSRC_ro = AFE_READ(ASM_CH01_OBUF_RDPNT)- AFE_READ(ASM_OBUF_SADR);
        // outSRC_wo = AFE_READ(ASM_CH01_OBUF_WRPNT)-AFE_GET_REG(ASM_OBUF_SADR);
        // outSRC_count = (outSRC_wo > outSRC_ro) ?
        //                                 outSRC_wo - outSRC_ro :
        //                                 outSRC_wo + outSRC_mem_size - outSRC_ro;
        ...
        if(gHwsrc_port_semphr && (pdTRUE == xSemaphoreGiveFromISR(gHwsrc_port_semphr,&xHigherPriorityTaskWoken)))
        {
            ...
            hal_src_set_irq_enable(0, FALSE);
...
//    uint32_t sample_size = Clock_Skew_Asrc_Get_Input_SampleSize(sink->transform->source, sink);
    ...
    afe_asrc_compensating_t pcdc_base_step = COMPENSATING_ADD_0_XPPM;
    afe_asrc_compensating_t asrc_step = COMPENSATING_ADD_0_XPPM;
    ...
    static uint32_t hw_output_current_write_idx = 0;
        ...
        if (hal_audio_status_get_agent_status(HAL_AUDIO_AGENT_MEMORY_DL1)) {
            ...
            if (afe_get_asrc_irq_is_enabled(AFE_MEM_ASRC_1, ASM_IER_IBUF_EMPTY_INTEN_MASK) && (irq_status & ASM_IFR_IBUF_EMPTY_INT_MASK) && (AFE_READ(ASM_CH01_IBUF_WRPNT) == AFE_READ(ASM_CH01_IBUF_RDPNT))) {
                ...
                if (asrc_sinkwpt_InBuf_wpt_count >= (xppm_framesize)) {
                    ...
                    DSP_MW_LOG_W("asrc out prewpt %d wpt %d Rpt %d output_xppm %d outbuflen %d cp %d", 6, pre_hw_output_write_idx, hw_output_current_write_idx, hw_output_read_idx, output_xppm_framesize, sink->param.audio.AfeBlkControl.u4asrc_buffer_size, accumulate_output_xppm_framesize);
#endif /*ASRC_CLCOK_SKEW_DEBUG*/

                    /*** PCDC ASI cnt and drift apply ***/
                    if (pcdc_asi_threshold_counter(sink->transform->source, sink, sample_size, sink->param.audio.src_rate)) {
                        pcdc_base_step = pcdc_drift_select_hwscr_step();

                        /*set step*/
                        if (((int32_t)afe_block->u4asrcSetCompensatedSamples - (int32_t)COMPENSATING_ADD_0_XPPM) > 0) {
                            asrc_step = pcdc_base_step + (afe_block->u4asrcSetCompensatedSamples - COMPENSATING_ADD_0_XPPM);
                            //DSP_MW_LOG_W("asrc ++, step:%d, pcdcstep:%d, clkstep:%d", 3, asrc_step, pcdc_base_step, afe_block->u4asrcSetCompensatedSamples);
                        } else if (((int32_t)afe_block->u4asrcSetCompensatedSamples - (int32_t)COMPENSATING_ADD_0_XPPM) < 0) {
                            asrc_step = pcdc_base_step + (afe_block->u4asrcSetCompensatedSamples - COMPENSATING_ADD_0_XPPM);
                            //DSP_MW_LOG_W("asrc --, step:%d, pcdcstep:%d, clkstep:%d", 3, asrc_step, pcdc_base_step, afe_block->u4asrcSetCompensatedSamples);
                        } else {
                            asrc_step = pcdc_base_step;
                            //DSP_MW_LOG_W("asrc pcdc, step:%d, pcdcstep:%d, clkstep:%d", 3, asrc_step, pcdc_base_step, afe_block->u4asrcSetCompensatedSamples);
                        }
                    } else {
                        asrc_step = afe_block->u4asrcSetCompensatedSamples;
                    }

                    afe_set_asrc_compensating_sample(AFE_MEM_ASRC_1, sink->param.audio.rate, asrc_step);
                    if ((asrc_step) <= COMPENSATING_ADD_976_XPPM && (int32_t)(asrc_step) >= (int32_t)COMPENSATING_SUB_976_XPPM) {
                        afe_block->u4asrcGetaccumulate_array[asrc_step]++;
                        //DSP_MW_LOG_W("asrc set compensated sample %d, sample_counter:0x%x, decode_asi:0x%x",3,asrc_step, sample_counter, sink->transform->source->param.n9_a2dp.predict_asi);
                    } else {
                        DSP_MW_LOG_W("asrc set wrong compensated sample %d", 1, asrc_step);
                    }
...
#else
    ret |= hal_nvic_register_isr_handler(AUDIOSYS1_IRQn, (hal_nvic_isr_t)hal_audio_irq_src_in_stream_handler);
#endif
