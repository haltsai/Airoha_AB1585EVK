...
void afe_set_asrc_compensating_sample(afe_mem_asrc_id_t asrc_id, uint32_t output_buffer_rate, afe_asrc_compensating_t cp_point)
{
    uint32_t  addr_offset;
    addr_offset = asrc_id * 0x100;
    S32 step = cp_point - (AFE_HWSRC_STEP_NUM - 1) / 2;
    U32 RG_Value = ((output_buffer_rate * (U32)4096) / (U32)100);

    output_buffer_rate = 96000;

    if (cp_point < AFE_HWSRC_STEP_NUM) {
        if (step < 0) {
            step = step * (-1);
//            RG_Value = (((output_buffer_rate*(U32)4096)/(U32)100) - (output_buffer_rate*(U32)step*4096)/(4096*4*100));
            RG_Value = (((output_buffer_rate * (U32)4096) / (U32)100) - (output_buffer_rate * (U32)step) / (4 * 100));
        } else {
//            RG_Value = (((output_buffer_rate*(U32)4096)/(U32)100) + (output_buffer_rate*(U32)step*4096)/(4096*4*100));
            RG_Value = (((output_buffer_rate * (U32)4096) / (U32)100) + (output_buffer_rate * (U32)step) / (4 * 100));
        }
        ...
        log_hal_msgid_error("asrc set wrong compensating sample %d", 1, cp_point);
    }
