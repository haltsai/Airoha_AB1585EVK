...
hal_audio_path_interconnection_input_t hal_audio_path_get_input_interconnection(hal_audio_path_selection_parameter_t *path_selection)
{
    ...
    switch (path_selection->port) {
        ...
        case HAL_AUDIO_CONTROL_DEVICE_DIGITAL_MIC_DUAL:
            if (path_selection->parameters.audio_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_1) {
                interconnection_input = AUDIO_INTERCONNECTION_INPUT_I08;
            } else if (path_selection->parameters.audio_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2) {
                interconnection_input = AUDIO_INTERCONNECTION_INPUT_I10;
            } else if (path_selection->parameters.audio_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_3) {
                interconnection_input = AUDIO_INTERCONNECTION_INPUT_I12;
            } else if (path_selection->parameters.audio_interface == HAL_AUDIO_CONTROL_DEVICE_INTERFACE_4) {
...
bool hal_audio_path_set_connection(hal_audio_path_parameter_t *handle, hal_audio_control_status_t control)
{
    ...
    for (connection_sequence = 0 ; connection_sequence < handle->connection_number ; connection_sequence++) {
        ...
        if (handle->with_updown_sampler[connection_select]) {
            HAL_AUDIO_LOG_INFO("upwdown handle->audio_input_rate:%d handle->audio_output_rate:%d\r\n", 2, handle->audio_input_rate,handle->audio_output_rate);
            ...
            if (control == 0) {
                ...
                uint64_t up_ch23_l = ((uint64_t)AFE_READ(AFE_CONN38_1)) << 32 | AFE_READ(AFE_CONN38);
                uint64_t up_ch23_r = ((uint64_t)AFE_READ(AFE_CONN39_1)) << 32 | AFE_READ(AFE_CONN39);
                // check input port
                ...
                if (bit_position & dn_ch23_r) { // First in Last out
                    ...
                    updown_id = AFE_UPDOWN_SAMPLER_UP_CH01;
                } else if (bit_position & up_ch23_r) {
                    upwdown_sampler_input_port = AUDIO_INTERCONNECTION_OUTPUT_O39;
                    upwdown_sampler_output_port = AUDIO_INTERCONNECTION_INPUT_I44;
                    updown_configuration.is_echo_configure_input = true;
                    updown_id = AFE_UPDOWN_SAMPLER_UP_CH23;
                } else if (bit_position & up_ch23_l) {
                    upwdown_sampler_input_port = AUDIO_INTERCONNECTION_OUTPUT_O38;
                    upwdown_sampler_output_port = AUDIO_INTERCONNECTION_INPUT_I45;
                    updown_configuration.is_echo_configure_input = true;
                    updown_id = AFE_UPDOWN_SAMPLER_UP_CH23;
                ...
                if ((handle->audio_input_rate[connection_select] % handle->audio_output_rate[connection_select] == 0)||(handle->audio_output_rate[connection_select] % handle->audio_input_rate[connection_select] == 0)) {
                    ...
                    if (handle->audio_input_rate[connection_select] >= handle->audio_output_rate[connection_select]) {
                        //Down sampler
                        updown_id = AFE_UPDOWN_SAMPLER_DOWN_CH01;
                        if ((connection_sequence+1 + control) > 3) { // each down sampler has only 2 channel
                            updown_id = AFE_UPDOWN_SAMPLER_DOWN_CH23;
                            if ((connection_sequence+1 + control) > 3) {
                        ...
                        //Up sampler
                        updown_id = AFE_UPDOWN_SAMPLER_UP_CH01;
                        if ((connection_sequence+1 + control) > 3) { // each up sampler has only 2 channel
                            updown_id = AFE_UPDOWN_SAMPLER_UP_CH23;
                            if ((connection_sequence+1 + control) > 3) {
                    ...
                    if ((connection_sequence+1 + control) % 2) {
...
WITHOUT_DOWNSAMPLER:
...
#ifndef AIR_MITO_AUDIO_PATH_ENABLE
    ...
    if (input_port_memory_select & HAL_AUDIO_MEMORY_DL_DL1)
    {
        //L
        ...
        hal_audio_path_set_interconnection(AUDIO_INTERCONNECTION_DISCONNECT, model_connection_channel, AUDIO_INTERCONNECTION_INPUT_I14, AUDIO_INTERCONNECTION_OUTPUT_O08);
        //R
        ...
        hal_audio_path_set_interconnection(AUDIO_INTERCONNECTION_DISCONNECT, model_connection_channel, AUDIO_INTERCONNECTION_INPUT_I15, AUDIO_INTERCONNECTION_OUTPUT_O09);
    }
    #endif
