...
int spm_latency_time_checking(void)
{
    ...
    SPM_LOG_I("[dsp][spm_latency_time_checking] SPM_SEQUENCER_32K_REG_1 is 0x%08x", 1, *SPM_SEQUENCER_32K_REG_1);
    SPM_LOG_I("[dsp][spm_latency_time_checking] SPM_SEQUENCER_32K_REG_2 is 0x%08x", 1, *SPM_SEQUENCER_32K_REG_2);
    SPM_LOG_I("[dsp][spm_latency_time_checking] SPM_SEQUENCER_32K_REG_3 is 0x%08x", 1, *SPM_SEQUENCER_32K_REG_3);
    SPM_LOG_I("[dsp][spm_latency_time_checking] SPM_SEQUENCER_32K_REG_4 is 0x%08x", 1, *SPM_SEQUENCER_32K_REG_4);
    ...
    if(latency_time_32k < (int)latency_time_min_32k || latency_time_32k > (int)latency_time_max_32k ){
        SPM_LOG_E("[dsp]spm_latency_time_checking is error, max of latency time is %d in 32k", 1, (int)latency_time_max_32k);

        return(latency_time_error);
    }

    SPM_LOG_I("[dsp][spm_latency_time_checking] latency_time_32k is %d", 1, latency_time_32k);
...
void spm_mtcmos_vow_on(void)
{
    ...
    SPM_LOG_I("[dsp] spm vow mtcmos control on", 0);

    if ((*SPM_PWR_STATUS & 0x200) != 0) {
        SPM_LOG_I("spm_mtcmos_vow is already on\r\n", 0);
        return;
    }
...
void spm_mtcmos_vow_off(void)
{
    ...
    SPM_LOG_I("[dsp] spm vow mtcmos control off", 0);

    if ((*SPM_PWR_STATUS & 0x200) == 0) {
        SPM_LOG_I("spm_mtcmos_vow is already off\r\n", 0);
        return;
    }
