...
static void msg_enqueue(aud_msg_node_t *tx, aud_msg_queue_t queue)
{
    ...
    if (tx->msg.ccni_message[0] & 0x00800000) {   /* put the msg to the front of queue */
        ...
        *front_ptr = tx;
        ...
        tx->msg.ccni_message[0] -= 0x00800000;
...
static hal_ccni_message_t msg_dequeue(aud_msg_queue_t queue)
{
    ...
    if (queue == AUDIO_MSG_TX) {
        ...
        DSP_MW_LOG_D("[REALTIME_RX] rx dequeue\r\n", 0);
        front_ptr = (aud_msg_node_t **)&rx_q_front;
        rear_ptr = (aud_msg_node_t **)&rx_q_rear;
    }
    #ifdef AIR_AUDIO_NONREALTIME_RX_ENABLE
...
static aud_msg_cb_node_t *AUD_MSG_IRQ_LEVEL_GET_CB_NODE(void)
{
    ...
    if (IRQ_Level_Rx_cb_count < IRQ_Level_Rx_cb_count) {
        ...
        DSP_MW_LOG_E("[IRQ_LEVEL_RX] Audio IRQ_LEVEL_RX_msg register callback over size cur:(%d) max:(%d).\r\n", 2, IRQ_Level_Rx_cb_count, IRQ_LEVEL_RX_CB_MAX);
        if (IRQ_Level_Rx_cb_count < 65535) { //UINT16_MAX
            IRQ_Level_Rx_cb_count ++;
        }
#ifdef AIR_RX_TX_CALLBACK_MEMORY_DEDICATE_SIZE_LIMIT_ASSERT
        platform_assert("[IRQ_LEVEL_RX] Audio IRQ_LEVEL_RX_msg register callback over size.", __func__, __LINE__); //Assert if need
#else
        cb_node_ptr = (aud_msg_cb_node_t *)pvPortMalloc(sizeof(aud_msg_cb_node_t));
#endif
    }
...
static void aud_msg_rx_ack(hal_ccni_message_t ack)
{
    ...
    if (rx_ack == NULL) {
        DSP_MW_LOG_E("[REALTIME_RX]AUDIO CCNI MSG ACK malloc failed !!\r\n", 0);
        return;
    }
    ...
    DSP_MW_LOG_D("[REALTIME_RX]rx_ack pMsg[0]=0x%08x, pMsg[1]=0x%08x, tx_q_front=0x%x\r\n", 3, rx_ack->msg.ccni_message[0], rx_ack->msg.ccni_message[1], (int32_t)tx_q_front);
...
#ifdef AIR_AUDIO_NONREALTIME_RX_ENABLE
static void aud_msg_nonrealtime_rx_ack(hal_ccni_message_t ack)
{
    uint32_t tx_msg_queue_empty = false;
    aud_msg_node_t *rx_ack = (aud_msg_node_t *)pvPortMalloc(sizeof(aud_msg_node_t));
    if (rx_ack == NULL) {
        DSP_MW_LOG_E("[NONREALTIME_RX] AUDIO CCNI MSG ACK malloc failed !!\r\n", 0);
        return;
    }
    rx_ack->msg.ccni_message[0] = ack.ccni_message[0] | 0x80000000; /* [31]=1 -> ACK, [23]=1 -> to the front of queue */
    rx_ack->msg.ccni_message[1] = ack.ccni_message[1];
    ...
    /* Enter critical section */
    audio_sem_take(CALLER_NONREALTIME_RX_ACK, xAud_TxMsgSem, false); //xAud_Nonrealtime_Rx_TxMsgSem
    ...
    DSP_MW_LOG_D("[NONREALTIME_RX] rx_ack pMsg[0]=0x%08x, pMsg[1]=0x%08x, tx_q_front=0x%x\r\n", 3, rx_ack->msg.ccni_message[0], rx_ack->msg.ccni_message[1], (int32_t)tx_q_front);
    ...
    /* Leave critical section */
    audio_sem_give(CALLER_NONREALTIME_RX_ACK, xAud_TxMsgSem, false); //xAud_Nonrealtime_Rx_TxMsgSem

    /* Wakeup TX task if necessary */
    if (tx_msg_queue_empty != false) {
        audio_sem_give(CALLER_NONREALTIME_RX_ACK, xAud_TxTaskSem, false);
    }
}
#endif
...
static void aud_msg_IRQ_Level_rx_ack(hal_ccni_message_t ack)
{
    ...
    aud_msg_node_t *rx_ack = (aud_msg_node_t *)pvPortMalloc(sizeof(aud_msg_node_t));
    if (rx_ack == NULL) {
        DSP_MW_LOG_E("[IRQ_LEVEL_RX] AUDIO CCNI MSG ACK malloc failed !!\r\n", 0);
        return;
    }
    ...
    DSP_MW_LOG_D("[IRQ_LEVEL_RX] rx_ack pMsg[0]=0x%08x, pMsg[1]=0x%08x, tx_q_front=0x%x\r\n", 3, rx_ack->msg.ccni_message[0], rx_ack->msg.ccni_message[1], (int32_t)tx_q_front);
...
void aud_msg_rx_task(void)
{
    ...
    while (1) {
        p = &rx_q_front;
        if (*p != NULL) {
            DSP_MW_LOG_D("[REALTIME_RX] rx queue is not empty\r\n", 0);
            ...
            if (cur.ccni_message[0] & 0x80000000) { /* If ccni_message[0] bit 31 = 1, it stands for an ACK */
                ...
                if (cb == NULL) {   /* ack only */
                    ...
                    if ((uint32_t)(end_time - start_time) > REALTIME_RX_TASK_MAX_RUNTIME) {
                        DSP_MW_LOG_W("[REALTIME_RX] long callback time:%d us from rx msg:0x%x", 2, (uint32_t)(end_time - start_time), (uint32_t)msg_id);
                    }
                }
            }
        } else {
            DSP_MW_LOG_D("[REALTIME_RX] rx queue is empty\r\n", 0);
...
void aud_msg_nonrealtime_rx_task(void)
{
    ...
    while (1) {
        ...
        if (*p != NULL) {
            DSP_MW_LOG_D("[NONREALTIME_RX] rx queue is not empty\r\n", 0);

            hal_ccni_mask_event(CCNI_CM4_TO_DSP0_RX_EVENT);    /* to avoid race condition (rx queue)*/
            //hal_ccni_mask_event(AUDIO_N9_RX_EVENT);
            //hal_ccni_mask_event(CCNI_N9_TO_DSP0_EVENT1);
            cur = msg_dequeue(AUDIO_MSG_NONREALTIME_RX);
            memcpy(&ccni_memo_nonrealtime_rx.deq_, &cur, sizeof(hal_ccni_message_t));
            //hal_ccni_unmask_event(CCNI_N9_TO_DSP0_EVENT1);
            //hal_ccni_unmask_event(AUDIO_N9_RX_EVENT);
            hal_ccni_unmask_event(CCNI_CM4_TO_DSP0_RX_EVENT);
            ...
            if (cur.ccni_message[0] & 0x80000000) { /* If ccni_message[0] bit 31 = 1, it stands for an ACK */
                ...
                if (cb == NULL) {   /* ack only */
                    ...
                    aud_msg_nonrealtime_rx_ack(ack);//aud_msg_rx_ack(ack);
                } else {
                    uint32_t start_time, end_time;
                    hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &start_time);
                    if (cb->ack_option & 0x1) { /* NO ACK */
                        if (cb->cb_func != NULL) {
                            rx_cb.callback = cb->cb_func;
                            rx_cb.callback(cur, NULL);
                            memcpy(&ccni_memo_nonrealtime_rx.exec_, &cur, sizeof(hal_ccni_message_t));
                        }
                    } else if (cb->ack_option & 0x2) {   /* execute callback function before response ACK. */
                        memset((void *)&ack, 0, sizeof(hal_ccni_message_t));
                        ack.ccni_message[0] = cur.ccni_message[0] & 0x7FFF0000;    /* copy ID to ack */
                    ...
                    if ((uint32_t)(end_time - start_time) > NONREALTIME_RX_TASK_MAX_RUNTIME) {
                        DSP_MW_LOG_W("[NONREALTIME_RX] long callback time:%d us from rx msg:0x%x", 2, (uint32_t)(end_time - start_time), (uint32_t)msg_id);
                    }
                }
            }
        } else {
            DSP_MW_LOG_D("[NONREALTIME_RX] rx queue is empty\r\n", 0);
...
void aud_msg_IRQ_Level_Rx_execute(hal_ccni_message_t *irq_rx)
{
    ...
    if (cb == NULL) {   /* ack only */
        memset((void *)&ack, 0, sizeof(hal_ccni_message_t));
        ack.ccni_message[0] = cur.ccni_message[0] & 0x7FFF0000;    /* copy ID to ack */
        DSP_MW_LOG_D("[IRQ_LEVEL_RX] aud_msg_IRQ_Level_Rx_execute, CB NULL\r\n", 0);
        ...
        if ((uint32_t)(end_time - start_time) > 100) {
            DSP_MW_LOG_E("[IRQ_LEVEL_RX] long callback time:%d us from rx msg:0x%x", 2, (uint32_t)(end_time - start_time), (uint32_t)msg_id);
        ...
        if (tx_q_front != NULL) {
            ...
            if(((cur.ccni_message[0] & 0x7FFF0000) >> 16) == (MSG_DSP2MCU_RECORD_DATA_NOTIFY)){
                if (record_ccni_msg_existed) {
                    ...
                    DSP_MW_LOG_W("[Record_Notify_Special_Case] MSG 0x%x is sent, clear flag %d\r\n", 2, ((cur.ccni_message[0] & 0x7FFF0000) >> 16), record_ccni_msg_existed);
                }
            }
            #endif
            ...
            if ((msg_ID & MSG_DSP2LOCAL_AUDIO_PROCESS) != MSG_DSP2LOCAL_AUDIO_PROCESS) {
                ...
                if (gpt_timer0) {
                    ...
                    if ((uint32_t)(gpt_timer1 - gpt_timer0) > TX_TASK_MAX_RUNTIME) {
                        DSP_MW_LOG_W("DSP Tx long callback time:%d us from tx msg:0x%x", 2, (uint32_t)(gpt_timer1 - gpt_timer0), msg_ID);
                    }
                }
            } else {
...
#else
#if defined(AIR_GAMING_MODE_DONGLE_ENABLE) || defined(AIR_BLE_AUDIO_DONGLE_ENABLE) || defined(AIR_ULL_AUDIO_V2_DONGLE_ENABLE) || defined(AIR_ADVANCED_PASSTHROUGH_ENABLE)
...
static int msg_nonrealtime_rx_task_init(void)
{
    ...
    if (ret != 1) {
        DSP_MW_LOG_E("[NONREALTIME_RX] Audio MSG NONREALTIME_RX task create fail !!\r\n", 0);
        return 0;
    }
    return 1;
}
...
void aud_msg_rx_handler(hal_ccni_event_t event, void *msg)
{
    ...
    if(pMsg[0] & 0x40000000){ /* If ccni_message[0] bit 30 = 1, it stands for an IRQ Level CCNI */
        ...
        if (irq_rx == NULL) {
            DSP_MW_LOG_E("[IRQ_LEVEL_RX] AUDIO CCNI MSG malloc failed !!\r\n", 0);
        ...
        DSP_MW_LOG_D("[IRQ_LEVEL_RX] Rx_handler end\r\n", 0);
        goto _Rx_Error_Handleing;
        #endif
...
aud_msg_status_t aud_msg_tx_handler(hal_ccni_message_t msg, uint8_t to_the_front, uint8_t from_ISR)
{
    ...
    if (audio_sem_take(CALLER_TX_HDR, xAud_TxMsgSem, (BOOL)from_ISR) == true) {
        ...
        if (msg.ccni_message[0] == (MSG_DSP2MCU_RECORD_DATA_NOTIFY << 16)) {
            if (record_ccni_msg_existed) {
                DSP_MW_LOG_W("[Record_Notify_Special_Case] MSG 0x%x existed in TX queue, ignore the new one.\r\n", 1, MSG_DSP2MCU_RECORD_DATA_NOTIFY);
                /* Leave critical section */
                ...
                DSP_MW_LOG_W("[Record_Notify_Special_Case] Receive MSG 0x%x, set flag %d.\r\n", 2, MSG_DSP2MCU_RECORD_DATA_NOTIFY, record_ccni_msg_existed);
            }
        }
        #endif
...
{
    DSP_MW_LOG_I("[IRQ_LEVEL_RX] aud_msg_IRQ_level_rx_register_callback, ID 0x%x\r\n", 1, ID);
#ifdef AIR_RX_TX_CALLBACK_MEMORY_DEDICATE_ENABLE
    if (IRQ_Level_Rx_cb_count == 0) { //AUD_MSG_GET_CB_NODE!!
        ...
        if (IRQ_Level_Rx_cb_mem_pool == NULL) {
            DSP_MW_LOG_E("[IRQ_LEVEL_RX] AUDIO CCNI MSG CB malloc mem poll failed !!\r\n", 0);
    ...
    if (cur == NULL) {  /* Nothing in the queue */
        ...
        DSP_MW_LOG_I("[IRQ_LEVEL_RX] AUDIO CCNI MSG CB RX cur:(%d) max:(%d) id(0x%x) ptr(0x%x).\r\n", 4, IRQ_Level_Rx_cb_count, IRQ_LEVEL_RX_CB_MAX, ID, cb); //debug
#endif
#else
        aud_msg_cb_node_t *cb = (aud_msg_cb_node_t *)pvPortMalloc(sizeof(aud_msg_cb_node_t));
#endif
        if (cb == NULL) {
            DSP_MW_LOG_E("[IRQ_LEVEL_RX] AUDIO CCNI MSG CB malloc failed !!\r\n", 0);
            return AUDIO_MSG_STATUS_ERROR;
        }
...
aud_msg_status_t aud_msg_init(void)
{
    ...
    if (xAud_Nonrealtime_RxTaskSem == NULL) {
        DSP_MW_LOG_E("[NONREALTIME_RX] Audio xAud_Nonrealtime_RxTaskSem init failed\r\n", 0);
