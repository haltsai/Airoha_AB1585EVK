...
ATTR_TEXT_IN_IRAM void i2s_slave_0_ul_interrupt_handler(vdma_event_t event, void  *user_data)
{
    ...
    if (afe_block->u4asrcflag) {
        if (AFE_GET_REG(ASM_CH01_OBUF_RDPNT) == AFE_GET_REG(ASM_CH01_OBUF_WRPNT)) {
            printf("slave 0 asrc out buffer RPTR=WPTR, R=0x%x, W=0x%x, asrc in buffer, R=0x%x, W=0x%x", AFE_GET_REG(ASM_CH01_OBUF_RDPNT), AFE_GET_REG(ASM_CH01_OBUF_WRPNT), AFE_GET_REG(ASM_CH01_IBUF_RDPNT), AFE_GET_REG(ASM_CH01_IBUF_WRPNT));
        ...
        #ifndef AIR_GAMING_MODE_DONGLE_I2S_IN_ENABLE
...
ATTR_TEXT_IN_IRAM void i2s_slave_1_ul_interrupt_handler(vdma_event_t event, void  *user_data)
{
    ...
    if (afe_block->u4asrcflag) {
        if (AFE_GET_REG(ASM_CH01_OBUF_RDPNT) == AFE_GET_REG(ASM_CH01_OBUF_WRPNT)) {
            printf("slave 1 asrc out buffer RPTR=WPTR, R=0x%x, W=0x%x, asrc in buffer, R=0x%x, W=0x%x", AFE_GET_REG(ASM_CH01_OBUF_RDPNT), AFE_GET_REG(ASM_CH01_OBUF_WRPNT), AFE_GET_REG(ASM_CH01_IBUF_RDPNT), AFE_GET_REG(ASM_CH01_IBUF_WRPNT));
            source->streamBuffer.BufferInfo.bBufferIsFull = TRUE;
        }
        afe_block->u4ReadIdx = AFE_GET_REG(ASM_CH01_IBUF_RDPNT) - AFE_GET_REG(ASM_IBUF_SADR);
        //DSP_MW_LOG_I("[HAS][HWSRC2] read point = 0x%x",1,afe_block->u4ReadIdx);
...

        {
            ...
            if (source && source->transform && source->transform->sink) {
                ...
                if (sink->type == SINK_TYPE_N9BLE && callback_ptr->Status == CALLBACK_HANDLER) {
                    printf("Callback busy! source=%d, sink=%d", source->type, sink->type);
#if !DL_TRIGGER_UL
...
void afe_dl3_interrupt_handler(void)
{
    ...
    if (1) {//afe_get_memory_path_enable(AUDIO_DIGITAL_BLOCK_MEM_DL3)
        ...
        #if 0 // dl3 agent read dump
        uint32_t osize,dl3_read_offset;
        static uint32_t pre_dl3_read_offset = 0;
        osize = AFE_GET_REG(AFE_DL3_END) - dl_base_addr+1;

        dl3_read_offset = hw_current_read_idx - dl_base_addr;
        U32 cur_data_size = dl3_read_offset >= pre_dl3_read_offset
                                ? dl3_read_offset - pre_dl3_read_offset
                                : dl3_read_offset + osize - pre_dl3_read_offset;  
        U16 data_size1 = 0,data_size2=0,data_size=0;
        if (dl3_read_offset >= pre_dl3_read_offset) {
            data_size = dl3_read_offset - pre_dl3_read_offset;
            LOG_AUDIO_DUMP(dl_base_addr+pre_dl3_read_offset,cur_data_size,AUDIO_DCHS_DL_UART_CH);
        } else {
            data_size1 = osize - pre_dl3_read_offset;
            LOG_AUDIO_DUMP(dl_base_addr+pre_dl3_read_offset,data_size1,AUDIO_DCHS_DL_UART_CH);
            data_size2 = dl3_read_offset;
            LOG_AUDIO_DUMP(dl_base_addr,data_size2,AUDIO_DCHS_DL_UART_CH);
        }
        pre_dl3_read_offset = dl3_read_offset;
        #endif
#if defined (AIR_WIRED_AUDIO_ENABLE)
...
{
    uint32_t ack_msg;
    ack_msg = ((MSG_DSP2MCU_AUDIO_AMP << 16) | enable);
    DTM_enqueue(DTM_EVENT_ID_AUDIO_AMP_NOTIFY, ack_msg, false);
    DSP_MW_LOG_E("[DSP AMP] enqueue to dtm task, id [0x%x], enable %d\r\n", 2, ack_msg, enable);
}
