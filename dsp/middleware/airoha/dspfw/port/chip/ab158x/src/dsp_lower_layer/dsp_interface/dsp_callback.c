...
TaskHandle_t  DSP_Callback_Config(SOURCE source, SINK sink, VOID *feature_list_ptr, BOOL isEnable)
{
    ...
    if (sink->type == SINK_TYPE_VP_AUDIO) {
    ...
    } else if ((sink->type == SINK_TYPE_AUDIO_DL3) || (sink->type == SINK_TYPE_DSP_VIRTUAL)) {
        #if defined(AIR_ADVANCED_PASSTHROUGH_ENABLE)
        if (source->param.audio.scenario_id == AUDIO_TRANSMITTER_ADVANCED_PASSTHROUGH) {
        ...
        sink->taskid = DHP_TASK_ID;
#endif
#if defined(AIR_BT_LE_LC3PLUS_ENABLE) && defined(AIR_BT_CODEC_BLE_ENABLED)
    } else if ((source->type == SOURCE_TYPE_N9BLE)
               && (source->param.n9ble.codec_type== BT_BLE_CODEC_LC3PLUS)) {
        source->taskId = DHP_TASK_ID;
        sink->taskid = DHP_TASK_ID;
#endif
...
#ifdef AIR_AUDIO_I2S_SLAVE_TDM_ENABLE
    if (stream->source->type == SOURCE_TYPE_AUDIO || (stream->source->type >= SOURCE_TYPE_SUBAUDIO_MIN && stream->source->type <= SOURCE_TYPE_SUBAUDIO_MAX) || (stream->source->type == SOURCE_TYPE_TDMAUDIO)) {
#else
...
#else
#ifdef AIR_AUDIO_I2S_SLAVE_TDM_ENABLE
    if ((stream->sink->type == SINK_TYPE_AUDIO) || (stream->sink->type == SINK_TYPE_VP_AUDIO) || (stream->sink->type == SINK_TYPE_AUDIO_DL3) || (stream->sink->type == SINK_TYPE_AUDIO_DL12) || (stream->sink->type == SINK_TYPE_TDMAUDIO) || (stream->sink->type == SINK_TYPE_ADV_PASSTRU))
#else
    if ((stream->sink->type == SINK_TYPE_AUDIO) || (stream->sink->type == SINK_TYPE_VP_AUDIO) || (stream->sink->type == SINK_TYPE_AUDIO_DL3) || (stream->sink->type == SINK_TYPE_AUDIO_DL12) || (stream->sink->type == SINK_TYPE_ADV_PASSTRU))
#endif
    {
...
VOID DSP_Callback_ParaSetup(DSP_STREAMING_PARA_PTR stream)
{
...
#endif
    {
        ...
        {
            if (source->param.bt_common.scenario_sub_id == AUDIO_TRANSMITTER_ULL_AUDIO_V2_DONGLE_UL_USB_OUT_0)
            {
...
#endif /* AIR_FULL_ADAPTIVE_ANC_ENABLE */
#if defined AIR_DUAL_CHIP_MIXING_MODE_ROLE_MASTER_ENABLE || defined AIR_DUAL_CHIP_MIXING_MODE_ROLE_SLAVE_ENABLE
    ...
    if (sink->type == SINK_TYPE_N9SCO) {
        ...
        else if (sink->param.data_ul.scenario_type == AUDIO_TRANSMITTER_ULL_AUDIO_V2_DONGLE)
        {
            ...
            if (sink->param.data_ul.scenario_sub_id == AUDIO_TRANSMITTER_WIRED_AUDIO_USB_OUT) {
                frameSize = sink->param.data_ul.max_payload_size;
                stream->callback.EntryPara.out_channel_num          = 2;
            }
        }
#endif
...
ATTR_TEXT_IN_IRAM VOID DSP_Callback_Processing(DSP_STREAMING_PARA_PTR stream) {
    ...
    if (handler.handlingStatus == CALLBACK_HANDLER) {
        ...
        if (!((stream->sink->type <= SINK_TYPE_AUDIO_TRANSMITTER_MIN) && (stream->sink->type >= SINK_TYPE_AUDIO_TRANSMITTER_MAX) && (stream->sink->param.data_ul.scenario_sub_id == AUDIO_TRANSMITTER_GAMING_MODE_VOICE_HEADSET))) {
            ...
            if (stream->streamingStatus == STREAMING_START || stream->streamingStatus == STREAMING_DEINIT) {
                ...
                stream->streamingStatus = STREAMING_ACTIVE;
                #ifdef AIR_BT_CODEC_BLE_ENABLED
                if (stream->sink->type == SINK_TYPE_N9BLE)
                {
                    *callbackOutSizePtr = 0;
                }
                else
    ...
    if ((((stream->callback.Status == CALLBACK_SUSPEND) &&
          ...
          ((stream->source->type != SOURCE_TYPE_A2DP && stream->source->param.n9_a2dp.codec_info.codec_cap.type != BT_A2DP_CODEC_AIRO_CELT) || (stream->source->type == SOURCE_TYPE_A2DP && stream->source->param.n9_a2dp.codec_info.codec_cap.type == BT_A2DP_CODEC_AIRO_CELT && (afe_get_dl1_query_data_amount() < 124))) &&
#endif
#endif
