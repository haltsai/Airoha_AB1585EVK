...
#define ReadREG(_addr)          (*(volatile uint32_t *)(_addr))
    ...
    if (source->scenario_type == AUDIO_SCENARIO_TYPE_GAMING_MODE_MUSIC_DONGLE_I2S_IN) {
        DSP_MW_LOG_I("[ULL][I2S IN] vdma addr 0x%x size %d threshold %d, src1 in_addr 0x%x size %d, src1 out_addr 0x%x size %d, src2 in_addr 0x%x size %d, src2 out_addr 0x%x size %d", 11,
        ...
        /* We should stop I2S DMA and start I2S DMA by ULL */
        ...
        if (vdma_setting.audio_interface == HAL_AUDIO_INTERFACE_1) {
            ...
            DSP_MW_LOG_I("DSP UL start i2s slave audio_interface = %d fail", 1, vdma_setting.audio_interface);
            return -1;
        }
        dma_channel = g_i2s_slave_vdma_channel_infra[port_tmp * 2 + vdma_setting.is_ul_mode];
        i2s_vdma_status = vdma_stop(dma_channel);
        if (i2s_vdma_status != VDMA_OK) {
            DSP_MW_LOG_I("DSP UL start i2s slave set vdma_stop fail %d", 1, i2s_vdma_status);
            return -1;
        }
    }
#else
