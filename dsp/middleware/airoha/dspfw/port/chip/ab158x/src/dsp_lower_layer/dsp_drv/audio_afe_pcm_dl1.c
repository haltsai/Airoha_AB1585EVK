...
static int32_t pcm_dl1_start(SINK sink)
{
    ...
    if (mem_handle->memory_select == HAL_AUDIO_MEMORY_DL_DL1 || mem_handle->memory_select == HAL_AUDIO_MEMORY_DL_SRC1) {
        ...
        get_value_handle.get_device_rate.device_interface = (hal_audio_device_interface_t)runtime->audio_interface;
...
static int32_t pcm_dl1_open(SINK sink)
{
...
#else
    #ifdef AIR_MITO_AUDIO_PATH_ENABLE
        if (gAudioCtrl.Afe.AfeDLSetting.scenario_type == AUDIO_SCENARIO_TYPE_HFP_DL) {
            hal_audio_device_parameter_i2s_master_t device_i2s_master_temp = {
            HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER,
            HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2,
            48000,
            AUDIO_SCENARIO_TYPE_HFP_DL,
            0,
            HAL_AUDIO_I2S_I2S,
            HAL_AUDIO_I2S_WORD_LENGTH_32BIT,
            0,
            0,
            0,
            0,
        ...
        if (handle->audio_output_rate[0] == 0) {
            ...
            assert(0);
                ...
                if (gAudioCtrl.Afe.AfeDLSetting.scenario_type == AUDIO_SCENARIO_TYPE_HFP_DL) {
                    ...
                    path1_temp.scenario_type = AUDIO_SCENARIO_TYPE_HFP_DL;
                    ...
                    path3_temp.scenario_type = AUDIO_SCENARIO_TYPE_HFP_DL;
    ...
    //sine_generator.port_parameter.device_interface = HAL_AUDIO_CONTROL_DEVICE_INTERFACE_1;
...
static int32_t pcm_dl1_close(SINK sink)
{
    ...
    hal_audio_set_device(device_handle, device, HAL_AUDIO_CONTROL_OFF);
        ...
        if (sink->scenario_type == AUDIO_SCENARIO_TYPE_HFP_DL) {
            hal_audio_device_parameter_i2s_master_t device_i2s_master_temp = {
            HAL_AUDIO_CONTROL_DEVICE_I2S_MASTER,
            HAL_AUDIO_CONTROL_DEVICE_INTERFACE_2,
            48000,
