...
ATTR_TEXT_IN_IRAM U32 SourceSizeAudioAfe(SOURCE source)
{
    ...
    if ((source->streamBuffer.BufferInfo.bBufferIsFull) || (data_size >= audio_frame_size)) { //[CHK this part]
        ...
        if ((source->scenario_type == AUDIO_SCENARIO_TYPE_GAMING_MODE_MUSIC_DONGLE_LINE_IN) && (hal_nvic_query_exception_number() > 0)) {
            DSP_MW_LOG_E("[SourceSizeAudioAfe] ull dongle line in afe no enough data, bBufferIsFull:%d, audio_frame_size%d, data_size:%d, wo:%d, ro:%d, length:%d, buffer_per_channel_shift:%d", 7, source->streamBuffer.BufferInfo.bBufferIsFull, audio_frame_size, data_size, writeOffset, readOffset, length, buffer_per_channel_shift);
        }
#endif /* AIR_GAMING_MODE_DONGLE_LINE_IN_ENABLE */
#ifdef AIR_GAMING_MODE_DONGLE_I2S_IN_ENABLE
        if ((source->scenario_type == AUDIO_SCENARIO_TYPE_GAMING_MODE_MUSIC_DONGLE_I2S_IN) && (hal_nvic_query_exception_number() > 0)) {
            DSP_MW_LOG_E("[SourceSizeAudioAfe] ull dongle i2s in afe no enough data, bBufferIsFull:%d, audio_frame_size%d, data_size:%d, wo:%d, ro:%d, length:%d, buffer_per_channel_shift:%d", 7, source->streamBuffer.BufferInfo.bBufferIsFull, audio_frame_size, data_size, writeOffset, readOffset, length, buffer_per_channel_shift);
        }
#endif /* AIR_GAMING_MODE_DONGLE_I2S_IN_ENABLE */
        return 0;
    }
