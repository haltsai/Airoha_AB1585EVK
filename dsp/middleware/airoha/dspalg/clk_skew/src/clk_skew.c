...
ATTR_TEXT_IN_IRAM_LEVEL_2 S16 clk_skew_check_status(DSP_CLOCK_SKEW_CTRL_t *pClkSkewCtrl, DSP_CLOCK_SKEW_SETUP_t* ClkSkewSetup)
{
    ...
    if ((pClkSkewCtrl->IntrDownCnt > 0) && (pClkSkewCtrl->CompensatedPolarity != 0)) {
        ...
        if ((pClkSkewCtrl->IntrDownCnt == 1) && (dir == CLK_SKEW_UL) && (pClkSkewCtrl->InterruptHandleCnt == 3) && (pClkSkewCtrl->CompensatedPolarity < 0)) {
        ...
        if ((pClkSkewCtrl->AccumulatedSamples > 0) && (pClkSkewCtrl->CompensatedSamples == 0)) {
            ...
            if (dir == CLK_SKEW_DL) {
                ...
                if (pClkSkewCtrl->UL_Prev_Polarity == 0) {
                    ...
                    DSP_MW_LOG_I("[CLK_SKEW][%d] first set polarity: %d", 2, dir, pClkSkewCtrl->UL_Prev_Polarity);
                } else if (pClkSkewCtrl->UL_Prev_Polarity == 1) {
                    DSP_MW_LOG_I("[CLK_SKEW][%d] UL polarity is diffrent, prev:%d, now:%d", 3, dir, pClkSkewCtrl->UL_Prev_Polarity, pClkSkewCtrl->CompensatedPolarity);
                ...
                if (pClkSkewCtrl->InterruptHandleCnt == 0) {
                    if (pClkSkewCtrl->UL_Pol_Change_Samples == -1) {
                        DSP_MW_LOG_I("[CLK_SKEW] UL_Pol_Change_Samples != 0:%d", 1, pClkSkewCtrl->UL_Pol_Change_Samples);
...
#else
        fs = clk_skew_fs_converter((stream_samplerate_t)(U8)stream_function_get_samplingrate(para));
#endif
    }else{
...
ATTR_TEXT_IN_IRAM_LEVEL_2 VOID Clock_Skew_Offset_Update(BT_CLOCK_OFFSET_SCENARIO type, SOURCE source, SINK sink)
{
    ...
    if (ClkSkewRCDCCtrl->first_offset_flag == TRUE) {
        ...
        if(ClkSkewSetup->clk_skew_dir == CLK_SKEW_DL){
            ClkSkewRCDCCtrl->initial_offset = -(AFE_GET_REG(AFE_AUDIO_BT_SYNC_MON1) & 0xFFFF); // initial offset
            DSP_MW_LOG_I("[CLK_SKEW]init offset:%d",1,ClkSkewRCDCCtrl->initial_offset);
        }
...
{
    DSP_CLOCK_SKEW_PARAM_t* ClkSkewParam = (DSP_CLOCK_SKEW_PARAM_t*)Clock_Skew_Get_Mem_From_SrcSnk(source, sink);
    S16 cp_samples = 0,cp_samples_pcdc = 0;
    ...
    return cp_samples + cp_samples_pcdc;
}
...
//    S8 *Buf1 = (S8 *)stream_function_get_1st_inout_buffer(para);
    ...
    if (FrameSize == 0) {
        return FALSE;
    }  else if ((ClkSkewCtrl->FrameSize == 0) || (ClkSkewCtrl->BytesPerSample == 0)) {//initialize
