...
bool stream_function_leakage_compensation_initialize(void *para)
{
    ...
    if ((uint32_t)mem_size > stream_codec_get_working_buffer_length(para)) {
        DSP_MW_LOG_E("[RECORD_LC]mem size is insufficient. required:%d, alloc:%d", 2, mem_size, stream_codec_get_working_buffer_length(para));
        AUDIO_ASSERT(0);
    }

    DSP_MW_LOG_I("[RECORD_LC]ld_get_aec_nr_memsize()=%d", 1, mem_size);

    p_working_memory = stream_function_get_working_buffer(para);
    if (p_working_memory == NULL) {
        DSP_MW_LOG_I("[Leakage detection] working_buffer allocate fail", 0);
        AUDIO_ASSERT(0);
    }
...
//        DSP_MW_LOG_I("[RECORD_LC]report_cnt:%d, report_res:%d", 2, report_cnt, report_res);
        ...
        if (ld_aecref1_power != 0) {
            ...
            if (mean_power > (double)(ld_NvKey.ld_thrd)/32768) {
                //PASS
                report_res++;
                DSP_MW_LOG_I("[RECORD_LC]frame %d res:%d ->PASS", 2, report_cnt, (int)(mean_power*32768));
            } else {
                //FAIL
                DSP_MW_LOG_I("[RECORD_LC]frame %d res:%d ->FAIL", 2, report_cnt, (int)(mean_power*32768));
            }
