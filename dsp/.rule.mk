...
$(OUTDIR)/%.o : $(ROOTDIR)/%.c
	...
	@if [ -e "$@" ]; then rm -f "$@"; fi ;\
	$(CC) @$(basename $@)_CFLAGS $(INC:%=-I"$(ROOTDIR)/%") $(DEFINE:%=-D%) -MMD -MF $(subst .o,.d,$@) -c -o $@ $< 2>>$(ERR_LOG) ;\
	if [ "$$?" != "0" ]; then \
...
$(OUTDIR)/%.o : $(ROOTDIR)/%.cpp
	...
	@if [ -e "$@" ]; then rm -f "$@"; fi ;\
	$(CPP) $(COM_CPPFLAG) @$(basename $@)_CFLAGS $(INC:%=-I"$(ROOTDIR)/%") $(DEFINE:%=-D%) -MMD -MF $(subst .o,.d,$@) -c -o $@ $< 2>>$(ERR_LOG) ;\
	if [ "$$?" != "0" ]; then \
		echo "Build... $$(basename $@) FAIL"; \
		echo "Build... $@ FAIL" >> $(BUILD_LOG); \
	else \
