...
bool app_smcharger_ui_shell_event(bool from_isr, int event_id, void *data, size_t data_len)
{
    ...
    if (ret != UI_SHELL_STATUS_OK) {
        APPS_LOG_MSGID_E(LOG_TAG" send smcharger event fail", 0);
    }
...
static void app_smcharger_ui_shell_public_event(bool from_isr, app_smcharger_action_t action, uint8_t data)
{
    ...
    if (event_para == NULL) {
        APPS_LOG_MSGID_E(LOG_TAG" shell_public_event fail", 0);
        return;
    }
    ...
    switch (drv_event) {
        ...
        case SMCHG_LID_OPEN: {
            ...
            if (data_len == 1) {
                ...
                APPS_LOG_MSGID_E(LOG_TAG" [DRV]callback, LID_OPEN with error data_len", 0);
            }
            break;
        }
        ...
        case SMCHG_CHG_OFF: {
            ...
            if (data_len == 1) {
                ...
                APPS_LOG_MSGID_E(LOG_TAG" [DRV]callback, OFF with error data_len", 0);
            }
            break;
        }
        ...
        case SMCHG_CHG_KEY: {
            ...
            if (data_len == 1) {
                ...
                APPS_LOG_MSGID_E(LOG_TAG" [DRV]callback, CHARGER_KEY with error data_len", 0);
            }
            break;
        }
        ...
        default: {
            APPS_LOG_MSGID_E(LOG_TAG" [DRV]callback, error event", 0);
            break;
        }
    }
...
static app_smcharger_action_status_t app_smcharger_switch_bt(bool on, bool direct_off)
{
    ...
    if (BT_POWER_ON_NORMAL == bt_power_on_get_config_type()
        ...
        if (on || direct_off) {
            ret = ui_shell_send_event(FALSE, EVENT_PRIORITY_HIGNEST, EVENT_GROUP_UI_SHELL_APP_INTERACTION,
                                      APPS_EVENTS_INTERACTION_REQUEST_ON_OFF_BT,
                                      (void *)on, 0, NULL, 0);
        } else {
            ret = ui_shell_send_event(FALSE, EVENT_PRIORITY_HIGNEST, EVENT_GROUP_UI_SHELL_APP_INTERACTION,
...
{
    APPS_LOG_MSGID_I(LOG_TAG" app_smcharger_init", 0);
...
bool app_smcharger_show_led_bg_pattern()
{
    ...
    switch (battery_state) {
        ...
        case SMCHARGER_BATTERY_STATE_FULL:
            APPS_LOG_MSGID_I(LOG_TAG" show BG_LED none", 0);
            ret = FALSE;
            break;
        case SMCHARGER_BATTERY_STATE_CHARGING:
            APPS_LOG_MSGID_I(LOG_TAG" show BG_LED CHARGING", 0);
        ...
        case SMCHARGER_BATTERY_STATE_CHARGING_FULL:
            APPS_LOG_MSGID_I(LOG_TAG" IDLE BG_LED for CHARGING_FULL", 0);
        ...
        case SMCHARGER_BATTERY_STATE_SHUTDOWN:
            APPS_LOG_MSGID_I(LOG_TAG" show BG_LED LOW_BATTERY/SHUTDOWN", 0);
        ...
        case SMCHARGER_BATTERY_STATE_THR:
            APPS_LOG_MSGID_I(LOG_TAG" show error BG_LED SMCHARGER_BATTERY_STATE_THR", 0);
...
{
    APPS_LOG_MSGID_I(LOG_TAG" handle_case_battery %d", 1, case_battery);
    if (case_battery <= 100
        ...
        APPS_LOG_MSGID_E(LOG_TAG" handle_case_battery invalid case_battery", 0);
    }
}
...
void app_smcharger_handle_key_event(uint32_t key_value)
{
    ...
    if (g_smcharger_context != NULL && key_value <= 255
        ...
        APPS_LOG_MSGID_E(LOG_TAG" handle_key_event invalid key_value", 0);
    }
}
...
void app_smcharger_handle_user_data(int state, int event_id, uint8_t user_data)
{
    ...
    if ((state == STATE_SMCHARGER_LID_OPEN || state == STATE_SMCHARGER_LID_CLOSE)
        ...
        APPS_LOG_MSGID_E(LOG_TAG" handle_user_data invalid key_value", 0);
    }
}
...
static void smcharger_atci_get_context_str(uint8_t *response_buf)
{
    ...
    snprintf((char *)response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE,
             "[%02X] battery_state=%d own=%s[peer=%s] battery=%d[peer=%d] aws_link_type=%d\r\n",
...
void app_smcharger_update_bat()
{
    ...
    if (role == BT_AWS_MCE_ROLE_AGENT && g_smcharger_context != NULL) {
        ...
        if (AUDIO_CHANNEL_L == channel) {
            ...
            APPS_LOG_MSGID_I(LOG_TAG" [XIAOAI_VA] invalid L/R Channel", 0);
        }
...
uint8_t app_smcharger_read_case_battery_nvkey()
{
    ...
    APPS_LOG_MSGID_I(LOG_TAG" read_case_battery_nvkey, read status=%d", 1, status);
    if (status == NVKEY_STATUS_ITEM_NOT_FOUND) {
        size = sizeof(uint8_t);
        status = nvkey_write_data(NVID_APP_CASE_BATTERY_INFO, (const uint8_t *)&case_battery, size);
        APPS_LOG_MSGID_I(LOG_TAG" read_case_battery_nvkey, write status=%d", 1, status);
    }
    return case_battery;
}
