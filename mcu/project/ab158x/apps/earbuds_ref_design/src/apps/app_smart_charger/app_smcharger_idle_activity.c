...
{
    APPS_LOG_MSGID_I(LOG_TAG" Print_SMCharger_Context [%02X] charger_exist=%d charger_state=%d shutdown=%d "
                     "battery_percent=%d [peer=%d] smcharger_state=%d [peer=%d] "
                     "battery_state=%d", 9,
                     bt_device_manager_aws_local_info_get_role(),
                     charger_exist_state, charging_state, ctx->shutdown_state,
...
static app_smcharger_battery_state_t app_smcharger_update_battery_state(app_smcharger_context_t *smcharger_ctx)
{
    ...
    if (smcharger_ctx->battery_state != new_state) {
        ...
        APPS_LOG_MSGID_I(LOG_TAG" same battery_state=%d", 1, new_state);
    }
    return new_state;
}
...
static void smcharger_notify_battery_to_peer(app_smcharger_context_t *smcharger_ctx)
{
    ...
    if (aws_link_type == BT_AWS_MCE_SRV_LINK_NONE) {
        APPS_LOG_MSGID_I(LOG_TAG" [%02X] AWS not connected, no need notify battery", 1, role);
    } else {
        ...
        APPS_LOG_MSGID_I(LOG_TAG" [%02X] Send battery(%d) to peer",
...
static void partner_notify_case_battery_level_to_agent(app_smcharger_context_t *smcharger_ctx)
{
    ...
    bt_aws_mce_srv_link_type_t aws_link_type = bt_aws_mce_srv_get_link_type();
    if (aws_link_type == BT_AWS_MCE_SRV_LINK_NONE) {
        APPS_LOG_MSGID_I(LOG_TAG" [%02X] AWS not connected, no need notify case_battery_level", 1, role);
        ...
        APPS_LOG_MSGID_I(LOG_TAG" [%02X] Send case battery(%d) to agent",
                         2, role, smcharger_ctx->case_battery_percent);
        bt_aws_mce_report_send_event(&aws_data);
    }
}
...
static void smcharger_notify_state_to_peer(app_smcharger_context_t *smcharger_ctx)
{
    ...
    bt_aws_mce_srv_link_type_t aws_link_type = bt_aws_mce_srv_get_link_type();
    if (aws_link_type == BT_AWS_MCE_SRV_LINK_NONE) {
        APPS_LOG_MSGID_I(LOG_TAG" [%02X] AWS not connected, no need notify smcharger_state", 1, role);
    } else {
        ...
        aws_data->param = (void *)data_array;
        APPS_LOG_MSGID_I(LOG_TAG" [%02X] Send smcharger_state(%d) to peer",
...
static bool smcharger_idle_battery_event_group(struct _ui_shell_activity *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    ...
    switch (event_id) {
        ...
        case APPS_EVENTS_BATTERY_PERCENT_CHANGE: {
            ...
            APPS_LOG_MSGID_I(LOG_TAG" [DRV]Current battery_percent=%d->%d",
    ...
    switch (event) {
        case BT_DEVICE_MANAGER_POWER_EVT_CLASSIC_STANDBY_COMPLETE: {
            if (BT_DEVICE_MANAGER_POWER_RESET_TYPE_NORMAL == status) {
                APPS_LOG_MSGID_I(LOG_TAG" smcharger BT classic power off", 0);
            }
            break;
        }
        case BT_DEVICE_MANAGER_POWER_EVT_CLASSIC_ACTIVE_COMPLETE: {
            if (BT_DEVICE_MANAGER_POWER_STATUS_SUCCESS == status) {
                APPS_LOG_MSGID_I(LOG_TAG" smcharger BT classic power on", 0);
            }
            break;
        }
...
static bool smcharger_idle_interaction_event_group(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    ...
    switch (event_id) {
        ...
        case APPS_EVENTS_INTERACTION_RHO_END: {
            APPS_LOG_MSGID_I(LOG_TAG" [new Partner] Received RHO end", 0);
            app_smcharger_context_t *smcharger_ctx = (app_smcharger_context_t *)self->local_context;
            smcharger_ctx->agent_prepare_rho_flag = FALSE;
            ...
            if (APP_RHO_RESULT_SUCCESS == rho_ret) {
                ...
                partner_notify_case_battery_level_to_agent(smcharger_ctx);
                smcharger_notify_state_to_peer(smcharger_ctx);
                APPS_LOG_MSGID_I(LOG_TAG" [new Partner] RHO success", 0);
            } else {
                APPS_LOG_MSGID_I(LOG_TAG" [new Partner] RHO fail, agent retry check", 0);
        ...
        case APPS_EVENTS_INTERACTION_PARTNER_SWITCH_TO_AGENT: {
            APPS_LOG_MSGID_I(LOG_TAG" [new Agent] Received RHO end", 0);
            app_smcharger_context_t *smcharger_ctx = (app_smcharger_context_t *)self->local_context;
            smcharger_ctx->agent_prepare_rho_flag = FALSE;
            app_rho_result_t rho_ret = (app_rho_result_t)extra_data;
            if (APP_RHO_RESULT_SUCCESS == rho_ret) {
                APPS_LOG_MSGID_I(LOG_TAG" [new Agent] RHO success", 0);
                smcharger_notify_battery_to_peer(smcharger_ctx);
                smcharger_notify_state_to_peer(smcharger_ctx);
            } else {
                APPS_LOG_MSGID_I(LOG_TAG" [new Agent] RHO fail", 0);
            }
            break;
        }
#endif
    ...
    switch (event_group) {
        ...
        case EVENT_GROUP_UI_SHELL_CHARGER_CASE: {
            ...
            if (event_id == SMCHARGER_EVENT_SYNC_STATE) {
                ...
                APPS_LOG_MSGID_I(LOG_TAG" CHARGER_IN = Charger_exist(1)", 0);
                uint8_t battery_percent = battery_management_get_battery_property(BATTERY_PROPERTY_CAPACITY);
                smcharger_ctx->shutdown_state = calculate_shutdown_state(battery_management_get_battery_property(BATTERY_PROPERTY_VOLTAGE));
                APPS_LOG_MSGID_I(LOG_TAG" [DRV]Current charger_exist=1 battery_percent=%d (%d) shutdown_state=%d",
