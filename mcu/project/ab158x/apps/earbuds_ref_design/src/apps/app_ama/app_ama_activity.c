    ...
    if (load_err != VA_MODEL_MANAGER_ERROR_CODE_OK) {
        ...
        if (load_info.model_flash_address == 0x00 || load_info.model_length == 0x00) {
            ...
            if (type == MULTI_VA_TYPE_AMA) {
                bool ull_upstreaming = app_ama_is_ull_upstreaming();
...
static void app_ama_activity_start_wwe()
{
    ...
    bool ull_upstreaming = app_ama_is_ull_upstreaming();
    bool suspended = ama_audio_is_suspended();
    APPS_LOG_MSGID_I(APP_AMA_ACTI", app_ama_activity_start_wwe, ULL upstreaming : %d, suspended : %d", 2, ull_upstreaming, suspended);
...
static bool app_ama_handle_key_event(apps_config_key_action_t key_event)
{
    ...
    bool ull_upstreaming = app_ama_is_ull_upstreaming();
    bool suspended = ama_audio_is_suspended();
    APPS_LOG_MSGID_I(APP_AMA_ACTI", app_ama_handle_key_event, ULL upstreaming : %d, suspended : %d", 2, ull_upstreaming, suspended);
...
void app_ama_forward_at_cmd(AMA_FORWARD_AT_COMMAND_IND_T *ind)
{
    ...
    printf("AMA - app_ama_forward_at_cmd, forward cmd : %s", ind->command);

    if (memcmp(ind->command, AMA_FORWARD_AT_CMD_STR_ATD, strlen(AMA_FORWARD_AT_CMD_STR_ATD)) == 0) {
        APPS_LOG_MSGID_I(APP_AMA_ACTI", app_ama_forward_at_cmd, Dial the phone number ongoing", 0);
        ...
        if ((strlen((char *)(ind->command)) - strlen(AMA_FORWARD_AT_CMD_STR_ATD) - 1) < 64) {
            memcpy(x_number, ind->command + strlen(AMA_FORWARD_AT_CMD_STR_ATD), (strlen((char *)(ind->command)) - strlen(AMA_FORWARD_AT_CMD_STR_ATD) - 1));
    ...
    if (send_status == BT_STATUS_SUCCESS) {
        APPS_LOG_MSGID_I(APP_AMA_ACTI", app_ama_forward_at_cmd, Execute HFP AT CMD succeed", 0);
...
void app_ama_handle_stop_recording()
{
    ...
    bool ull_upstreaming = app_ama_is_ull_upstreaming();
    bool suspended = ama_audio_is_suspended();
    APPS_LOG_MSGID_I(APP_AMA_ACTI", app_ama_handle_stop_recording, ULL upstreaming : %d, suspended : %d", 2, ull_upstreaming, suspended);
