...
static void app_gsound_srv_write_init_state(uint8_t init_state)
{
    ...
    uint32_t size = 1;
    nvdm_status_t status = nvdm_write_data_item(NVDM_GROUP_GSOUND, NVDM_GSOUND_ITEM_INIT_STATE,
                                                NVDM_DATA_ITEM_TYPE_RAW_DATA,
                                                (uint8_t *)&init_state, size);
    GSOUND_LOG_I(LOG_TAG" write_init_state, init_state=%d status=%d", 2, init_state, status);
#endif
}
...
static void app_gsound_srv_update_init_state(bool selected)
{
    ...
    nvdm_status_t status = nvdm_read_data_item(NVDM_GROUP_GSOUND, NVDM_GSOUND_ITEM_INIT_STATE,
                                               (uint8_t *)&init_state, &size);
    ...
    if (selected) {
        if ((status == NVDM_STATUS_ITEM_NOT_FOUND) ||
            ((status == NVDM_STATUS_OK) && (init_state == GSOUND_INITIAL_STATE_DISABLED))) {
            init_state = GSOUND_INITIAL_STATE_ENABLED;
            update_nvdm = TRUE;
        }
    } else {
        if ((status == NVDM_STATUS_OK) && (init_state == GSOUND_INITIAL_STATE_ENABLED)) {
...
static void app_gsound_srv_play_vp(uint8_t clip)
{
    ...
    GSOUND_LOG_I(LOG_TAG" play_vp, %d->%d", 2, clip, vp_index);
    if (vp_index != 0xFF) {
...
static bool app_gsound_srv_get_serial_num(uint8_t *serial_num, uint32_t max_len)
{
    ...
    if (status != NVKEY_STATUS_OK) {
        GSOUND_LOG_E(LOG_TAG" get_serial_num, fail %d", 1, status);
        return FALSE;
    }
...
static bool app_gsound_srv_get_version(char *version, uint32_t max_len)
{
    ...
    nvkey_status_t status = nvkey_data_item_length(NVID_APP_GSOUND_INFO, &nvkey_size);
    if (status != NVKEY_STATUS_OK) {
        GSOUND_LOG_E(LOG_TAG" get_version, fail %d", 1, status);
        return FALSE;
    }
    ...
    if (data != NULL && status == NVKEY_STATUS_OK) {
        sn_length = data->serial_num_length;
        ver = (uint8_t *)data;
        ver_length = ver[MODULE_SN_LENGTH_SIZE + sn_length];
        memcpy(version, &ver[MODULE_SN_LENGTH_SIZE + sn_length + VER_LENGTH_SIZE], (ver_length > max_len) ? max_len : ver_length);
        GSOUND_LOG_I(LOG_TAG" get_version, module_id=0x%08X sn_length=%d ver_length=%d",
                     3, data->module_id, sn_length, ver_length);
    }
    if (data != NULL) {
        vPortFree(data);
    }
...
{
    #if defined(AIR_BT_FAST_PAIR_ENABLE)
    *device_id = app_fast_pair_get_model_id();
    GSOUND_LOG_I(LOG_TAG" get_device_id from fastpair, module_id=0x%08X", 1, *device_id);
    return true;
    #else
    uint32_t nvkey_size = 0;
    nvkey_status_t status = nvkey_data_item_length(NVID_APP_GSOUND_INFO, &nvkey_size);
    if (status != NVKEY_STATUS_OK) {
        GSOUND_LOG_E(LOG_TAG" get_device_id, fail %d", 1, status);
        return FALSE;
    }
