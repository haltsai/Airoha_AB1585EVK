...
static void app_le_audio_bis_mute(bool in_ear)
{
    if (in_ear) {
        ...
        APPS_LOG_MSGID_I(LOG_TAG" bis_mute, unmute bt_status=0x%08X", 1, bt_status);
    } else {
        bt_status_t bt_status = bt_sink_srv_le_volume_set_mute(BT_SINK_SRV_LE_STREAM_TYPE_OUT, TRUE);
        APPS_LOG_MSGID_I(LOG_TAG" bis_mute, mute bt_status=0x%08X", 1, bt_status);
    }
}
#endif
