...
static app_mps_cccd_t app_lea_mps_cccd[APP_LE_AUDIO_MAX_LINK_NUM] = {0};
...
void app_le_audio_mps_init(void)
{
    ...
    memset(app_lea_mps_cccd, 0, sizeof(app_mps_cccd_t)*APP_LE_AUDIO_MAX_LINK_NUM);
}
...
void app_le_audio_mps_send_battery(bt_handle_t handle)
{
    ...
    if (index >= APP_LEA_MAX_CONN_NUM) {
        APPS_LOG_MSGID_E(LOG_TAG" send_battery, error handle=0x%04X", 1, handle);
        return;
    }

    if (app_lea_mps_cccd[index].battery_level_cccd == MPS_GATTS_NOTI_ENABLE) {
        ...
        APPS_LOG_MSGID_E(LOG_TAG" send_battery, NO CCCD enabled", 0);
    }
}
