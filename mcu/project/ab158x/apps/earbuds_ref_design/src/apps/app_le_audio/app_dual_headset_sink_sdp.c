...
bool app_le_audio_dhss_read_peer_le_addr(void)
{
...
#ifdef MTK_NVDM_ENABLE
    nvdm_status_t status = nvdm_read_data_item(NVDM_GROUP_DHSS, NVDM_DHSS_ITEM_PAIRED_LE_ADDR, addr, &size);
    if (NVDM_STATUS_OK != status) {
        APPS_LOG_MSGID_E(LOG_TAG" read_peer_le_addr, NVDM not exist!", 0);
        return FALSE;
    }
    ...
    APPS_LOG_MSGID_I(LOG_TAG" read_peer_le_addr, offset=%d type=%d addr=%02X:%02X:%02X:%02X:%02X",
...
bool app_le_audio_dhss_read_local_le_addr(void)
{
    ...
    APPS_LOG_MSGID_I(LOG_TAG" read_local_le_addr, offset=%d type=%d addr=%02X:%02X:%02X:%02X:%02X",
                     8, offset, app_dhss_dual_headset_pair_address[offset],
                     app_dhss_dual_headset_pair_address[offset + 1],
                     app_dhss_dual_headset_pair_address[offset + 2],
                     app_dhss_dual_headset_pair_address[offset + 3],
...
#ifdef MTK_NVDM_ENABLE
    uint32_t size = DHSS_LE_ADDR_LENGTH;
    nvdm_status_t status = nvdm_write_data_item(NVDM_GROUP_DHSS, NVDM_DHSS_ITEM_PAIRED_LE_ADDR, NVDM_DATA_ITEM_TYPE_RAW_DATA, (uint8_t *)&app_dhss_dual_headset_pair_address[offset], size);
    app_lea_dhss_read_peer_le_addr = TRUE;
