...
static app_le_audio_ccp_callback_t       app_lea_ccp_callback = NULL;
 ...
 * Static function
**************************************************************************************************/
static void app_le_audio_ccp_set_attribute_callback(void)
{
    APPS_LOG_MSGID_I(LOG_TAG" set_attribute_callback", 0);
...
static bool app_le_audio_ccp_discovery_callback(bt_gattc_discovery_event_t *event, bool is_gtbs)
{
    ...
    APPS_LOG_MSGID_I(LOG_TAG" discovery_callback, is_gtbs=%d charc_num=%d",
                     2, is_gtbs, app_lea_ccp_service.char_count_found);
    ...
    param.callback = app_le_audio_ccp_set_attribute_callback;
    ...
    if (is_gtbs) {
        app_lea_ccp_callback(event->conn_handle, (0 != app_lea_ccp_service.char_count_found));
    }
 ...
 * Public function
**************************************************************************************************/
void app_le_audio_ccp_init(app_le_audio_ccp_callback_t callback)
{
    if (NULL != app_lea_ccp_callback) {
        return;
    }

    app_lea_ccp_callback = callback;
    ...
    bt_gattc_discovery_service_register(&service_uuid, &app_lea_ccp_service, app_le_audio_ccp_discover_gtbs_callback);
    service_uuid.uuid.uuid16 = BT_SIG_UUID16_TELEPHONE_BEARER_SERVICE;
    bt_gattc_discovery_service_register(&service_uuid, &app_lea_ccp_service, app_le_audio_ccp_discover_tbs_callback);
}
