    ...
    switch (event_id) {
        ...
        case BT_CM_EVENT_VISIBILITY_STATE_UPDATE: {
            ...
            if (visible_update->visibility_state) {
                ...
                APPS_LOG_MSGID_I(LOG_TAG"[LEA] start advertising for EDR visible", 0);
#ifdef MTK_AWS_MCE_ENABLE
                app_le_audio_start_advertising(TRUE, TRUE, APP_LE_AUDIO_ADV_TIME);
#else
                                                        ...
                                                        size_t data_len)
{
    ...
    switch (event_id) {
        case LE_SINK_SRV_EVENT_REMOTE_INFO_UPDATE: {
            ...
            APPS_LOG_MSGID_I(LOG_TAG" ble_audio_state=%d Link=%d->%d SRV=%d->%d", 5,
