...
int16_t s_agent_adaptive_eq_index = 0;
int16_t s_partner_adaptive_eq_index = 0;
...
static void app_events_audio_event_adaptive_eq_notify_callback(int16_t eq_index)
{
    ...
    if (role != BT_AWS_MCE_ROLE_AGENT && role != BT_AWS_MCE_ROLE_NONE) {
        ...
        APPS_LOG_MSGID_I(LOG_TAG"Received agent adaptive eq index %d -> %d", 2, s_agent_adaptive_eq_index, eq_index);
...
static void app_adaptive_eq_audio_resource_init(void)
{
    ...
    if (s_app_aeq_context.resource_handle != NULL) {
        ...
        APPS_LOG_MSGID_I(LOG_TAG" audio_resource_init success.", 0);
    } else {
        APPS_LOG_MSGID_I(LOG_TAG" audio_resource_init fail.", 0);
    }
}
        ...
        case AUDIO_TRANSMITTER_EVENT_START_SUCCESS: {
            ...
            if (AUD_ID_INVALID != s_app_aeq_context.transmit_id
                ...
                if (AUDIO_TRANSMITTER_STATUS_SUCCESS != audio_transmitter_stop(s_app_aeq_context.transmit_id)) {
                    APPS_LOG_MSGID_I(LOG_TAG" audio_transmitter_callback: trans_id=0x%x stop fail.", 1, s_app_aeq_context.transmit_id);
                }
            }
            break;
        }
...
static aeq_control_status_t app_aeq_race_control_aeq_detect(bt_aws_mce_role_t role, uint8_t action)
{
    aeq_control_status_t status = RACE_AEQ_STATUS_SUCCESS;
...
static void app_aeq_race_control(aeq_control_param_t *param)
{
    ...
    if (role == BT_AWS_MCE_ROLE_AGENT && BT_AWS_MCE_SRV_LINK_NONE == bt_aws_mce_srv_get_link_type()) {
        APPS_LOG_MSGID_I(LOG_TAG" race_control[%02x]:  Agent not AWS Connected", 0);
    ...
    switch (type) {
        ...
        case PEQ_AEQ_DETECT_CONTROL: {
            status = app_aeq_race_control_aeq_detect(role, action);
            break;
        }
        default:
            break;
