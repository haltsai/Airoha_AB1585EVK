...
static app_battery_state_t _get_battery_state(struct _ui_shell_activity *self)
{
    ...
    if (s_battery_context.charger_exist_state) {
        ...
        } else if (s_battery_context.charging_state == CHARGER_STATE_EOC) {
            new_state = APP_BATTERY_STATE_CHARGING_EOC;
                                      ...
                                      size_t data_len)
{
    ...
    } else if (bat_state == APP_BATTERY_STATE_CHARGING_EOC
               && old_state != APP_BATTERY_STATE_CHARGING_EOC) {
        APPS_LOG_MSGID_I(LOG_TAG" start power off when received EOC charging state.", 0);
        ui_shell_send_event(FALSE, EVENT_PRIORITY_HIGNEST, EVENT_GROUP_UI_SHELL_APP_INTERACTION,
                            APPS_EVENTS_INTERACTION_REQUEST_IMMEDIATELY_POWER_OFF, NULL, 0, NULL, 0);
    }
