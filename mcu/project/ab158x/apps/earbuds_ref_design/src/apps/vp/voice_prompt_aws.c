...
static void voice_prompt_aws_if_timer_start()
{
    ...
    if (g_voice_prompt_aws_ctx.IF_timer_active) {
        ...
        VP_LOG_MSGID_I(LOG_TAG" IF timer start ret %d", 1, ret);
    }
...
static voice_prompt_status_t voice_prompt_aws_get_remote_addr(bt_bd_addr_t *addr)
{
    ...
    VP_LOG_MSGID_I(LOG_TAG" Get AWS SP addr num %d, %x:%x:%x:%x:%x:%x", 7, num, *(addr_log + 5), *(addr_log + 4),
...
voice_prompt_status_t voice_prompt_aws_exit_sniff()
{
    ...
    if (status == BT_STATUS_SUCCESS || status == BT_STATUS_PENDING) {
        ...
        VP_LOG_MSGID_I(LOG_TAG" exiting sniff mode...", 0);
        ...
        VP_LOG_MSGID_I(LOG_TAG" already exited", 0);
        /* Disable sniff mode while VP playing. */
        voice_prompt_aws_enable_sniff(false);
        return VP_STATUS_SUCCESS;
    } else {
        VP_LOG_MSGID_I(LOG_TAG" exiting sniff mode fail, 0x%x, play anyway", 1, status);
        g_voice_prompt_aws_ctx.aws_state = VOICE_PROMP_AWS_STATE_ACTIVE;
        return VP_STATUS_SUCCESS;
    }
}
...
static void voice_prompt_aws_sync_cb(bt_aws_mce_report_info_t *info)
{
    ...
    switch (sync_ctx->type) {
        case VOICE_PROMPT_SYNC_TYPE_PLAY: {        /**<  param voice_prompt_aws_sync_param_t. */
            voice_prompt_aws_sync_cb_play(sync_ctx->param);
            break;
        }
        case VOICE_PROMPT_SYNC_TYPE_STOP: {        /**<  param voice_prompt_sync_stop_t. */
            voice_prompt_aws_sync_cb_stop(sync_ctx->param);
            break;
        }
        case VOICE_PROMPT_SYNC_TYPE_LANG: {        /**<  param voice_prompt_set_lang_t. */
            voice_prompt_aws_sync_cb_lang(sync_ctx->param);
            break;
        }
        case VOICE_PROMPT_SYNC_TYPE_PLAY_PEER: {   /**<  param voice_prompt_param_t. */
            voice_prompt_aws_sync_cb_play_peer(sync_ctx->param);
            break;
        }
        case VOICE_PROMPT_SYNC_TYPE_STOP_PEER: {   /**<  param voice_prompt_stop_peer_t. */
            voice_prompt_aws_sync_cb_stop_peer(sync_ctx->param);
            break;
        }
        default: {
            VP_LOG_MSGID_E(LOG_TAG" sync_cb type wrong %d", 1, sync_ctx->type);
            break;
        }
    }
