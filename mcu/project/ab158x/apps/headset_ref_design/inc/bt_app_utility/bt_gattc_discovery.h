...
#define BT_GATTC_DISCOVERY_MAX_SRVS_INSTANCE (10)  /**< Maximum number of service instance supported by this module. */
...
/**
 *  @brief Structure for holding multiple instances service information found during
 *         the discovery process.
 */
typedef struct {
    uint8_t instance_count;                                             /**< Number of service instances found. */
    uint8_t instance_processed;                                         /**< Number of service instances processed. */
    uint16_t start_handle[BT_GATTC_DISCOVERY_MAX_SRVS_INSTANCE];        /**< List of service handle range start. */
    uint16_t end_handle[BT_GATTC_DISCOVERY_MAX_SRVS_INSTANCE];          /**< List of service handle range end. */
} bt_gattc_discovery_service_instance_t;
...
typedef struct {
    ...
    uint16_t                    discovery_start_handle;       /**< Discovery Service Handle Range start. */
    uint16_t                    discovery_end_handle;         /**< Discovery Service Handle Range end. */
    ...
    bool                        is_included_definition;       /**< Discovery included service. */
    bt_gattc_discovery_service_instance_t *service_instance;  /**< Pointer for multiple instances service discovery.*/
...
typedef struct {
    ...
    bool                             last_instance;    /**< Variable to indicate whether this discovery result is the last instance or not. */
...
/**
 * @brief   Function for responding discovery event has been received.
 */
void bt_gattc_discovery_event_response(void);
#endif

/**
 * @brief   Function for getting the current connection handle of discovery.
 * @param[out]  conn_handle The handle of the connection for which the discovery.
 * @return     #BT_GATTC_DISCOVERY_STATUS_SUCCESS         Operation success.
 *             #BT_GATTC_DISCOVERY_STATUS_FAIL            Operation fail.
 */
