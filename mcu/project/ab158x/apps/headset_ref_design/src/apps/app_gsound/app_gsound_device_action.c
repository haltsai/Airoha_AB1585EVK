...
static void app_gsound_device_action_turn_on_anc(void)
{
    ...
    bool ret =  app_anc_service_enable(target_anc_filter_id,
                                       target_anc_type,
                                       target_anc_runtime_gain,
                                       NULL);
    GSOUND_LOG_I(LOG_TAG" turn_on_anc, ret=%d", 1, ret);
}

static void app_gsound_device_action_turn_off_anc_pt(void)
{
    bool ret = app_anc_service_disable();
    GSOUND_LOG_I(LOG_TAG" turn_off_anc_pt, ret=%d", 1, ret);
}
...
static void app_gsound_device_action_turn_on_pt(void)
{
    bool ret = app_anc_service_enable(AUDIO_ANC_CONTROL_PASS_THRU_FILTER_DEFAULT,
                                      ...
                                      NULL);
    GSOUND_LOG_I(LOG_TAG" turn_on_pt, ret=%d", 1, ret);
}
#endif
...
static int32_t app_gsound_device_action_get_anc_level(void)
{
    ...
    GSOUND_LOG_I(LOG_TAG" get_anc_level, level=%d", 1, level);
    return level;
#else
    return 0;
#endif
...
{
#ifdef MTK_ANC_ENABLE
    int32_t level = app_gsound_device_action_get_anc_pt_level(AUDIO_ANC_CONTROL_TYPE_PASSTHRU_FF);
    GSOUND_LOG_I(LOG_TAG" get_pt_level, level=%d", 1, level);
    return level;
#else
    return 0;
#endif
...
static void app_gsound_device_action_decode_check(const uint8_t *device_actions, int device_actions_size)
{
    ...
    GSOUND_LOG_I(LOG_TAG" decode_check, device_actions_size=%d", 1, device_actions_size);
    ...
    GSOUND_LOG_I(LOG_TAG" decode_check, bat[0] %d %d %d", 3, bat[0].remaining_battery_percentage, bat[0].connected, bat[0].device_type);
    GSOUND_LOG_I(LOG_TAG" decode_check, bat[1] %d %d %d", 3, bat[1].remaining_battery_percentage, bat[1].connected, bat[1].device_type);
    GSOUND_LOG_I(LOG_TAG" decode_check, bat[2] %d %d %d", 3, bat[2].remaining_battery_percentage, bat[2].connected, bat[2].device_type);
    GSOUND_LOG_I(LOG_TAG" decode_check, noise=%d ambient=%d touch=%d", 3, state_key_all.noise_cancellation_level,
                 state_key_all.ambient_mode_level, state_key_all.touch_control_enabled);
}
*/
...
static bool app_gsound_device_action_is_touch_control_enabled(void)
{
    ...
    */
    ...
    GSOUND_LOG_I(LOG_TAG" is_touch_control_enabled, status=%d", 1, status);
    return (status == 0);
}
#endif
...
static uint8_t *app_gsound_device_action_pb_encode(uint32_t *buf_len)
{
    ...
    if (!nano_pb_encode(&stream_dummy, CurrentDeviceState_fields, &state_key)) {
        GSOUND_LOG_I(LOG_TAG" pb_encode, error", 0);
        return NULL;
    }
    *buf_len = stream_dummy.bytes_written;
    ...
    if (!nano_pb_encode(&stream, CurrentDeviceState_fields, &state_key)) {
        GSOUND_LOG_TEXT(LOG_TAG" pb_encode, error=%s", PB_GET_ERROR(&stream));
        vPortFree(buf);
        return NULL;
    }

    GSOUND_LOG_I(LOG_TAG" pb_encode, success buf_len=%d", 1, *buf_len);
...
static bool app_gsound_device_action_decode_param(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
    ...
    if (!nano_pb_decode(stream, Execution_ParamsEntry_fields, param)) {
        GSOUND_LOG_TEXT(LOG_TAG" decode_param error=%s", PB_GET_ERROR(stream));
        return FALSE;
    }
    GSOUND_LOG_I(LOG_TAG" decode_param, success kind=%d", 1, param->value.which_kind);
    return TRUE;
}
...
static void app_gsound_device_action_turn_off(bool on_off)
{
    if (!on_off) {
        ...
        GSOUND_LOG_I(LOG_TAG" power_off, send aws status=0x%08X", 1, bt_status);
#endif
        ...
        *need_rho = FALSE;
        ...
        if (status != UI_SHELL_STATUS_OK) {
            vPortFree(need_rho);
            GSOUND_LOG_E(LOG_TAG" power_off, send ui_shell event fail", 0);
        }
    }
}
static void app_gsound_device_action_touch_control(bool on_off)
{
    ...
    */
    uint8_t status = apps_set_touch_control_enable(on_off, TRUE);
    GSOUND_LOG_I(LOG_TAG" touch_control, on_off=%d status=%d", 2, on_off, status);
...
static bool app_gsound_device_action_command_handler(char *command, Execution_ParamsEntry *param)
{
    ...
    if (strcmp(command, "action.devices.commands.headphone.UpdateNoiseCancellation") == 0) {
        ...
        GSOUND_LOG_I(LOG_TAG" action NoiseCancellation, %2X,%2X,%2X,%2X,%2X,%2X,%2X,%2X", 8,
        ...
        uint8_t *ptr = (uint8_t *) & (param->value.kind.number_value);
        GSOUND_LOG_I(LOG_TAG" action AmbientMode, %2X,%2X,%2X,%2X,%2X,%2X,%2X,%2X", 8,
                     ptr[0], ptr[1], ptr[2], ptr[3], ptr[4], ptr[5], ptr[6], ptr[7]);
        ...
        GSOUND_LOG_I(LOG_TAG" action OnOff, %d", 1, param->value.kind.bool_value);
        ...
        GSOUND_LOG_I(LOG_TAG" action TouchControl, %d", 1, param->value.kind.bool_value);
...
bool app_gsound_device_action_consume(const uint8_t *const action_state_key)
{
    if (strcmp((const char *)action_state_key, DEVICE_ACTION_STATE_KEY_ALL) == 0) {
        GSOUND_LOG_I(LOG_TAG" app_gsound_device_action_consume, key_all=0x%8X",
                     1, gsound_device_action_state_key.key_all);
        if (gsound_device_action_state_key.key_all != NULL) {
...
bool app_gsound_device_action_do_action(const uint8_t *device_actions, int device_actions_size)
{
    ...
    if (!nano_pb_decode(&stream, Execution_fields, &execution)) {
        GSOUND_LOG_TEXT(LOG_TAG" do_action, decode error=%s, device_actions_size=%d",
                        PB_GET_ERROR(&stream), device_actions_size);
        return FALSE;
    }

    GSOUND_LOG_I(LOG_TAG" do_action, decode OK size=%d", 1, device_actions_size);
