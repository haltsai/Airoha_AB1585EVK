...
static app_le_audio_aird_client_info_t          app_lea_aird_client_info[APP_LE_AUDIO_MAX_LINK_NUM] = {0};
static app_le_audio_aird_client_callback_t      app_lea_aird_client_callback = NULL;

static app_le_audio_aird_block_stream_t         app_lea_aird_client_block_stream = APP_LE_AUDIO_AIRD_BLOCK_STREAM_NONE;
...
static void app_le_audio_aird_client_proc_bt_group(uint32_t event_id, void *extra_data, size_t data_len)
{
    switch (event_id) {
        case BT_POWER_ON_CNF: {
            app_lea_aird_client_block_stream = APP_LE_AUDIO_AIRD_BLOCK_STREAM_NONE;
            break;
        }
    }
}
...
static bool app_le_audio_aird_client_start_pre_action(bt_handle_t handle, app_le_audio_aird_client_info_t *info)
{
    ...
    APPS_LOG_MSGID_I(LOG_TAG" start_pre_action, start handle=0x%04X state=%d", 2, handle, info->state);
    ...
    switch (info->state) {
        case APP_LE_AUDIO_AIRD_CLIENT_STATE_SRV_DISCOVERY_COMPLETE: {
            if (BT_HANDLE_INVALID != info->att_handle_tx_cccd) {
                ...
                APPS_LOG_MSGID_E(LOG_TAG" start_pre_action, state=SRV_DISCOVERY_COMPLETE invalid att_handle_tx_cccd", 0);
            }
        }
        /*Pass through*/
        case APP_LE_AUDIO_AIRD_CLIENT_STATE_SET_CCCD_TX: {
            APPS_LOG_MSGID_I(LOG_TAG" start_pre_action, state=SET_CCCD_TX block_stream=%d", 1, app_lea_aird_client_block_stream);
            info->state = APP_LE_AUDIO_AIRD_CLIENT_STATE_READY;
            if (APP_LE_AUDIO_AIRD_BLOCK_STREAM_NONE != app_lea_aird_client_block_stream) {
                app_le_audio_aird_action_block_stream_t param = {0};
                param.type = app_lea_aird_client_block_stream;
...
static void app_le_audio_aird_client_discovery_callback(bt_gattc_discovery_event_t *event)
{
    ...
    if (event == NULL) {
        ...
        return;
    } else if ((BT_GATTC_DISCOVERY_EVENT_FAIL == event->event_type && !event->last_instance)
...
void app_le_audio_aird_client_init(void)
{
    ...
    bt_gattc_discovery_service_register(&aird_srv_uuid, &app_lea_aird_discovery, app_le_audio_aird_client_discovery_callback);
}
...
{
    if (suspend) {
        app_lea_aird_client_block_stream = APP_LE_AUDIO_AIRD_BLOCK_STREAM_UNIDIRECTIONAL;
    } else {
        app_lea_aird_client_block_stream = APP_LE_AUDIO_AIRD_BLOCK_STREAM_NONE;
    }
...
void app_le_audio_aird_client_notify_block_stream(app_le_audio_aird_block_stream_t type)
{
    ...
    app_le_audio_aird_action_block_stream_t param = {0};
    app_lea_aird_client_block_stream = type;
    param.type = type;
...
bool app_le_audio_aird_client_is_support_hid_call(bt_handle_t handle)
{
    ...
    bool ret = (info != NULL && info->mode == APP_LE_AUDIO_AIRD_MODE_SUPPORT_HID_CALL);
    if (!ret) {
        if (info == NULL) {
            APPS_LOG_MSGID_I(LOG_TAG" is_support_hid_call, NO handle=0x%04X->NULL info", 1, handle);
        } else {
            APPS_LOG_MSGID_I(LOG_TAG" is_support_hid_call, NO state=%d mode=%d", 2, info->state, info->mode);
        }
    }
    return ret;
}
                                          ...
                                          void *param, uint32_t param_len)
{
    ...
    if (info == NULL) {
        ...
        goto exit;
    } else if (info->mode != APP_LE_AUDIO_AIRD_MODE_SUPPORT_HID_CALL) {
        APPS_LOG_MSGID_E(LOG_TAG" send_action, not support HID_CALL", 0);
        goto exit;
    }
    ...
    if (BT_STATUS_SUCCESS == bt_status) {
        ...
        APPS_LOG_MSGID_E(LOG_TAG" send_action, bt_status=0x%08X", 1, bt_status);
    }
