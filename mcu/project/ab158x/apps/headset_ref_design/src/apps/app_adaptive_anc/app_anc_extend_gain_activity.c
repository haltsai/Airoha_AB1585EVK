...
{
    APPS_LOG_MSGID_I(LOG_TAG" get_extend_gain_streaming_status: %d", 1, s_app_anc_gain_ctx.is_streaming_started);
    return s_app_anc_gain_ctx.is_streaming_started;
}

void app_anc_extend_gaing_control_streaming(bool start_streaming)
{
    APPS_LOG_MSGID_I(LOG_TAG" control_extend_gain_streaming: start_streaming=%d, curr_state=%d",
                     2, start_streaming, s_app_anc_gain_ctx.is_streaming_started);
...
{
    APPS_LOG_MSGID_I(LOG_TAG" notify_result, notify_result=%d", 1, notify_result);
...
#ifdef MTK_NVDM_ENABLE
    uint32_t size = sizeof(app_anc_gain_control_t);
    nvkey_status_t status = nvkey_write_data(NVID_APP_ANC_GAIN_CONTROL,
...
{
    bool ret = FALSE;
    APPS_LOG_MSGID_I(LOG_TAG" gain_control_wn, [%02X] action=%d", 2, role, action);
    ...
    switch (action) {
        ...
        case RACE_ANC_GAIN_CONTROL_ENABLE : {
            ...
            if (wn_enable == s_app_anc_gain_control.wn_enable) {
                APPS_LOG_MSGID_I(LOG_TAG" gain_control_wn, WIND_NOISE, already on state %d", 1, wn_enable);
                ret = TRUE;
                break;
            }
            APPS_LOG_MSGID_I(LOG_TAG" gain_control_wn,  WIND_NOISE, enable? %d, uu %d",
                             2, wn_enable, s_app_anc_gain_control.uu_enable);
        ...
        case RACE_ANC_GAIN_CONTROL_RESUME: {
            ...
            if (wn_suspend == s_app_anc_gain_control.wn_suspend) {
                APPS_LOG_MSGID_I(LOG_TAG" gain_control_wn, [%02X] WIND_NOISE, already on suspend state %d", 2, role, wn_suspend);
                ret = TRUE;
                break;
            }
    ...
    APPS_LOG_MSGID_I(LOG_TAG" gain_control_wn, [%02X] ret=%d", 1, ret);
...
{
    bool ret = FALSE;
    APPS_LOG_MSGID_I(LOG_TAG" gain_control_uu, [%02X] action=%d", 2, role, action);
    ...
    switch (action) {
        ...
        case RACE_ANC_GAIN_CONTROL_ENABLE: {
            ...
            if (uu_enable == s_app_anc_gain_control.uu_enable) {
                APPS_LOG_MSGID_I(LOG_TAG" gain_control_uu, USER_UNAWARE already on state %d", 1, uu_enable);
                ret = TRUE;
                break;
            }
        ...
        case RACE_ANC_GAIN_CONTROL_RESUME: {
            ...
            if (uu_suspend == s_app_anc_gain_control.uu_suspend) {
                APPS_LOG_MSGID_I(LOG_TAG" gain_control_uu, USER_UNAWARE already suspend state %d", 1, uu_suspend);
                ret = TRUE;
                break;
            }
    ...
    APPS_LOG_MSGID_I(LOG_TAG" gain_control_uu, ret=%d", 1, ret);
    app_anc_extend_gain_save_control_status();
...
{
    bool ret = FALSE;
    APPS_LOG_MSGID_I(LOG_TAG" gain_control_ed, [%02X] action=%d", 2, role, action);
    ...
    switch (action) {
        ...
        case RACE_ANC_GAIN_CONTROL_ENABLE: {
            ...
            if (ed_enable == s_app_anc_gain_control.ed_enable) {
                APPS_LOG_MSGID_I(LOG_TAG" gain_control_ed, ENVIRONMENT_DETECTION already on state %d", 1, ed_enable);
                ret = TRUE;
                break;
            }
        ...
        case RACE_ANC_GAIN_CONTROL_RESUME: {
            ...
            if (suspend != s_app_anc_gain_control.ed_suspend) {
                APPS_LOG_MSGID_I(LOG_TAG" gain_control_ed: ENVIRONMENT_DETECTION, already suspend state %d", 1, s_app_anc_gain_control.ed_suspend);
                ret = TRUE;
                break;
            }
            if (suspend) {
    ...
    APPS_LOG_MSGID_I(LOG_TAG" gain_control_ed, ret=%d", 1, ret);
    app_anc_extend_gain_save_control_status();
    return ret;
}
#endif
...
static void app_anc_extend_gain_control(anc_gain_control_param_t *param)
{
    ...
    if (!s_app_anc_gain_ctx.is_streaming_started
        ...
        if ((s_app_anc_gain_control.wn_enable && !s_app_anc_gain_control.wn_suspend)
            ...
            APPS_LOG_MSGID_E(LOG_TAG" gain_control, start streaming", 0);
        ...
        if ((!s_app_anc_gain_control.wn_enable && !s_app_anc_gain_control.uu_enable && !s_app_anc_gain_control.ed_enable)
            ...
            APPS_LOG_MSGID_E(LOG_TAG" gain_control, stop streaming", 0);
                                          ...
                                          audio_anc_control_gain_t gain_2, uint32_t target_gpt)
{
    ...
    APPS_LOG_MSGID_I(LOG_TAG" set_local, suspend status: uu_suspend=%d, wn_suspend=%d, ed_suspend=%d",
                     3, s_app_anc_gain_control.uu_suspend, s_app_anc_gain_control.wn_suspend, s_app_anc_gain_control.ed_suspend);
...
static bool app_anc_extend_gain_proc_audio_anc_group(ui_shell_activity_t *self, uint32_t event_id, void *extra_data, size_t data_len)
{
    ...
    if (event_id == AUDIO_ANC_CONTROL_EVENT_OFF) {
        ...
        APPS_LOG_MSGID_I(LOG_TAG" ANC ON : type=%d, filter_id=%d", 2, anc_result->cur_type, anc_result->cur_filter_id);
...
static atci_status_t app_anc_gain_atci_handler(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_EXECUTION: {
            ...
            } else if (strstr(cmd, "set") > 0) {
//                uint8_t type   = 0;
//                uint8_t action = 0;
                anc_gain_control_param_t extend_gain_control = {0};
                sscanf(cmd, "set,%d,%d", &extend_gain_control.type, &extend_gain_control.action);
                app_anc_extend_gain_control(&extend_gain_control);
            } else {
