...
ATTR_TEXT_IN_SYSRAM void sfc_isr(hal_nvic_irq_t irq_number)
{
    uint32_t mac_ctl = 0;
    uint32_t count;
    uint32_t mask;
...
hal_flash_status_t hal_flash_write(uint32_t address, const uint8_t *data, uint32_t length)
{
    ...
    if (hal_core_status_read(HAL_CORE_MCU) == HAL_CORE_ACTIVE) {
        ...
        log_hal_msgid_info("[hal_flash_write] address=0x%x, length = %d ,w_time = %d result ok!\r\n", 3, (address & (~HAL_FLASH_BASE_ADDRESS)), length, write_time);
    }
    return HAL_FLASH_STATUS_OK;
}
