...
void gpt_nvic_register(void)
{
    ...
    if (is_init == false) {
        hal_nvic_disable_irq(GPT_IRQ_NUM);
        hal_nvic_register_isr_handler(GPT_IRQ_NUM, gpt_interrupt_handler);
        hal_nvic_enable_irq(GPT_IRQ_NUM);
        is_init = true;
...
hal_gpt_status_t sw_gpt_start_timer_for_absolute_time(uint32_t handle, hal_sw_gpt_absolute_parameter_t *absolute_parameter, sw_gpt_type_t sw_type)
{
    ...
    uint32_t next_index, next_count;
    hal_gpt_port_t gpt_port = HAL_GPT_MAX;
    ...
    /* means the next timer's expire count  is less than the running timer's expire count
    so should re-start the next timer */
    if ((next_index != g_sw_gpt_running_note[sw_type])&&(gpt_port != HAL_GPT_MAX)) {
        g_sw_gpt_running_note[sw_type] = next_index;
        clear_gpt_and_restart_hw_gpt(next_count, gpt_port); // restart the gpt.
    }
    hal_nvic_restore_interrupt_mask(mask);
