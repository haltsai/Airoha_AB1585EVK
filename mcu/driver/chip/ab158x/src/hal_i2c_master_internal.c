...
#define     I3C_SOURCE_CLOCK_PERIOD    19.2//9.17 for 109Mhz, 19.2 for 52M
    ...
    log_hal_i2c_info("[hal][i2c] config hs timing %d = %d/%d\r\n", 3, ext_tm, tm_max, clk_unit);
    ...
    switch (op_code) {
            ...
            if (s_i2c_sleep_handle[i2c_port] != SLEEP_LOCK_INVALID_ID) {
                if (args != 0 && sleep_st == false) {
                    ...
                    if (status == HAL_SLEEP_MANAGER_OK) {
                        //log_hal_i2c_info("[hal][i2c%d] lock sleep succ %d\r\n", 2, i2c_port,hal_sleep_manager_is_sleep_locked());
                        sleep_st = true;
                    } else {
                        //log_hal_i2c_info("[hal][i2c%d] lock sleep fail %d\r\n", 2, i2c_port,hal_sleep_manager_is_sleep_locked());
                    }
                } else if (args == 0 && sleep_st == true) {
                    status = hal_sleep_manager_unlock_sleep(s_i2c_sleep_handle[i2c_port]);
                    if (status == HAL_SLEEP_MANAGER_OK) {
                        //log_hal_i2c_info("[hal][i2c%d] unlock sleep succ %d\r\n", 2, i2c_port,hal_sleep_manager_is_sleep_locked());
                        sleep_st = false;
                    } else {
                        //log_hal_i2c_info("[hal][i2c%d] unlock sleep fail %d\r\n", 2, i2c_port,hal_sleep_manager_is_sleep_locked());
                    }
                }
            }
        } break;
        ...
        case I2C_IOCTRL_ENABLE_CLOCK: {
            if (args) {
                ...
                //log_hal_i2c_info("[hal][i2c%d] clock enable", 1, i2c_port);
            } else {
                hal_clock_disable(s_i2c_clk_pdn_table[i2c_port]);
                //log_hal_i2c_info("[hal][i2c%d] clock disable", 1, i2c_port);
            }
        }
        break;
#endif
...
void    i2c_master_dump_register_infra(hal_i2c_port_t  i2c_port)
{
    ...
    log_hal_i2c_warning("GLB->STA:%x, CPU0_CFG:%x, BUSY:%x, INTR:%x, CLK_CFG:%x\r\n", 5,
