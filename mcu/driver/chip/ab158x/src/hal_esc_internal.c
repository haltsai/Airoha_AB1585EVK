...
ATTR_RWDATA_IN_TCM ESC_STATIC uint32_t g_backup_esc_reg[ESC_REG_MAX_IDX];
...
ATTR_TEXT_IN_TCM ESC_STATIC void ESC_MacEnable(void);
ATTR_TEXT_IN_TCM ESC_STATIC void ESC_MacLeave(void);
ATTR_TEXT_IN_TCM ESC_STATIC void ESC_MacTrigger(void);
ATTR_TEXT_IN_TCM ESC_STATIC void ESC_MacWaitReady(void);
ATTR_TEXT_IN_TCM ESC_STATIC void esc_isr(hal_nvic_irq_t irq);
ATTR_TEXT_IN_TCM ESC_STATIC void ESC_Dev_Command(const uint16_t CS, const uint32_t cmd);
ATTR_TEXT_IN_TCM ESC_STATIC void ESC_Dev_Command_Ext(
    const uint16_t CS,
    const uint8_t *cmd,
    uint8_t *data,
    const uint16_t outl,
...
/*!
  ...
  @remarks This function shall only be invoked in bootloader.
           Other callers must be aware of interrupts during the MAC mode
*/
ATTR_TEXT_IN_TCM ESC_STATIC void ESC_Dev_Command_Ext(const uint16_t CS, const uint8_t *cmd, uint8_t *data, const uint16_t outl, const uint16_t inl)
{
    uint32_t tmp;
    uint32_t i, j;
...
ATTR_TEXT_IN_TCM ESC_STATIC void esc_device_reset(void)
{
#if defined(MICRO_CHIP_23A1024)
...
ATTR_TEXT_IN_TCM ESC_STATIC bool esc_is_4_byte_mode(void);

ATTR_TEXT_IN_TCM ESC_STATIC void flash_write_en(void);

ATTR_TEXT_IN_TCM ESC_STATIC bool write_qe_bit(uint8_t bit_value);

ATTR_TEXT_IN_TCM ESC_STATIC bool device_is_busy(uint8_t status);

ATTR_TEXT_IN_TCM ESC_STATIC uint8_t read_device_reg(esc_device_reg_type_t type);

ATTR_TEXT_IN_TCM ESC_STATIC hal_esc_flash_status_t esc_flash_parameter_check(uint32_t addr, void *ptr, uint32_t len);

ATTR_TEXT_IN_TCM ESC_STATIC void ESC_Write_C1A3(uint8_t cmd, uint32_t address, const uint8_t *buff, uint32_t length);

ATTR_TEXT_IN_TCM ESC_STATIC void ESC_Write_C1A4(uint8_t cmd, uint32_t address, const uint8_t *buff, uint32_t length);
...
ATTR_TEXT_IN_TCM ESC_STATIC void ESC_GPRAM_Write(uint32_t gpram_offset, const uint8_t *buff, int32_t length)
{
    uint32_t i;
...
ATTR_TEXT_IN_TCM void esc_internal_init(void)
{
    ...
    } else (g_esc_ctl.clock == ESC_CLOCK_52MHz) {
    ...
    } else (g_esc_ctl.clock == ESC_CLOCK_104MHz) {
...
ATTR_TEXT_IN_TCM ESC_STATIC int32_t esc_check_ready_and_resume(void)
{
