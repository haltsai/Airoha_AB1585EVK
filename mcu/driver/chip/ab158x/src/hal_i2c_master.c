...
static i2c_transfer_config_t        s_i2c0_q_buf[HAL_I2C_MASTER_0_FIFO_DEPTH] = {0};
static i2c_transfer_config_t        s_i2c1_q_buf[HAL_I2C_MASTER_1_FIFO_DEPTH] = {0};
static i2c_transfer_config_t        s_i2c2_q_buf[HAL_I2C_MASTER_2_FIFO_DEPTH] = {0};
static i2c_transfer_config_t       *s_i2c_q_buf[HAL_I2C_MASTER_MAX] = {s_i2c0_q_buf, s_i2c1_q_buf, s_i2c2_q_buf, (i2c_transfer_config_t *) NULL};
static const uint8_t                s_i2c_q_szb[HAL_I2C_MASTER_MAX] = {HAL_I2C_MASTER_0_FIFO_DEPTH, HAL_I2C_MASTER_1_FIFO_DEPTH, HAL_I2C_MASTER_2_FIFO_DEPTH, 0};
...
static void                 _hal_i2c_master_gpio_timing(hal_i2c_port_t i2c_port, bool is_i2c_mode);
...
static  void    hal_i2c_backup_all_register()
{
    ...
    for (i2c_port = 0; i2c_port < HAL_I2C_MASTER_AO; i2c_port++) {
...
hal_i2c_status_t    hal_i2c_master_init(hal_i2c_port_t i2c_port, hal_i2c_config_t *i2c_config)
{
    ...
    _hal_i2c_master_gpio_timing(i2c_port, true);

#ifdef HAL_SLEEP_MANAGER_ENABLED
    sleep_management_register_all_secure_suspend_callback(SLEEP_BACKUP_RESTORE_I2C0, (sleep_management_suspend_callback_t)hal_i2c_backup_all_register, NULL);
    sleep_management_register_all_secure_resume_callback(SLEEP_BACKUP_RESTORE_I2C0, (sleep_management_resume_callback_t) hal_i2c_restore_all_register, NULL);
#endif
...
hal_i2c_status_t    hal_i2c_master_deinit(hal_i2c_port_t i2c_port)
{
    ...
    _hal_i2c_master_gpio_timing(i2c_port, false);
...
***********************************************************************************************/
ATTR_TEXT_IN_TCM hal_i2c_status_t    hal_i2c_master_send_to_receive_polling(hal_i2c_port_t i2c_port, hal_i2c_send_to_receive_config_t *i2c_send_to_receive_config)
{
   ...
   // i2c_op_ioctl(i2c_port, I2C_IOCTRL_LOCK_SLEEP, false); /* unlock sleep */
    if (status != I2C_IRQ_STAT_SUCCESS) {
        log_hal_i2c_warning("[hal][i2c%d][poll]: error %d, intr stat 0x%x \r\n", 3, i2c_port, status, temp);
        hal_i2c_master_dump(i2c_port);          //step 4
        return HAL_I2C_STATUS_ERROR;
    } else {
        return HAL_I2C_STATUS_OK;
    }
...
hal_i2c_status_t hal_i2c_master_send_to_receive_dma_ex_none_blocking(hal_i2c_port_t i2c_port, hal_i2c_send_to_receive_config_ex_no_busy_t *config)
{
    ...
    if (temp == I2C_HW_STATE_USED_NORMAL) {
        hal_nvic_restore_interrupt_mask(save_mask);
        log_hal_i2c_error("[hal][i2c%d] dma nb send recv fail: dev busy(%d)", 2, i2c_port, temp);
        return HAL_I2C_STATUS_ERROR;
    }
...
ATTR_TEXT_IN_TCM static hal_i2c_status_t    _hal_i2c_config_transfer_poll(hal_i2c_port_t i2c_port, i2c_transfer_config_t *config)
{
    i2c_private_info_t     *priv_info = NULL;
...
ATTR_TEXT_IN_TCM static hal_i2c_status_t    _hal_i2c_config_transfer(hal_i2c_port_t i2c_port, i2c_transfer_config_t *config)
{
    i2c_private_info_t     *priv_info = NULL;
...
static void    _hal_i2c_interrupt_handle(hal_nvic_irq_t irq_number)
{
    ...
    if (q_stat < 0) {
        log_hal_i2c_error("[hal][i2c%d] irq handle err: pop q fail(%d), irq_sta(%x)", 2, i2c_port, q_stat, irq_status);
        return;
    }
    ...
    /* call user callback */
    //log_hal_i2c_info("[hal][i2c%d] i2c irq triggered ", 1, i2c_port);
...
static hal_i2c_status_t _hal_i2c_master_send_and_receive_dma_with_nb(hal_i2c_port_t i2c_port, hal_i2c_send_to_receive_config_ex_no_busy_t *config)
{
    ...
    if (pconfig == NULL) {
        ...
        log_hal_i2c_error("[hal][i2c%d] dma send recv err 0: none item avail", 0);
        return HAL_I2C_STATUS_ERROR;
    }
    ...
    hal_nvic_restore_interrupt_mask(save_mask); /* enable irq */
    if (temp < 0) {
        log_hal_i2c_error("[hal][i2c%d] dma send recv err: q push fail(%d)", 1, (int) temp);
        return HAL_I2C_STATUS_ERROR;
    }
    hal_nvic_save_and_set_interrupt_mask(&save_mask); /* disable irq */
    ...
    /* step 5: start */
    hal_nvic_save_and_set_interrupt_mask(&save_mask); /* disable irq */
    if (i2c_op_ioctl(i2c_port, I2C_IOCTRL_GET_BUSY_STAT, 0) == 0 && (priv_info->hw_state & I2C_HW_STATE_DEV_BUSY_MASK) == 0) {
        if (queue_top(&priv_info->i2c_queue, (int *) &temp) >= 0) {
            i2c_op_ioctl(i2c_port, I2C_IOCTRL_LOCK_SLEEP,   true);   /* lock sleep */
            pconfig = (i2c_transfer_config_t *)temp;
...
{
#if !(defined(__EXT_BOOTLOADER__) || defined(__EXT_DA__))
    uint8_t  s_i2c_gpio[HAL_I2C_MASTER_MAX][2] = {
        { HAL_I2C_MASTER_0_SCL_PIN, HAL_I2C_MASTER_0_SDA_PIN },
        { HAL_I2C_MASTER_1_SCL_PIN, HAL_I2C_MASTER_0_SDA_PIN },
        { HAL_I2C_MASTER_2_SCL_PIN, HAL_I2C_MASTER_0_SDA_PIN },
        {                     0xFF,                      0xFF},
    };
    ...
    if (is_i2c_mode) {
        hal_gpio_set_capacitance(s_i2c_gpio[i2c_port][0], HAL_GPIO_CAPACITANCE_3);
        hal_gpio_set_capacitance(s_i2c_gpio[i2c_port][1], HAL_GPIO_CAPACITANCE_3);
#ifdef HAL_GPIO_FEATURE_SET_SLEW_RATE
        hal_gpio_set_slew_rate(s_i2c_gpio[i2c_port][0]);
        hal_gpio_set_slew_rate(s_i2c_gpio[i2c_port][0]);
#endif
    } else {
        hal_gpio_clear_capacitance(s_i2c_gpio[i2c_port][0]);
        hal_gpio_clear_capacitance(s_i2c_gpio[i2c_port][1]);
#ifdef HAL_GPIO_FEATURE_SET_SLEW_RATE
        hal_gpio_clear_slew_rate(s_i2c_gpio[i2c_port][0]);
        hal_gpio_clear_slew_rate(s_i2c_gpio[i2c_port][0]);
#endif
    }
#endif /* HAL_GPIO_FEATURE_SET_CAPACITANCE */
#endif /* BL & DA */
}
