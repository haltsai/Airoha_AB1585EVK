...
hal_pinmux_status_t hal_pinmux_set_function(hal_gpio_pin_t gpio_pin, uint8_t function_index)
{
    ...
    if (shift != 0xFF) {
        GPIO_REG32(gpio_cfg_table[gpio_pin].g_cfg_reg + GPIO_SET_ADDR) = 1 << shift;
    }
#if 0
...
hal_gpio_status_t hal_gpio_set_rsel_register(hal_gpio_pin_t gpio_pin, uint8_t gpio_pu, uint8_t gpio_pd, uint8_t gpio_rsel0, uint8_t gpio_rsel1)
{
    ...
    if (gpio_cfg_table[gpio_pin].rsel_shift != 0xff) {
        GPIO_REG32(gpio_cfg_table[gpio_pin].pu_reg + shift_addr[gpio_pu ? 1 : 0]) = pupd_shift;
        GPIO_REG32(gpio_cfg_table[gpio_pin].pd_reg + shift_addr[gpio_pd ? 1 : 0]) = pupd_shift;
        GPIO_REG32(gpio_cfg_table[gpio_pin].rsel_reg + shift_addr[gpio_rsel0 ? 1 : 0]) = rsel_shift;
        GPIO_REG32(gpio_cfg_table[gpio_pin].rsel_reg + shift_addr[gpio_rsel1 ? 1 : 0]) = rsel_shift << 1;
    } else {
        hal_nvic_restore_interrupt_mask(mask);
        return HAL_GPIO_STATUS_ERROR_PIN;
    }
...
hal_gpio_status_t hal_gpio_set_capacitance(hal_gpio_pin_t gpio_pin, hal_gpio_capacitance_t cap)
{
    ...
    shift = gpio_cfg_table[gpio_pin].eh_shift;
    if (shift != 0xFF) {
        GPIO_REG32(gpio_cfg_table[gpio_pin].eh_reg + GPIO_CLR_ADDR) = GPIO_REG_THREE_BIT_SET_CLR << shift;
        GPIO_REG32(gpio_cfg_table[gpio_pin].eh_reg + GPIO_SET_ADDR) = ((cap << 1) | 0x1) << shift;
    } else {
        hal_nvic_restore_interrupt_mask(mask);
        return HAL_GPIO_STATUS_ERROR_PIN;
    }
