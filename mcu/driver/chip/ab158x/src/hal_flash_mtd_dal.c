...
*/
volatile uint32_t g_flash_pingpang_flag = 523;
uint8_t org_data;
...
ATTR_TEXT_IN_RAM int SF_DAL_OTP_ERASE_WINBOND(void *MTDData, uint32_t block)
{
    ...
    if (D->AddressCycle == 4) {
        SFC_GPRAM->RW_SF_GPRAM_DATA = (addr2 << SFC_GENERIC_8_BIT_OFFSET) | cmd;
        SFC_GPRAM->RW_SF_GPRAM_DATA_OF_4 = addr2 >> SFC_GENERIC_24_BIT_OFFSET;
        SFC->RW_SF_MAC_OUTL = 5;
    } else if (D->AddressCycle == 3) {
