...
void sw_dma_irq_handler(hal_nvic_irq_t irq_number)
{
    ...
    if (irq_number != SW_DMA_CHANNEL_IRQ_NUMBER) {
        assert(0);
    }
...
hal_sw_gdma_status_t hal_sw_gdma_start(hal_sw_dma_config_info_t *info)
{
    ...
    if (room_left >= 1)  {
        ...
        /* virtual to physical */
        sw_dma_internal_config_info[BWrite(sw_dma_buffer)].source_address = hal_memview_mcu_to_infrasys(info->source_address);
        sw_dma_internal_config_info[BWrite(sw_dma_buffer)].destination_address = hal_memview_mcu_to_infrasys(info->destination_address);
