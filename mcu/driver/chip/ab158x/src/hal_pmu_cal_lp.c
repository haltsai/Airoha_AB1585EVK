...
void pmu_set_sys_ldo(void)
{
    ...
    if (bat_cfg.volt_sel >= 4300) { //if CV >=4.3V, VSYSLDO output +1, todo, need remove?
        tmp_val++;
...
void pmu_set_buck_ripple(void)
{
     ...
     */
    ripple_vaud18_en = nvkey.ripple_vcore_en;
}
...
void pmu_set_vcore_dl(void)
{
    ...
    if (nvkey.vcore_dummy_en) {
        pmu_set_register_value_lp(RG_VCORE_RVD_ADDR, 0x1, 2, nvkey.vcore_dummy_en);
        pmu_set_register_value_lp(RG_VCORE_RVD_ADDR, 0x1F, 3, nvkey.data[nvkey.sel - 1].val);
    } else {
...
void pmu_set_vio18_dl(void)
{
    ...
    if (nvkey.en) {
        pmu_set_register_value_lp(RG_VIO18_RVD_ADDR, 0x1, 2, nvkey.en);
        pmu_set_register_value_lp(RG_VIO18_RVD_ADDR, 0x1F, 3, nvkey.data[nvkey.sel - 1].val);
    } else {
...
void pmu_cal_vbat_adc(void)
{
    ...
    volt = bat_table[bat_cfg.volt_sel];
...
void pmu_cal_cv_stop_curr(void)
{
    ...
    vichg_adc.cv_stop_curr_perc = nvkey.cv_stop_curr[nvkey.sel].perc;
...
void pmu_cal_chg_jeita_warm(void)
{
    ...
    jeita_cv = bat_table[bat_cfg.volt_sel] - jeita.warm_state_dac_dec;
...
void pmu_cal_chg_jeita_cool(void)
{
    ...
    jeita_cv = bat_table[bat_cfg.volt_sel] - jeita.cool_state_dac_dec;
    adc = pmu_lerp(otp_dac.dac_4v35.volt, vichg_adc.adc_4v35, otp_dac.dac_4v2.volt, vichg_adc.adc_4v2, jeita_cv);
    prec = cv_stop_curr_adc.cv_stop_curr[cv_stop_curr_adc.sel - 1].perc;
