    ...
    *(volatile uint32_t *)(0x400A000C) |= 0x3; // global top interrupt enable
    ...
    for (hal_uart_port_t uart_port = HAL_UART_0; uart_port < HAL_UART_MAX; uart_port++) {
        if (uart_port == g_uart_port_for_logging) {
            ...
            */
            //printf("restore uart_port = %d",uart_port);
            ...
            */
            ...
            // delay = ((10000000) / g_uart_baudrate_map[g_uart_config[uart_port].baudrate]) + 2; //TODO
            // hal_gpt_delay_us(delay); //TODO
    ...
    // log_para->infra_reinit_flag  &= ~(0x1 << 0); //clear infra re-initialization flag.
}
#endif /*HAL_SLEEP_MANAGER_ENABLED*/
...
ATTR_TEXT_IN_TCM uint32_t uart_get_hw_rptr(hal_uart_port_t uart_port, int32_t is_rx)
{
    ...
    /* TODO: need cache enable */
#ifdef HAL_CACHE_MODULE_ENABLED
    read_pointer = hal_cache_cacheable_to_noncacheable(read_pointer);
#else
    read_pointer |= 0x20000000;
#endif
    if (is_rx) {
...
ATTR_TEXT_IN_TCM uint32_t uart_get_hw_wptr(hal_uart_port_t uart_port, int32_t is_rx)
{
    ...
    /* TODO: need cache enable */
#ifdef HAL_CACHE_MODULE_ENABLED
    write_pointer = hal_cache_cacheable_to_noncacheable(write_pointer);
#else
    write_pointer |= 0x20000000;
#endif
    if (is_rx) {
