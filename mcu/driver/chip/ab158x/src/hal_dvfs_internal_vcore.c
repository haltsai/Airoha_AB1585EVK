...
ATTR_RWDATA_IN_TCM dvfs_opp_t dvfs_domain = {
    ...
    .max_opp_idx = (HAL_DVFS_OPP_NUM - 1), /* This currently should indicate HAL_DVFS_OPP_HIGH */
#if defined(MTK_BOOT_SYS_CLK_HIGH)
    .min_opp_idx = HAL_DVFS_OPP_HIGH,
#elif defined(MTK_BOOT_SYS_CLK_MID)
    .min_opp_idx = HAL_DVFS_OPP_MID,
#elif defined(MTK_BOOT_SYS_CLK_LOW)
    .min_opp_idx = HAL_DVFS_OPP_LOW,
#else
#error /* Need MTK_BOOT_SYSTEM_CLK definition */
#endif
 ...
 */
static inline void dvfs_opp_ref_cnt_proc(hal_dvfs_lock_parameter_t request_type, dvfs_frequency_t request_opp_idx)
{
    if (request_type == HAL_DVFS_LOCK) {
        if ((dvfs_domain.opp_ref_cnt_arr[request_opp_idx] + 1) < dvfs_domain.opp_ref_cnt_arr[request_opp_idx]) {
            /* Check if reference count is  */
            log_hal_msgid_info("DVFS opp ref count overflow err! opp_ref_cnt_arr[%d] == %d\r\n", 2, request_opp_idx, dvfs_domain.opp_ref_cnt_arr[request_opp_idx]);
            assert(0);
        }
        dvfs_domain.opp_ref_cnt_arr[request_opp_idx]++;
    } else { /*HAL_DVFS_UNLOCK */
        /* Check if reference count is smaller than 0 (this means someones ref count call is unbalanced) */
        if ((dvfs_domain.opp_ref_cnt_arr[request_opp_idx] - 1) < 0) {
            log_hal_msgid_info("DVFS opp ref count err! (smaller than 1) opp_ref_cnt_arr[%d] == %d\r\n", 2, request_opp_idx, dvfs_domain.opp_ref_cnt_arr[request_opp_idx]);
            assert(0);
        }
        dvfs_domain.opp_ref_cnt_arr[request_opp_idx]--;
    }
 ...
 */
static inline dvfs_frequency_t dvfs_next_opp_sel(void)
{
...
ATTR_TEXT_IN_TCM hal_dvfs_status_t dvfs_request_proc(hal_dvfs_lock_parameter_t request_type, dvfs_frequency_t request_opp_idx, dvfs_frequency_t *selected_opp_idx)
{
    ...
    uint8_t ref_cnt_arr_after_proc[HAL_DVFS_OPP_NUM] = {0};
    ...
    dvfs_domain.dvfs_switch_interval = dvfs_cur_ticks;
    dvfs_opp_ref_cnt_proc(request_type, request_opp_idx);
    *selected_opp_idx = dvfs_next_opp_sel();
