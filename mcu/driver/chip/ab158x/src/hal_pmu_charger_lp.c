...
void pmu_chg_init(void)
{
    ...
    pmu_set_register_value_lp(CHG_PRE_COMPLETE_INT_EN_ADDR, CHG_PRE_COMPLETE_INT_EN_MASK, CHG_PRE_COMPLETE_INT_EN_SHIFT, 0x1);
...
void pmu_ntc_update_state(int *ret_temp, pmu_ntc_state_t *curr_state)
{
    ...
    uint8_t ntc_state = PMU_NTC_NORM;
...
void pmu_ntc_get_efuse_ragpio(uint16_t *ret)
{
    ...
    uint16_t offset_err = ((efuse >> 10) & 0x1FF);
    uint16_t gain_err = (efuse & 0x3FF);

    if (ntc_cfg1.dump)
        log_hal_msgid_info("[PMU_JEITA]ntc_get_efuse_ragpio, efuse[0x%X], ragpio[%d], offset_err[%d], gain_err[%d]", 4, efuse, ragpio, offset_err, gain_err);
...
void pmu_ntc_get_efuse_r2(uint16_t *ret)
{
    ...
    if (ntc_cfg1.dump)
        log_hal_msgid_info("[PMU_JEITA]ntc_get_efuse_r2, efuse[0x%X], ldo14[%d], r2[%d]", 3, efuse, ldo14, r2);
...
uint16_t pmu_bat_get_pure_vbat(void)
{
    ...
    if if ((chg_sta == CHG_CC1) || (chg_sta == CHG_CC2) || (chg_sta == CHG_CV)) {
        ...
        if (chg_rsel.state == PMU_RSEL_VBAT) {
            if (executing_status == PMU_NTC_WARM)
                pmu_chg_rsel_ctl(PMU_RSEL_WARM);
            else if (executing_status == PMU_NTC_COOL)
                pmu_chg_rsel_ctl(PMU_RSEL_COOL);
            else
                pmu_chg_rsel_ctl(PMU_RSEL_NORM);
        }
...
void pmu_eoc_ctrl(pmu_power_operate_t en)
{
    ...
    if (vchg1 && (!vchg2)) {
        log_hal_msgid_warning("[PMU_CHG]eoc_ctrl, add chg_out intr", 0);
        ...
        log_hal_msgid_warning("[PMU_CHG]eoc_ctrl, add chg in intr", 0);
        if (chg_int_flag == CHG_INT_NONE) {
