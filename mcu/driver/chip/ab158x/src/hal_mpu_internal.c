...
ATTR_ZIDATA_IN_TCM MPU_MAIR0_Type g_mpu_mair0;
ATTR_ZIDATA_IN_TCM MPU_MAIR1_Type g_mpu_mair1;
...
ATTR_TEXT_IN_TCM void mpu_status_restore(void)
{
    ...
    for (region = HAL_MPU_REGION_0; region < HAL_MPU_REGION_MAX; region ++) {
        if (mpu_region_en & 1) {
            ...
            MPU->RLAR = g_mpu_entry[region].mpu_rlar.w;
        }
        mpu_region_en = mpu_region_en >> 1;
    }
    MPU->MAIR0 = g_mpu_mair0.w;
    MPU->MAIR1 = g_mpu_mair1.w;
