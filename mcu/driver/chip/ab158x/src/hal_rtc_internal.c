...
#ifdef HAL_RTC_GPIO3_RTC_MODE_WAKEUP_DISABLED
            wk_en    = false;
#endif
        }
        break;
...
#if 0
static void switch_system_32k_to_dcxo()
{
    ...
    hal_rtc_switch_clock_with_check(HAL_RTC_OSC32K_DCXO_MODE); /* switch to DCXO-32k */
...
static void switch_system_32k_to_eosc()
{
    uint32_t temp_flag = 0;
    uint32_t irq_mask = 0;
    ...
    hal_rtc_switch_clock_with_check(HAL_RTC_OSC32K_EOSC_MODE); /* switch to EOSC-32k */
    hal_gpt_delay_us(500); /* dummy wait for BT */
    bt_wait_clock_ready(); /*about0~ 500us: clear reg and polling reg */
    if (temp_flag == 1) {
        bt_switch_clock_to_32K();
...
{
    static uint32_t s_user_enable_state = 0;
    ...
    hal_gpt_delay_ms(1);
    log_rtc_warn("[hal][rtc] hal_rtc_switch_to_dcxo(%d:%d-%d-%d-%dus)", 5, tick1 - tick0, rtick2 - rtick1, rtick3 - rtick2, rtick4 - rtick3, rtick5 - rtick4);
}
