...
hal_wdt_status_t hal_wdt_init(hal_wdt_config_t *wdt_config)
{
    ...
    printf("[hal][wdt] init mod(%d),tm(%ds)", (int) wdt_config->mode, (int) wdt_config->seconds);
#endif
...
#ifndef __EXT_BOOTLOADER__
    printf("[hal][wdt] sw rst, caller:0x%x", (unsigned int)lr_reg);
#endif
    hal_gpt_delay_ms(10);
