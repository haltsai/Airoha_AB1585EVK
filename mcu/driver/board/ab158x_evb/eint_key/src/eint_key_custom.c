...
void eint_key_custom_init(void)
{
    ...
    if ((BSP_EINTKEY0_PIN == 0xff) && (BSP_EINTKEY1_PIN == 0xff) && (BSP_EINTKEY2_PIN == 0xff) && (BSP_EINTKEY3_PIN == 0xff) && (BSP_EINTKEY4_PIN == 0xff)) {
        log_hal_msgid_info("[eint_key]eint key pin has not been configured by ept tool on GPIO page\r\n", 0);
        assert(0);
    }

    if ((BSP_EINTKEY0_EINT == 0xff) && (BSP_EINTKEY1_EINT == 0xff) && (BSP_EINTKEY2_EINT == 0xff) && (BSP_EINTKEY3_EINT == 0xff) && (BSP_EINTKEY4_EINT == 0xff)) {
        log_hal_msgid_info("[eint_key]eint number has not been configured by ept tool on EINT page\r\n", 0);
        assert(0);
    }
    ...
    eint_key_mapping[3].gpio_port = BSP_EINTKEY3_PIN;
    eint_key_mapping[4].gpio_port = BSP_EINTKEY4_PIN;
    ...
    eint_key_mapping[3].eint_mode = BSP_EINTKEY3_PIN_M_EINT;
    eint_key_mapping[4].eint_mode = BSP_EINTKEY4_PIN_M_EINT;
    ...
    eint_key_mapping[3].eint_number = BSP_EINTKEY3_EINT;
    eint_key_mapping[4].eint_number = BSP_EINTKEY4_EINT;
    ...
    eint_key_mapping[3].key_data = BSP_EINT_KEY_DATA3;
    eint_key_mapping[4].key_data = BSP_EINT_KEY_DATA4;

    for (i = 0; i < BSP_EINT_KEY_NUMBER; i++) {
        log_hal_msgid_info("[eint_key]led%d: gpio = %d; mode = %d; eint=%d; key_data = %d\r\n", 5, \
                           i, \
