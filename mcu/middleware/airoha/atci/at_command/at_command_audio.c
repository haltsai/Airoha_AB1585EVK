...
#include "hal_log.h"
#if ((PRODUCT_VERSION == 1552) || defined(AM255X) || (PRODUCT_VERSION == 2822) || AIR_BTA_IC_PREMIUM_G2 || AIR_BTA_IC_PREMIUM_G3)
...
static void usb_audio_volumechange_cb(uint8_t ep_number, uint8_t channel, int32_t volume)
{
...
#include "va_model_manager.h"
                ...
                if (chr == NULL) {
                    ...
                    if (strstr((char *)chr, "ENABLE") != NULL) {
                        ...
                        printf("[test] %c, %ld", enable, user_volume);
                        if (enable) {
...
#include "audio_dump.h"
            ...
            /* SMT item for airoha automotive product */
                        ...
                        for (i = 0; i < TRANSMITTER_ID_MAX; i++) {
                            if ((g_transmitter_user[i].scenario_type == scenario_type)
                                ...
                                if (scenario_type == AUDIO_TRANSMITTER_WIRED_AUDIO) {
                                    ...
                                    runtime_config.wired_audio_runtime_config.vol_level.vol_level_r = param;
                ...
                config_s = strchr((char *)config_s, ',');
                config_s++;
                sscanf(config_s, "%u", &sample_rate);

                config_s = strchr((char *)config_s, ',');
                config_s++;
                sscanf(config_s, "%u", &format);

                config_s = strchr((char *)config_s, ',');
                config_s++;
                sscanf(config_s, "%u", &word_len);
                ...
                if ((device == HAL_AUDIO_DEVICE_I2S_MASTER) ||
                    ...
                    /* GPIO INIT */
                    hal_gpio_init(gpio_pin); //I2S master 0
