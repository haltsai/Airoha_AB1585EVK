...
#endif
#ifdef AIR_ADAPTIVE_EQ_ENABLE
    if (audio_path == PEQ_AUDIO_PATH_ADAPTIVE_EQ) {
...
void aud_send_aeq_share_info()
{
    ...
    hal_audio_dsp_controller_send_message(MSG_MCU2DSP_COMMON_ADEQ_SET_PARAM, AEQ_GET_INDEX_INFO, share_addr, true);
}
...
int32_t aud_aeq_get_sound_mode(bt_sink_srv_am_type_t type, uint8_t *peq_info)
{
    aeq_share_info_t *share_addr;
    share_addr = (aeq_share_info_t *)hal_audio_query_share_info(AUDIO_MESSAGE_TYPE_AEQ_MONITOR_ADAPTIVE_EQ);
    ...
    if(cur_type == A2DP){
        ...
        peq_info[0] = g_peq_handle.adaptive_peq_enable;
        peq_info[1] = peq_info[1] = g_peq_handle.adaptive_peq_sound_mode;
        s_agent_adaptive_eq_index = g_peq_handle.adaptive_peq_sound_mode;
        s_partner_adaptive_eq_index = g_peq_handle.adaptive_peq_sound_mode;
        ret = 0;
    }
    return ret;
}

int32_t aud_aeq_detect_get_status(void)
{
    int32_t ret = -1;
    uint8_t aeq_detect_status = 0;
    sysram_status_t status;
...
#define CHECK_KEEP(nvkey_id) ((nvkey_id >= NVKEY_DSP_PARA_PEQ_PATH_0) && (nvkey_id < NVKEY_DSP_PARA_PEQ_COEF_29)) ? 1 : 0
    ...
    uint8_t T_NVDM_F232[] =
    {
        0x01,0x01,0x01,0x01,0x01,0x01,0x00,0x01,0x01,0x01,0x00,0x01,0x01,0x03,0x00,0x00,
    };
...
void bt_codec_am_ull_ble_ul_open(bt_codec_le_audio_ul_t ul_param,bt_sink_srv_am_background_t *background_ptr)
{
    ...
    ble_codec.ul_param.context_type = AUDIO_CONTENT_TYPE_ULL_BLE;
...
#if 0
            //User customize sidetone parameters
            sidetone.in_misc_parms                   = ((in_device.afe.misc_parms & 0xFFFFF) | ((HAL_AUDIO_BIAS_SELECT_BIAS0 | HAL_AUDIO_BIAS_SELECT_BIAS4) << 20)); // bias_select enum : hal_audio_bias_selection_t
            sidetone.ul_adc_mode                     = HAL_AUDIO_ANALOG_INPUT_ACC20K;       // ul_adc_mode enum : hal_audio_analog_mdoe_t /*uplink adc mode: 0x0:ACC_10k, 0x1:ACC_20k, 0x2:DCC*/
            sidetone.bias1_2_with_LDO0               = 1;
            sidetone.iir_filter                      = HAL_AUDIO_UL_IIR_75HZ_AT_48KHZ;      // iir_filter enum : hal_audio_ul_iir_t
            sidetone.performance                     = AFE_PEROFRMANCE_LOW_POWER_MODE;      // performance enum : hal_audio_performance_mode_t /*uplink performance: 0:NM, 1:HP, 2:LP, 3:ULP, 4:SULP*/
            sidetone.in_device_sample_rate           = 16000;
#endif
#endif
    ...
    do {
        ...
        switch (feature_type) {
            ...
            case AM_NR_PARAM:
                ...
                break;
#if defined AIR_DUAL_CHIP_MIXING_MODE_ROLE_MASTER_ENABLE || defined AIR_DUAL_CHIP_MIXING_MODE_ROLE_SLAVE_ENABLE
            case AM_UART_COSYS_CONTROL: {
                mcu_dchs_cosys_ctrl_cmd_execute(ami_feature->feature_param.cosys_ctrl);
                break;
            }
#endif
            default:
