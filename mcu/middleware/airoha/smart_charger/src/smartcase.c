...
void DrvCharger_SmartCase_Init(void)
{
    ...
    switch (smchg_cfg.chg_type) {
        case SMCHG_TYPE_NONE_VBUS:
            log_hal_msgid_info("smchg, chg_type[none_vbus]", 0);
            break;
        case SMCHG_TYPE_NONE_VBUS_UART:
            log_hal_msgid_info("smchg, chg_type[none_vbus_uart]", 0);
            break;
...
#endif
            break;
        case SMCHG_TYPE_1WIRE_3PINS:
            log_hal_msgid_info("smchg, chg_type[1wire_3pins]", 0);
            break;
        case SMCHG_TYPE_VBUS_PATTERN:
            log_hal_msgid_info("smchg, chg_type[vbus_pattern]", 0);
            break;
    ...
    if (ChgCase_GetCaseType() == CASE_TYPE_SMART) {
        ...
        smchg_1wire_init();
#else
        DRV_SmartChargerCase_1wire_Init(NULL);
#endif
    } else {
        return;
    }
