...
static atci_status_t usb_at_dpset_handler(atci_parse_cmd_param_t *parse_cmd);
...
static atci_cmd_hdlr_item_t g_usb_atci_cmd[] = {
    ...
    {
        .command_head = "AT+USB_DPSET",
        .command_hdlr = usb_at_dpset_handler,
        .hash_value1 = 0,
        .hash_value2 = 0,
    },
};
...
static atci_status_t usb_at_dpset_handler(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    char *s = NULL;
    const char *format_s = "%x";
    uint32_t data;
    int status;

    LOG_MSGID_I(USB_ATCI, "usb_at_dpset_handler mode = %d", 1, parse_cmd->mode);
    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_EXECUTION: {
            s = parse_cmd->string_ptr + parse_cmd->name_len + 1;

            status = sscanf(s, format_s, &data);
            if (status > 0) {
                if ((bool)data) {
                    hal_usb_phy_preinit();
                    hal_usb_init();
                    hal_usb_pull_up_dp_line();
                    LOG_MSGID_I(USB_ATCI, "usb_at_dpset_handler mode 1", 0);
                } else {
                    hal_usb_deinit();
                    // USB_Release_Type();
                    LOG_MSGID_I(USB_ATCI, "usb_at_dpset_handler mode 0", 0);
                }
                snprintf((char *)response.response_buf,
                         ATCI_UART_TX_FIFO_BUFFER_SIZE,
                         "DP set: %d, 1(High), 0(Low)\r\n",
                         (bool)data
                        );
                response.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
            } else {
