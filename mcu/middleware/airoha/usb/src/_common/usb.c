...
static void usb_device_cfg_nvkey_read(void)
{
    ...
    nvkey_size   = sizeof(devcfg);
    nvkey_status = nvkey_read_data(NVID_USB_SETTING_0, (uint8_t *)&devcfg, &nvkey_size);

    if (nvkey_status == NVKEY_STATUS_OK) {
        g_usb_high_speed_enable = devcfg.hs_fs;
    }
    LOG_MSGID_I(USB, "usb_device_cfg_nvkey_read, nvkey_status[%d] hs_fs[%d]", 2, nvkey_status, devcfg.hs_fs);
}
...
#if defined(MTK_USB_XBOX_ENABLE)
        case USB_XBOX:
...
{
    LOG_MSGID_I(USB, "USB_Register_reset_Callback cb = 0x%X", 1, cb);
    if (cb == NULL) {
        LOG_MSGID_I(USB, "USB_Register_reset_Callback, cb is NULL WARNING!!", 0);
    }
    usb_reset_cb = cb;
}
...
static void USB_Reset(void)
{
    LOG_MSGID_I(USB, "USB_Reset", 0);
    uint32_t ep_num;

    if (usb_reset_cb) {
        usb_reset_cb();
    } else {
        LOG_MSGID_I(USB, "USB_Reset, usb_reset_cb is NULL WARNING!!", 0);
    }
...
{
    LOG_MSGID_I(USB, "USB_Register_Suspend_Callback cb = 0x%X", 1, cb);
    if (cb == NULL) {
        LOG_MSGID_I(USB, "USB_Register_Suspend_Callback, cb is NULL WARNING!!", 0);
    }
    usb_suspend_cb = cb;
}
...
{
    LOG_MSGID_I(USB, "USB_Register_Resume_Callback, cb = 0x%X", 1, cb);
    if (cb == NULL) {
        LOG_MSGID_I(USB, "USB_Register_Resume_Callback, cb is NULL WARNING!!", 0);
    }
    usb_resume_cb = cb;
}
...
static void USB_Resume(void)
{
    LOG_MSGID_I(USB, "USB_Resume, usb_resume_cb = 0x%X", 1, usb_resume_cb);

    if (usb_resume_cb) {
        usb_resume_cb();
    } else {
        LOG_MSGID_I(USB, "USB_Resume, usb_resume_cb is NULL WARNING!!", 0);
    }
}
...
extern uint8_t audio_hid_dscr[];
extern uint8_t audio_hid_report_dscr[];
#endif
...
static bool USB_Cmd_GetDescriptor(Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    LOG_MSGID_I(USB, "USB_Cmd_GetDescriptor : wValue = 0x%X", 1, pcmd->wValue);
        ...
        switch (pcmd->wValue & USB_CMD_DESCMASK) {
            ...
            case USB_CMD_OTHER_SPEED:
                ...
                if (byConfig >= gUsbDevice.devdscr.bNumConfigurations) {
                    ...
                    pcfg = (Usb_Cfg_Dscr *)gUsbDevice.conf;
                    pcfg->bDescriptorType = USB_OTHER_SPEED;
            ...
            case USB_CMD_HID_REPORT:
                USB_Generate_EP0Data(pep0state, pcmd, hid_report_dscr, 52);
                break;
#endif

#ifdef MTK_USB_AUDIO_HID_ENABLE
            case USB_CMD_HID_DESC:
                USB_Generate_EP0Data(pep0state, pcmd, audio_hid_dscr, USB_AUDIO_HID_DESCRIPTOR_LENGTH);
                break;
            case USB_CMD_HID_REPORT:
                USB_Generate_EP0Data(pep0state, pcmd, audio_hid_report_dscr, USB_HID_REPORT_LEN);
                break;
...
static bool USB_Cmd_SetConfiguration(Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    LOG_MSGID_I(USB, "USB_Cmd_SetConfiguration", 0);
    byConfig = (uint8_t)(pcmd->wValue & 0x00FF);
    ...
    LOG_MSGID_I(USB, "USB device settings as below", 0);
    LOG_MSGID_I(USB, "HS/FS:%d, VID:%04X, PID:%04X, ap_type: %d", 4,
...
static void USB_Stdcmd(Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    LOG_MSGID_I(USB, "USB_Stdcmd: bRequest = 0x%X wIndex = 0x%X wValue = 0x%X", 3, pcmd->bRequest, pcmd->wIndex, pcmd->wValue);

    switch (pcmd->bRequest) {
        case USB_SET_ADDRESS:
            //LOG_I(USB, "USB_SET_ADDRESS\n");
        ...
        default:
            bError = true;
            LOG_MSGID_I(USB, "bRequest = 0x%X is out of case", 1, pcmd->bRequest);
            break;
    }

    bool usb_host_detect_enable = true;
    bool usb_audio_spk2_feature = false;

    #ifdef MTK_USB_AUDIO_2ND_SPEAKER
    usb_audio_spk2_feature = g_usb_audio_spk2_feature;
    #endif

    /* If speaker2 or high speed mode is enabled, the detect mechanism will be triggered for smart phone */
    if (usb_host_detect_enable && (usb_audio_spk2_feature || g_usb_high_speed_enable)) {
...
static void USB_Endpoint0_Idle(void)
{
    ...
    //LOG_MSGID_I(USB, "USB_Endpoint0_Idle\n");
    //LOG_MSGID_I(USB, "    -bmRequestType:%x\n", 1, gUsbDevice.cmd.bmRequestType);
    //LOG_MSGID_I(USB, "    -bRequest     :%x\n", 1, gUsbDevice.cmd.bRequest);
    //LOG_MSGID_I(USB, "    -wValue       :%x\n", 1, gUsbDevice.cmd.wValue);
    //LOG_MSGID_I(USB, "    -wIndex       :%x\n", 1, gUsbDevice.cmd.wIndex);
    //LOG_MSGID_I(USB, "    -wLength      :%x\n", 1, gUsbDevice.cmd.wLength);

    //LOG_MSGID_I(USB, "USB, bmRequestType:0x%x, bRequest:0x%x, wValue:0x%x, wIndex:0x%x, wLength0x%x", 5,
        ...
        case USB_CMD_CLASSREQ:
            ...
            if ((gUsbDevice.cmd.bmRequestType == USB_CMD_CLASSIFIN) || (gUsbDevice.cmd.bmRequestType == USB_CMD_CLASSIFOUT)) {
                if ((gUsbDevice.cmd.wIndex & 0xFF) > USB_MAX_INTERFACE) {
                    ...
                    LOG_MSGID_I(common, "USB error occur, stall endpoint 1", 0);
                ...
                if (gUsbDevice.if_info[(gUsbDevice.cmd.wIndex & 0xff)].if_class_specific_hdlr != NULL) {
                    //LOG_MSGID_I(common, "USB if_info[%x] if_class_specific_hdlr", 1, (gUsbDevice.cmd.wIndex & 0xff));
                    gUsbDevice.if_info[(gUsbDevice.cmd.wIndex & 0xff)].if_class_specific_hdlr(&gUsbDevice.ep0info, &gUsbDevice.cmd);
                } else {
                    /* Error occur, stall endpoint*/
                    LOG_MSGID_I(common, "USB error occur, stall endpoint 2", 0);
                    hal_usb_update_endpoint_0_state(HAL_USB_EP0_DRV_STATE_READ_END, true, false);
                }
            }
                ...
                //if(((gUsbDevice.cmd.wIndex & 0x83) == g_UsbAudio[0].stream_ep_out_id) && g_UsbAudio[0].stream_if_info->if_class_specific_hdlr)
                if (((gUsbDevice.cmd.wIndex & 0x83) == 0x01) && g_UsbAudio[0].stream_if_info->if_class_specific_hdlr) {
                    /*wIdex: ep num*/
                    g_UsbAudio[0].stream_if_info->if_class_specific_hdlr(&gUsbDevice.ep0info, &gUsbDevice.cmd);
                }
                //else if (((gUsbDevice.cmd.wIndex & 0x83) == g_UsbAudio[1].stream_ep_out_id) && g_UsbAudio[1].stream_if_info->if_class_specific_hdlr)
                else if (((gUsbDevice.cmd.wIndex & 0x83) == 0x02) && g_UsbAudio[1].stream_if_info->if_class_specific_hdlr) {
                    g_UsbAudio[1].stream_if_info->if_class_specific_hdlr(&gUsbDevice.ep0info, &gUsbDevice.cmd);
                }
#ifdef MTK_USB_AUDIO_MICROPHONE
                //else if(((gUsbDevice.cmd.wIndex & 0x83) == g_UsbAudio[0].stream_ep_in_id) && g_UsbAudio[0].stream_microphone_if_info->if_class_specific_hdlr)
                else if (((gUsbDevice.cmd.wIndex & 0x80) == 0x80) && g_UsbAudio[0].stream_microphone_if_info->if_class_specific_hdlr) {
        ...
        case USB_CMD_VENDREQ:
#ifdef MTK_USB_XBOX_ENABLE
            LOG_MSGID_I(USB, "USB_CMD_VENDREQ", 0);
            if (gUsbDevice.cmd.bRequest == bMS_Vendor_Code) {
                ...
                if (gUsbDevice.cmd.wIndex == EXTENDED_COMPATIBLE_ID) {
                    LOG_MSGID_I(USB, "EXTENDED_COMPATIBLE_ID", 0);
                    ...
                    LOG_MSGID_I(USB, "EXTENDED_COMPATIBLE", 0);
...
void USB_Endpoint0_Tx(void)
{
    ...
    //LOG_MSGID_I(USB, "USB_Endpoint0_Tx\n", 0);
...
static void USB_Endpoint0_Hdlr(void)
{
    ...
    //LOG_MSGID_I(USB, "USB_Endpoint0_Hdlr\n", 0);
