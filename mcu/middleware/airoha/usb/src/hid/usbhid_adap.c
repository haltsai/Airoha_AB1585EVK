...
{
    if ((g_UsbHID[usb_port].txpipe == NULL) || (USB2UARTPort[usb_port].initialized == false) || (USB_Get_Device_State() != DEVSTATE_CONFIG)) {
        LOG_MSGID_E(common, "USB2UART_Check_Config, txpipe=0x%x, init=%d, state=%d\n", 3, g_UsbHID[usb_port].txpipe, USB2UARTPort[usb_port].initialized, USB_Get_Device_State());
        return false;
    } else {
        return true;
    }
...
void USB2UART_init(void)
{
    ...
    for (usb_port = 0; usb_port < MAX_USB_PORT_NUM; usb_port++) {
        ...
        g_UsbHID[usb_port].send_Rxilm = true;

        g_UsbHID[usb_port].config_send_Txilm = false;
...
void USB2UART_ClrRxBuffer(uint8_t port)
{
    ...
    /* Clear hardware FIFO if current status is CDC HID */
    if (USB2UART_Check_Config(usb_port) == true) {
        /* Clear RX FIFO */
        USB_Hid_Rx_ClrFifo(port);
...
uint16_t USB2UART_GetBytes(uint8_t port, uint8_t *buffaddr, uint16_t length)
{
    ...
    if (real_count >= length) {
        ...
        g_UsbHID[usb_port].send_Rxilm = true;  /*After this time get byte, buffer will be empty */
...
void USB2UART_ClrTxBuffer(uint8_t port)
{
    ...
    /* Stop DMA channel if current status is CDC HID */
    if (USB2UART_Check_Config(usb_port) == true) {
        hal_usb_stop_dma_channel(g_UsbHID[usb_port].txpipe->byEP, HAL_USB_EP_DIRECTION_TX);
        g_UsbHID[usb_port].setup_dma = false;
        if (g_UsbHID[usb_port].put_start == true) {
            g_UsbHID[usb_port].dmaf_setmember |= 0x20;
        }
    }
    savedMask = SaveAndSetIRQMask();
    ...
    {
        g_UsbHID[usb_port].send_Txilm = true;
    }
...
uint16_t USB2UART_PutBytes(uint8_t port, uint8_t *buffaddr, uint16_t length)
{
...
#if 0
    uint32_t usb_port = USB_PORT[port];
    int16_t  data_len = length;
    uint16_t i = 0;

    do {
        if (data_len > 64) {
            hal_usb_write_endpoint_fifo(g_UsbHID[usb_port].txpipe->byEP, 64, buffaddr + i * 64);
        } else {
            hal_usb_write_endpoint_fifo(g_UsbHID[usb_port].txpipe->byEP, data_len, buffaddr + i * 64);
        }

        hal_usb_set_endpoint_tx_ready(g_UsbHID[usb_port].txpipe->byEP);
        while (!hal_usb_is_endpoint_tx_empty(g_UsbHID[usb_port].txpipe->byEP));
        data_len -= 64;
        i++;
    } while (data_len > 0);
    ...
    if (USB2UART_Check_Config(usb_port) == false) {
        ...
        if ((USB2UARTPort[usb_port].initialized == true) && (USB_Get_Device_State() != DEVSTATE_CONFIG)) {
            g_UsbHID[usb_port].config_send_Txilm = true;  /* for PC set config later then can issue the first message */
    ...
    if ((real_count == 0) && (g_UsbHID[usb_port].dma_txcb_just_done == true)) {
        LOG_MSGID_I(common, "ASSERT", 0);
    }

    g_UsbHID[usb_port].dma_txcb_just_done = false;

    /* determine real sent data count */
    if (real_count > length) {
        real_count = length;
    } else {
        g_UsbHID[usb_port].send_Txilm = true;  /*After this time put bytes, buffer will be full */
        g_UsbHID[usb_port].config_send_Txilm = true; /* if be reseted, then it can issue the message waited for*/
    }
    RestoreIRQMask(savedMask);
    ...
    if (g_UsbHID[usb_port].txpipe != NULL) {
        if (g_UsbHID[usb_port].setup_dma == false) {
            g_UsbHID[usb_port].setup_dma = true;
            setup_dma = true;
            g_UsbHID[usb_port].put_start = true;
            g_UsbHID[usb_port].dmaf_setmember = 0;
        }
    }
    RestoreIRQMask(savedMask);
    ...
    g_UsbHID[usb_port].put_start = false;
    g_UsbHID[usb_port].dmaf_setmember = 0;
...
void USB2UART_PutData_Polling(uint8_t port, uint8_t *Buffaddr, uint16_t Length)
{
    ...
    while (toSend > 0) {
        ...
        hal_usb_set_endpoint_tx_ready(g_UsbHID[usb_port].txpipe->byEP);
        while (!hal_usb_is_endpoint_tx_empty(g_UsbHID[usb_port].txpipe->byEP));
        hal_usb_write_endpoint_fifo(g_UsbHID[usb_port].txpipe->byEP, Length, Buffaddr);
        hal_usb_set_endpoint_tx_ready(g_UsbHID[usb_port].txpipe->byEP);
        while (!hal_usb_is_endpoint_tx_empty(g_UsbHID[usb_port].txpipe->byEP));
