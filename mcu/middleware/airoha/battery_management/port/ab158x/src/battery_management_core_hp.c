...
int battery_basic_checkpoint[9] = {3443, 3513, 3579, 3617, 3667, 3748, 3853, 3954, 4062}; /*Index and Value output: For linear gauge*/
int precc_voltage[8] = {2400, 2550, 2700, 2850, 3000, 3150, 3300, 3450}; /*Index and Value output: PRECC Voltage*/
int bm_cc_cur[137] = {
...
int battery_jeita_percentage[8] = {20, 0, 0, 0, 40, 60, 80, 100 }; /*Index and Value output: Jeita percentage*/
int battery_icl_value[11] = {10, 75, 200, 300, 443, 500, 600, 700, 800, 900, 1000}; /*Index and Value output: ICL*/
...
void battery_charger_detect_callback(void)
{
    ...
    if (bm_cust.feature_2cv == BATTERY_OPERATE_ON) {
        battery_set_2cv_parameter(BM_2STEP_SET_PARA);
    }
...
#ifndef AIR_BATTERY_MANAGEMENT_STATIC_INIT_ENABLE
...
uint8_t battery_management_set_battery_category(void)
{
    ...
    bm_cust.battery_category = (buffer_b[67] << 8) + buffer_b[66];
...
battery_management_status_t battery_init_date_form_nvdm()
{
    ...
    bm_cust.powerhold = (buffer_g[1] << 8) + buffer_g[0];
    bm_cust.vbus_debounce = (buffer_g[3] << 8) + buffer_g[2];
    ...
    bm_cust.feature_2cv = (buffer_b[77] << 8) + buffer_b[76];
    ...
    if (bm_cust.feature_2cv == BATTERY_OPERATE_OFF) {
        ...
        bm_cust.iterm_irq_cv1_value = ((buffer_b[69] << 8) + buffer_b[68]) / 10;
        ...
        bm_cust.cv1_value = (buffer_b[71] << 8) + buffer_b[70];
        ...
        bm_cust.iterm_irq_cv2_value = ((buffer_b[73] << 8) + buffer_b[72]) / 10;
        ...
        bm_cust.cv2_value = (buffer_b[75] << 8) + buffer_b[74];
