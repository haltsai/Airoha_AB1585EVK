...
TimerHandle_t xbm_jeita_timer;                                      /*Timer : When With SW NTC is enable ,This timer will check SW NTC status*/
...
void battery_charger_in_callback() {
    LOG_MSGID_I(battery_management, "[BM_CHR]Vbus on\r\n", 0);
...
void battery_charger_out_callback() {
    LOG_MSGID_I(battery_management, "[BM_CHR]Vbus off\r\n", 0);
    uint32_t mask_pri;
...
void battery_timer_init(void) {
    ...
    if (bm_cust.feature_jeita == BATTERY_OPERATE_ON) {
        ...
        if (xTimerStart(xbm_jeita_timer, 0) != pdPASS) {
            LOG_MSGID_I(battery_management, "[BM_BASIC]xbm_jeita_Timer xTimerStart fail\n", 0);
        }
        LOG_MSGID_I(battery_management, "[BM_BASIC]battery_sw_ntc_feature enable:%d [%d sec] \n", 2,bm_cust.feature_jeita,intervale_time);
    } else {
