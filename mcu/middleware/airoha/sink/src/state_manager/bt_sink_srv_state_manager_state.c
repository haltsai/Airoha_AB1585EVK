        ...
        bt_sink_srv_state_t state)
{
    ...
    if (NULL != device) {
        bt_sink_srv_state_manager_update_state(context, device, psedev, false);
    } else {
    ...
    bool set_focus_device)
{
    ...
    */
    if (BT_SINK_SRV_STATE_MANAGER_DEVICE_TYPE_LE == device->type &&
        ...
        BT_SINK_SRV_STATE_OUTGOING == device->call_state ||
        BT_SINK_SRV_STATE_ACTIVE == device->call_state ||
        BT_SINK_SRV_STATE_HELD_REMAINING == device->call_state)) {
        is_le_call_ongoing = true;
    }
    bt_sink_srv_report_id("[Sink][StaMgr][Debug] is le call: %d", 1, is_le_call_ongoing);
...
void bt_sink_srv_state_manager_aws_mce_report_callback(bt_aws_mce_report_info_t *info)
{
    ...
    bt_sink_srv_report_id(
            "[Sink][StaMgr]aws mce report callback, type: 0x%x direction: 0x%x",
            2,
            header->type,
            header->direction);
