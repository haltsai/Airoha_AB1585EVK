...
static bool bt_sink_srv_cap_am_resume_timer_start(bt_sink_srv_cap_am_mode mode);
...
#endif
                        } else if (mode <= CAP_AM_UNICAST_MUSIC_MODE_1) {
...
static void bt_sink_srv_cap_am_play_callback(audio_src_srv_handle_t *handle)
{
    ...
    if (state == CAP_AM_STATE_SUSPENDING) {
        ...
        ble_bap_ase_id_list_t ase_list = bt_sink_srv_cap_stream_find_streaming_ase_id_list(conn_handle);
        bt_le_audio_direction_t direction = bt_sink_srv_cap_stream_find_streaming_ase_direction(conn_handle);
        ...
        (state == CAP_AM_STATE_PLAYING && g_cap_am_switch_suspending_mode == mode)))
    {
        ...
        if (mode == CAP_AM_BROADCAST_MUSIC_MODE) {
            ...
            if (mode <= CAP_AM_UNICAST_CALL_MODE_1 || ul_only ) {
        ...
        le_audio_log("[CAP][AM] DL config_info [codec:%X][sampling_freq:%X][frame_payload_length:%04X][volume:%x]", 4,
        ...
        if (config_info.frame_payload_length) {
	        if (mode == CAP_AM_UNICAST_MUSIC_MODE_0 || mode == CAP_AM_UNICAST_MUSIC_MODE_1) {
...
static void bt_sink_srv_cap_am_stop_callback(audio_src_srv_handle_t *handle)
{
    ...
    if (handle->type == AUDIO_SRC_SRV_PSEUDO_DEVICE_BLE && state == CAP_AM_STATE_PREPARE_STOP) {
        ...
        if (AUD_EXECUTION_FAIL == bt_sink_srv_ami_audio_stop(g_cap_am_ctl[mode].cap_aid)) {
            ...
            bt_sink_srv_cap_am_send_event_and_set_state(mode, AUDIO_SRC_SRV_EVT_READY);
        } else if (mode == CAP_AM_UNICAST_MUSIC_MODE_0 || mode == CAP_AM_UNICAST_MUSIC_MODE_1) {
...
static void bt_sink_srv_cap_am_reject_timer_callback(TimerHandle_t timer_id)
{
    ...
    if (g_cap_am_reject_mode == CAP_AM_UNICAST_MUSIC_MODE_0 ||
        g_cap_am_reject_mode == CAP_AM_UNICAST_MUSIC_MODE_1) {
        /*TO DO MCP PAUSE*/
        bt_le_audio_sink_action_param_t le_param = {
            .service_idx = 0xFF,
        };
...
static bool bt_sink_srv_cap_am_resume_timer_start(bt_sink_srv_cap_am_mode mode)
{
    ...
    bt_sink_srv_music_start_vp_detection(g_cap_am_ctl[mode].p_handle);
    return true;
}
