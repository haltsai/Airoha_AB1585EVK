...
static void bt_sink_srv_cap_stream_cis_established_notify_handler(ble_bap_cis_established_notify_t *p_msg)
{
    ...
    bt_handle_t cis_handle;
    uint8_t link_idx = le_audio_get_link_index(p_msg->connect_handle);
    bt_sink_srv_cap_am_mode mode = bt_sink_srv_cap_am_get_current_mode();
	...
	mode < CAP_AM_MODE_NUM && (mode % CAP_UNICAST_DEVICE_NUM == link_idx)) {
...
static void bt_sink_srv_cap_stream_ase_disabling_ind_handler(ble_bap_ase_disable_ind_t *p_msg)
{
    ...
    if (!bt_sink_srv_cap_stream_is_disabling_state_ok(state)) {
        ...
        if (ase_id_list.num_of_ase == 1 &&
            ...
            if (!bt_sink_srv_cap_am_audio_stop(link_index + CAP_AM_UNICAST_MUSIC_MODE_0)) {
            ...
            if (AUDIO_DIRECTION_SINK == direction) {
                ...
                if (!bt_sink_srv_cap_am_audio_stop(link_index + CAP_AM_UNICAST_CALL_MODE_0)) {
                    for (uint8_t i = 0; i < ase_id_list.num_of_ase; i++) {
                        ble_bap_ase_disable_response(p_msg->connect_handle, ase_id_list.ase_id[i], RESPONSE_CODE_SUCCESS, ERROR_REASON_NO_ERROR);
                    }
                }
...
static void bt_sink_srv_cap_stream_cis_disconnected_notify_handler(ble_bap_cis_disconnected_notify_t *p_msg)
{
    ...
    if (g_cap_stream_service_info.max_link_num == bt_sink_srv_cap_get_link_index(p_msg->connect_handle)) {
        ...
        if (ase_list.num_of_ase) {
            ...
            {
                if (!bt_sink_srv_cap_am_audio_stop(link_index + CAP_AM_UNICAST_MUSIC_MODE_0)) {
                    for (i = 0; i < ase_list.num_of_ase; i++) {
...
static bool bt_sink_srv_cap_stream_send_ase_streaming_state_timer_start(bt_handle_t le_handle, uint8_t ase_id)
{
    ...
    //.ase_id = ase_id;
    uint32_t data = data = (le_handle | (ase_id << 16));
...
#endif
ble_bap_ase_id_list_t bt_sink_srv_cap_stream_find_streaming_ase_id_list(bt_handle_t connect_handle)
{
    uint8_t link_index = 0, ase_index;
    ble_bap_ase_id_list_t ase_id_list = {0};
...
ble_bap_ase_id_list_t bt_sink_srv_cap_stream_find_processing_and_streaming_ase_id_list(bt_handle_t connect_handle)
{
    uint8_t link_index = 0, ase_index;
    ble_bap_ase_id_list_t ase_id_list = {0};
    ...
    if((link_index = bt_sink_srv_cap_get_link_index(connect_handle)) != g_cap_stream_service_info.max_link_num)
    {
        for(ase_index = 0; ase_index < CAP_MAX_ASE_NUM; ase_index++)
        {
            if((gp_cap_stream_link_info[link_index].ase_info[ase_index].ase_state == ASE_STATE_STREAMING ||
                ...
                (gp_cap_stream_link_info[link_index].ase_info[ase_index].sub_state < SUB_STATE_DISABLING_RESPONDING ||
                gp_cap_stream_link_info[link_index].ase_info[ase_index].sub_state == CAP_INVALID_UINT8))
            {
                ase_id_list.ase_id[ase_id_list.num_of_ase] = gp_cap_stream_link_info[link_index].ase_info[ase_index].ase_id;
                ase_id_list.num_of_ase++;
    ...
    le_audio_log("[CAP][stream] bt_sink_srv_cap_stream_find_processing_and_streaming_ase_id_list [num:%d], [ase_id: %x %x %x]", 4,
        ase_id_list.num_of_ase, ase_id_list.ase_id[0], ase_id_list.ase_id[1], ase_id_list.ase_id[2]);
    return ase_id_list;
}

bt_le_audio_direction_t bt_sink_srv_cap_stream_find_streaming_ase_direction(bt_handle_t connect_handle)
{
    uint8_t link_index = 0, ase_index;
    ble_bap_ase_id_list_t ase_id_list = {0};
    ...
    if((link_index = bt_sink_srv_cap_get_link_index(connect_handle)) != g_cap_stream_service_info.max_link_num)
    {
        for(ase_index = 0; ase_index < CAP_MAX_ASE_NUM; ase_index++)
        {
                ...
                gp_cap_stream_link_info[link_index].ase_info[ase_index].sub_state == SUB_STATE_ENABLING_NOTIFYING)
            {
                ase_id_list.ase_id[ase_id_list.num_of_ase] = gp_cap_stream_link_info[link_index].ase_info[ase_index].ase_id;
                ase_id_list.num_of_ase++;
                if (gp_cap_stream_link_info[link_index].ase_info[ase_index].direction == AUDIO_DIRECTION_SOURCE) {
                    le_audio_log("[CAP][stream] bt_sink_srv_cap_stream_find_streaming_ase_direction, SOURCE [ase_id:%d] ", 1,
                        gp_cap_stream_link_info[link_index].ase_info[ase_index].ase_id);
    ...
    if (ase_id_list.num_of_ase) {
        le_audio_log("[CAP][stream] bt_sink_srv_cap_stream_find_streaming_ase_direction, SINK [ase_id:%d] ", 1,
            ase_id_list.ase_id[ase_id_list.num_of_ase-1]);
...
bt_le_audio_direction_t bt_sink_srv_cap_stream_find_processing_and_streaming_ase_direction(bt_handle_t connect_handle)
{
    uint8_t link_index = 0, ase_index;
    ble_bap_ase_id_list_t ase_id_list = {0};
    ...
    if((link_index = bt_sink_srv_cap_get_link_index(connect_handle)) != g_cap_stream_service_info.max_link_num)
    {
        for(ase_index = 0; ase_index < CAP_MAX_ASE_NUM; ase_index++)
        {
                ...
                gp_cap_stream_link_info[link_index].ase_info[ase_index].sub_state == CAP_INVALID_UINT8))
            {
                ...
                ase_id_list.num_of_ase++;
                if (gp_cap_stream_link_info[link_index].ase_info[ase_index].direction == AUDIO_DIRECTION_SOURCE) {
                    le_audio_log("[CAP][stream] bt_sink_srv_cap_stream_find_processing_and_streaming_ase_direction, SOURCE [ase_id:%d] ", 1,
                        gp_cap_stream_link_info[link_index].ase_info[ase_index].ase_id);
                    return AUDIO_DIRECTION_SOURCE;
                }
            }
    ...
    if (ase_id_list.num_of_ase) {
        le_audio_log("[CAP][stream] bt_sink_srv_cap_stream_find_processing_and_streaming_ase_direction, SINK [ase_id:%d] ", 1,
            ase_id_list.ase_id[ase_id_list.num_of_ase-1]);
        return AUDIO_DIRECTION_SINK;
    }

    le_audio_log("[CAP][stream] bt_sink_srv_cap_stream_find_processing_and_streaming_ase_direction, FAIL handle:%x", 1, connect_handle);
    return CAP_INVALID_UINT8;
}
...
{
    ble_bap_ase_id_list_t ase_list = bt_sink_srv_cap_stream_find_streaming_ase_id_list(connect_handle);
    ...
    if (ase_list.num_of_ase) {
        ...
        if (direction == AUDIO_DIRECTION_SINK) {
            bt_sink_srv_cap_am_audio_start(CAP_AM_UNICAST_MUSIC_MODE_0 + link_idx);
        } else if (direction == AUDIO_DIRECTION_SOURCE) {
            bt_sink_srv_cap_am_audio_start(CAP_AM_UNICAST_CALL_MODE_0 + link_idx);
        }
        return true;
    }
    return false;
