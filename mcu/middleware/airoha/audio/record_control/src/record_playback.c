...
hal_audio_status_t hal_audio_start_stream_in(hal_audio_active_type_t active_type)
{
...
#else
    open_param.stream_in_param.afe.frame_size      = g_wwe_mode == WWE_MODE_AMA ? 160 : 240;
#endif
    ...
    if (is_running) {
        ...
        hal_audio_status_set_running_flag(audio_scenario_type, &open_param, true);
        if (g_wwe_mode == WWE_MODE_AMA) {
            hal_audio_status_set_running_flag(AUDIO_SCENARIO_TYPE_VOW, &open_param, true);
        }
    }
...
void hal_audio_stop_stream_in(void)
{
    ...
    // Stop recording
        ...
        // Temp protect in APP level    //TODO record middleware.
        hal_audio_status_set_running_flag(audio_scenario_type, NULL, false);
        if (g_wwe_mode == WWE_MODE_AMA) {
            hal_audio_status_set_running_flag(AUDIO_SCENARIO_TYPE_VOW, NULL, false);
        }
    } else {
...
//    open_param.stream_in_param.afe.audio_device = HAL_AUDIO_DEVICE_MAIN_MIC_DUAL;
    ...
    if (is_running) {
        ...
        // Temp protect in APP level    //TODO record middleware.
        hal_audio_status_set_running_flag(AUDIO_SCENARIO_TYPE_RECORD, &open_param, true);
    }
