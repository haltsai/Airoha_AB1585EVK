...
void *RACE_DSPREALTIME_PEQ_HDR(ptr_race_pkt_t pCmdMsg, uint8_t channel_id)
{
    typedef struct {
        ...
        uint8_t peq_PhaseAndPath;    //0x00:a2dp pre peq  0x01:a2dp  post peq  0x10:LINE_IN pre peq  0x11:LINE_IN post peq  0x03:adaptive eq
        uint8_t Peq_param[1];
    ...
    if (((pCmd->peq_PhaseAndPath != 0) && (pCmd->peq_PhaseAndPath != 1) && (pCmd->peq_PhaseAndPath != 0x10) && (pCmd->peq_PhaseAndPath != 0x11) && (pCmd->peq_PhaseAndPath != 0x03) && (pCmd->peq_PhaseAndPath != PEQ_SPECIAL_PHASE)
    ...
    if (pCmdMsg->hdr.length >= 4) { //hdr.length = hdr.id(2bytes) + pAnc_cmd->status(1byte) + pAnc_cmd->anc_id(1byte) + pAnc_cmd->param(0/1/2/4 bytes)
        switch (pAnc_cmd->param.header.ancId) {
            ...
            case RACE_FADP_ANC_GET_INFO: {
                ...
                anc_ret = AUDIO_ANC_CONTROL_EXECUTION_SUCCESS;
                audio_anc_control_get_full_adaptive_info(&(pEvt->param.getInfoRsp.FADP_ANC_state), NULL);
...
{
    RACE_MMI_GET_ENUM_EVT_STRU *event = RACE_ClaimPacket(RACE_TYPE_NOTIFICATION,
                                                         RACE_MMI_GET_ENUM,
                                                         sizeof(RACE_MMI_GET_ENUM_EVT_STRU),
                                                         channel_id);
    if (event != NULL) {
        event->module  = RACE_MMI_MODULE_AEQ_GROUP_ID;
        event->status  = 0;
        ...
        RACE_LOG_MSGID_E("[ADAPTIVE_EQ] aadaptive_eq_index_response, pkt claim fail", 0);
    }
}
...
void race_dsprealtime_adaptive_eq_control_response(uint8_t status, uint8_t type, uint8_t channel_id)
{
    ...
    if (role != BT_AWS_MCE_ROLE_AGENT && role != BT_AWS_MCE_ROLE_NONE) {
        RACE_LOG_MSGID_E("[ADAPTIVE_EQ] adaptive_eq_control_response, not agent role", 0);
        return;
    }
    ...
    if (event != NULL) {
        ...
        event->type = type;
        race_status_t race_status = race_flush_packet((void *)event, channel_id);
        RACE_LOG_MSGID_I("[ADAPTIVE_EQ] adaptive_eq_control_respons, race_status=%d", 1, race_status);
    } else {
        RACE_LOG_MSGID_E("[ADAPTIVE_EQ] adaptive_eq_control_respons, pkt claim fail", 0);
    }
}
...
void race_dsprealtime_adaptive_eq_detect_status_response(bool is_response, bool is_start, uint8_t status, uint8_t channel_id)
{
    ...
    if (role != BT_AWS_MCE_ROLE_AGENT && role != BT_AWS_MCE_ROLE_NONE) {
        RACE_LOG_MSGID_E("[ADAPTIVE_EQ] adaptive_eq_detect_status_response, not agent role", 0);
        return;
    }
    ...
    RACE_MMI_GET_ENUM_EVT_STRU *event = RACE_ClaimPacket(race_type,
                                                         ...
                                                         channel_id);
    if (event != NULL) {
        event->module  = RACE_MMI_MODULE_AEQ_DETECT_STATUS;
        event->status  = 0;
        event->data[0] = status;
        race_status_t race_status = race_flush_packet((void *)event, channel_id);
        RACE_LOG_MSGID_I("[ADAPTIVE_EQ] adaptive_eq_detect_status_response, race_status=%d", 1, race_status);
    } else {
        RACE_LOG_MSGID_E("[ADAPTIVE_EQ] adaptive_eq_detect_status_response, pkt claim fail", 0);
    }
}
#endif
