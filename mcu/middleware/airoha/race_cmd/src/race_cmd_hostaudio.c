...
static void race_mmi_get_aeq_group_id(uint8_t *peq_group_id, uint8_t *status, bt_sink_srv_am_type_t type)
{
    ...
    uint8_t peq_info[4];
    if (peq_group_id && status) {
        aud_aeq_get_sound_mode(type, (uint8_t *)peq_info);
        *peq_group_id = (peq_info[0] == 0) ? 0 : peq_info[1];
        *status = RACE_ERRCODE_SUCCESS;
    }
#else
...
static void race_mmi_get_aeq_status_id(uint8_t *peq_group_id, uint8_t *status, bt_sink_srv_am_type_t type)
{
    if(type == A2DP){
        if (peq_group_id && status) {
...
static void race_mmi_get_aeq_detect_status_id(uint8_t *peq_group_id, uint8_t *status, bt_sink_srv_am_type_t type)
{
    if(type == A2DP){
        if (peq_group_id && status) {
            if (aud_aeq_detect_get_status() == 1) {
                *peq_group_id = 1;
                *status = RACE_ERRCODE_SUCCESS;
            } else{
                *peq_group_id = 0;
...
void *RACE_MMI_GET_ENUM_HDR(ptr_race_pkt_t pCmdMsg, uint8_t channel_id)
{
    ...
    switch (pCmd->module) {
...
#ifdef AIR_ADAPTIVE_EQ_ENABLE
        case RACE_MMI_MODULE_AEQ_GROUP_ID: {
        ...
        case RACE_MMI_MODULE_AEQ_STATUS_ID: {
            pEvt = RACE_ClaimPacket((uint8_t)RACE_TYPE_RESPONSE, (uint16_t)RACE_MMI_GET_ENUM, (uint16_t)sizeof(RACE_MMI_GET_ENUM_EVT_STRU), channel_id);
            if (pEvt) {
                pEvt->module = pCmd->module;
                race_mmi_get_aeq_status_id(&pEvt->data[0], &pEvt->status, A2DP);
            }
            break;
        }

        case RACE_MMI_MODULE_AEQ_DETECT_STATUS_ID: {
            pEvt = RACE_ClaimPacket((uint8_t)RACE_TYPE_RESPONSE, (uint16_t)RACE_MMI_GET_ENUM, (uint16_t)sizeof(RACE_MMI_GET_ENUM_EVT_STRU), channel_id);
            if (pEvt) {
                pEvt->module = pCmd->module;
                race_mmi_get_aeq_detect_status_id(&pEvt->data[0], &pEvt->status, A2DP);
            }
            break;
        }
