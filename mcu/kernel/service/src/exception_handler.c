...
#if defined(MTK_SAVE_LOG_AND_CONTEXT_DUMP_ENABLE) && (PRODUCT_VERSION == 7686)
...
static void populate_build_info(char *info, int maxLen)
{
    ...
    if (maxLen > MAX_BUILD_INFO_LEN) {
        maxLen = MAX_BUILD_INFO_LEN;
    }
...
void stackDump(uint32_t stack[])
{
    ...
    if (minidump_base_address != 0) {
        ...
        /* dump SCB registers */
        current_addr += sizeof(taskContext);
        ret = offline_dump_region_write(OFFLINE_REGION_MINI_DUMP,
                                        current_addr,
                                        (uint8_t *)0xe000e008,
                                        4);
        ...
        minidump_header.context_size = sizeof(taskContext) + 17 * 4;
    }
#else
...
void memoryDumpAll(void)
{
    ...
    if (minidump_base_address != 0) {
        ...
        ret = exception_minidump_check_address(taskContext.psp - 100);
        if (ret == true) {
            ret = offline_dump_region_write(OFFLINE_REGION_MINI_DUMP,
                                            current_addr,
                                            (uint8_t *)(taskContext.psp - 100),
                                            MINIDUMP_PSPSTACK_LENGTH);
            if (ret != true) {
                minidump_base_address = 0;
                return;
            }
            minidump_header.regions[0].address = taskContext.psp - 100;
        ...
        ret = exception_minidump_check_address(taskContext.msp);
        if (ret == true) {
            ret = offline_dump_region_write(OFFLINE_REGION_MINI_DUMP,
                                            current_addr,
                                            (uint8_t *)taskContext.msp,
                                            MINIDUMP_MSPSTACK_LENGTH);
            if (ret != true) {
                minidump_base_address = 0;
                return;
            }
            minidump_header.regions[1].address = taskContext.msp;
...
#if defined(MTK_SAVE_LOG_AND_CONTEXT_DUMP_ENABLE)
#if (PRODUCT_VERSION == 7686) || (PRODUCT_VERSION == 7698)
    minidump_header.reason = EXCEPTION_HARD_FAULT;
    if (assert_expr.is_valid) {
...
#if defined(MTK_SAVE_LOG_AND_CONTEXT_DUMP_ENABLE)
#if (PRODUCT_VERSION == 7686) || (PRODUCT_VERSION == 7698)
    minidump_header.reason = EXCEPTION_MEMMANAGE_FAULT;
    if (assert_expr.is_valid) {
        minidump_header.assert_info.is_valid = assert_expr.is_valid;
        minidump_header.assert_info.expr = assert_expr.expr;
        minidump_header.assert_info.file = assert_expr.file;
...
#if defined(MTK_SAVE_LOG_AND_CONTEXT_DUMP_ENABLE)
#if (PRODUCT_VERSION == 7686) || (PRODUCT_VERSION == 7698)
    minidump_header.reason = EXCEPTION_BUS_FAULT;
    if (assert_expr.is_valid) {
        minidump_header.assert_info.is_valid = assert_expr.is_valid;
        minidump_header.assert_info.expr = assert_expr.expr;
        minidump_header.assert_info.file = assert_expr.file;
...
#if defined(MTK_SAVE_LOG_AND_CONTEXT_DUMP_ENABLE)
#if (PRODUCT_VERSION == 7686) || (PRODUCT_VERSION == 7698)
    minidump_header.reason = EXCEPTION_USAGE_FAULT;
    if (assert_expr.is_valid) {
        minidump_header.assert_info.is_valid = assert_expr.is_valid;
        minidump_header.assert_info.expr = assert_expr.expr;
        minidump_header.assert_info.file = assert_expr.file;
...
#if defined(MTK_SAVE_LOG_AND_CONTEXT_DUMP_ENABLE)
#if (PRODUCT_VERSION == 7686) || (PRODUCT_VERSION == 7698)
    minidump_header.reason = EXCEPTION_DEBUGMON_FAULT;
    if (assert_expr.is_valid) {
        minidump_header.assert_info.is_valid = assert_expr.is_valid;
        minidump_header.assert_info.expr = assert_expr.expr;
        minidump_header.assert_info.file = assert_expr.file;
