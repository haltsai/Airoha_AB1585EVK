...
#elif (defined(DAIR_BTA_IC_PREMIUM_G2) || defined(AIR_BTA_IC_PREMIUM_G3))
...
void bootreason_init(void)
{
    ...
    if (exception_config_mode.exception_mode_t.systemhang_pmic_mode != 1) {
        printf("systemhang_pmic_mode is disable!!!\r\n");
    }

    printf("[pc][s] 0x%x\r\n", *((volatile unsigned int *)0xE0101000));
    printf("[lr][s] 0x%x\r\n", *((volatile unsigned int *)0xE0101008));
    printf("[pc][ns] 0x%x\r\n", *((volatile unsigned int *)0xE0101004));
    printf("[lr][ns] 0x%x\r\n", *((volatile unsigned int *)0xE010100C));
    ...
    switch (lastreason) {
        ...
        case BOOTREASON_SLEEPERROR:
            ...
            if (EXCEPTION_STATUS_OK != exception_minidump_region_query_latest_index(&dump_index)) {
                printf("exception_minidump_region_query_latest_index error !!!\r\n");
            ...
            if (EXCEPTION_STATUS_OK != exception_minidump_region_query_info(dump_index,
                                                                            &dump_addr,
                                                                            &dump_len)) {
                printf("exception_minidump_region_query_info error !!!\r\n");
                ret = BOOTREASON_STATUS_ERROR;
                break;
            }
