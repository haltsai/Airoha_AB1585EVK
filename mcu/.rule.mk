...
$(BUILD_DIR)/%.o: $(SOURCE_DIR)/%.c
	...
	@if [ -n "$(OVERRIDE_CFLAGS)" ]; then \
		...
		echo $(CC) @$(basename $@)_OVERRIDE_CFLAGS -c $< -o $@ >> $(BUILD_LOG); \
		$(CC) @$(basename $@)_OVERRIDE_CFLAGS -c $< -o $@ 1>>$(BUILD_LOG) 2>>$(ERR_LOG); \
	else \
                echo $(sort $(CFLAGS)) > $(basename $@)_CFLAGS; \
                echo $(CC) @$(basename $@)_CFLAGS -MMD -MF $(basename $@).d -c $< -o $@ >> $(BUILD_LOG); \
                $(CC) @$(basename $@)_CFLAGS -MMD -MF $(basename $@).d -c $< -o $@ 1>>$(BUILD_LOG) 2>>$(ERR_LOG); \
	fi; \
	if [ "$$?" != "0" ]; then \
